---
storyId: "173.1"
storyTitle: "Migrate Settings Shell and Overview"
epic: "173-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-173-1-settings-shell
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.1 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 173.1 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** `/settings`; settings overview plus shared two-column/navigation layout for seven settings routes.
- **Required outcome:** Migrate Settings Shell and Overview. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Epics 166-FE and Story 167.1; this Story owns the settings layout consumed by 173.2–173.7.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_1["Story 173.1"]
  P2["Story 167.1"] --> S173_1["Story 173.1"]
```
...
- [ ] Confirm Story ID/title parity: `173.1` / `Migrate Settings Shell and Overview` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.1 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.1. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-1-settings-shell`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-1-settings-shell`
...
test ! -e "/private/tmp/wb-repricer-fe-173-1-settings-shell"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-1-settings-shell" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-1-settings-shell" "/private/tmp/wb-repricer-fe-173-1-settings-shell" main
git -C "/private/tmp/wb-repricer-fe-173-1-settings-shell" status --short --branch
```
...
- Owned Surface and settings-shell tests/fixtures.
- Canonical owned surface: `src/app/(dashboard)/settings/page.tsx`, `src/app/(dashboard)/settings/layout.tsx`, settings navigation components, exclusive tests.
...
- tokens/primitives/AppShell, settings APIs/hooks/types, child-route exclusive components.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.1.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **overview loading/empty/error where applicable, active navigation, restricted item, compact/mobile navigation.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/settings/page.tsx`, `src/app/(dashboard)/settings/layout.tsx`, settings navigation components, exclusive tests.
2. Deliver the user value and interaction surface exactly as recorded: `/settings`; settings overview plus shared two-column/navigation layout for seven settings routes.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: overview loading/empty/error where applicable, active navigation, restricted item, compact/mobile navigation.
6. Apply the Story contract for density and responsive behavior: two-column desktop, intentional stacked/Sheet navigation on narrow widths; no table/chart requirement.
7. Apply the Story accessibility contract: named settings navigation, visible current item, logical heading/focus order, mobile Sheet close/Escape/focus return.
...
- `npm test -- --run 'src/app/(dashboard)/settings' src/components/custom/settings`
- `npm run test:e2e -- e2e/settings-pages.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-1-settings-shell" diff --check
```
...
- `layout/navigation tests and responsive/theme screenshots for all navigation states.`;
- canonical local-validation intent: targeted settings-shell tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.1/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** two-column desktop, intentional stacked/Sheet navigation on narrow widths; no table/chart requirement.
- **Story accessibility contract:** named settings navigation, visible current item, logical heading/focus order, mobile Sheet close/Escape/focus return.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.1.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-1-settings-shell diff --check
git -C /private/tmp/wb-repricer-fe-173-1-settings-shell status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-1-settings-shell" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-1-settings-shell" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-1-settings-shell" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-1-settings-shell diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-1-settings-shell commit \
  -m "feat(settings): migrate settings shell and overview" \
  -m "Story 173.1. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-1-settings-shell push -u origin cdx/epic-173-story-1-settings-shell
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-1-settings-shell" \
  --title "Story 173.1: Migrate Settings Shell and Overview" \
  --body "Implements BMAD Story 173.1 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-1-settings-shell"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-1-settings-shell"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-1-settings-shell"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-1-settings-shell"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-1-settings-shell"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-1-settings-shell"
...
- `/private/tmp/wb-repricer-fe-173-1-settings-shell` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.1 worktree;
- local and remote branch queries return no `cdx/epic-173-story-1-settings-shell`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: shared-owner inventory, merge SHA, branch deletion, and removed worktree.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of `/settings`; settings overview plus shared two-column/navigation layout for seven settings routes. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Epics 166-FE and Story 167.1; this Story owns the settings layout consumed by 173.2–173.7. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: two-column desktop, intentional stacked/Sheet navigation on narrow widths; no table/chart requirement. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: named settings navigation, visible current item, logical heading/focus order, mobile Sheet close/Escape/focus return. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: overview loading/empty/error where applicable, active navigation, restricted item, compact/mobile navigation. | Test each applicable fixture and block merge on an unhandled state. |
