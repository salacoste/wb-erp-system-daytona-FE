---
storyId: "173.11"
storyTitle: "Migrate SKU Packaging"
epic: "173-FE"
...
branch: cdx/epic-173-story-11-sku-packaging
temporaryWorktree: /private/tmp/wb-repricer-fe-173-11-sku-packaging
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.11 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 173.11 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** SKU packaging list/form, validation, edit/save states.
- **Required outcome:** Migrate SKU Packaging. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Stories 173.8 and 173.10 where box-type presentation is consumed.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_11["Story 173.11"]
  P2["Story 167.1"] --> S173_11["Story 173.11"]
  P3["Story 173.8"] --> S173_11["Story 173.11"]
  P4["Story 173.10"] --> S173_11["Story 173.11"]
```
...
- [ ] Verify the merge SHA for **Story 173.10** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `173.11` / `Migrate SKU Packaging` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.11 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.11. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-11-sku-packaging`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-11-sku-packaging`
...
test ! -e "/private/tmp/wb-repricer-fe-173-11-sku-packaging"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-11-sku-packaging" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-11-sku-packaging" "/private/tmp/wb-repricer-fe-173-11-sku-packaging" main
git -C "/private/tmp/wb-repricer-fe-173-11-sku-packaging" status --short --branch
```
...
- owned packaging files/tests.
- Canonical owned surface: route entry and packaging-exclusive shipment components/tests.
...
- foundation/AppShell, box-type owner, shipment API/hooks/types/packing rules.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.11.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty, valid/invalid mapping, pending, success, failure.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route entry and packaging-exclusive shipment components/tests.
2. Deliver the user value and interaction surface exactly as recorded: SKU packaging list/form, validation, edit/save states.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty, valid/invalid mapping, pending, success, failure.
6. Apply the Story contract for density and responsive behavior: SKU/package/status/action primary columns; narrow row detail.
7. Apply the Story accessibility contract: entity-specific labels/actions, unit descriptions, validation summary, result announcement.
...
- `npm test -- --run 'src/app/(dashboard)/shipments/sku-packaging' src/components/custom/sku-packaging`
- `npm run test:e2e -- e2e/sku-packaging-page.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-11-sku-packaging" diff --check
```
...
- `packaging validation/save tests and responsive/theme screenshots.`;
- canonical local-validation intent: targeted packaging tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.11/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** SKU/package/status/action primary columns; narrow row detail.
- **Story accessibility contract:** entity-specific labels/actions, unit descriptions, validation summary, result announcement.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.11.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-11-sku-packaging diff --check
git -C /private/tmp/wb-repricer-fe-173-11-sku-packaging status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-11-sku-packaging" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-11-sku-packaging" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-11-sku-packaging" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-11-sku-packaging diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-11-sku-packaging commit \
  -m "feat(shipments): migrate sku packaging" \
  -m "Story 173.11. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-11-sku-packaging push -u origin cdx/epic-173-story-11-sku-packaging
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-11-sku-packaging" \
  --title "Story 173.11: Migrate SKU Packaging" \
  --body "Implements BMAD Story 173.11 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-11-sku-packaging"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-11-sku-packaging"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-11-sku-packaging"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-11-sku-packaging"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-11-sku-packaging"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-11-sku-packaging"
...
- `/private/tmp/wb-repricer-fe-173-11-sku-packaging` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.11 worktree;
- local and remote branch queries return no `cdx/epic-173-story-11-sku-packaging`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: branch/worktree cleanup and no shared-owner drift.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of SKU packaging list/form, validation, edit/save states. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Stories 173.8 and 173.10 where box-type presentation is consumed. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: SKU/package/status/action primary columns; narrow row detail. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: entity-specific labels/actions, unit descriptions, validation summary, result announcement. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty, valid/invalid mapping, pending, success, failure. | Test each applicable fixture and block merge on an unhandled state. |
