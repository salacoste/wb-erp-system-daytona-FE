---
storyId: "171.2"
storyTitle: "Migrate AI Admin Model Governance"
epic: "171-FE"
...
branch: cdx/epic-171-story-2-ai-admin-models-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.2: Migrate AI Admin Model Governance

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1712-migrate-ai-admin-model-governance` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As an authorized AI administrator, I want `/analytics/ai-admin/models` to list governed model versions and expose rollback status safely, so that I can inspect model lifecycle and perform an authorized rollback with exact scope.
- **Resolved owned surface:** `/analytics/ai-admin/models`; its `page.tsx`, complete `components/**` tree (`AdminModelsList`, content, table, pagination, rollback dialog, helpers), and colocated tests.
...
- **Exact branch:** `cdx/epic-171-story-2-ai-admin-models-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-2-ai-admin-models-shadcn" "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/ai-admin/models/**`.
- **Resolved owned files/tree:** `/analytics/ai-admin/models`; its `page.tsx`, complete `components/**` tree (`AdminModelsList`, content, table, pagination, rollback dialog, helpers), and colocated tests.
...
- **Canonical prohibition:** FS, `/analytics/models/**`, and sibling AI-admin routes.
...
1. From `/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/ai-admin/models"
   ```
...
4. Capture deterministic baseline evidence for: SC plus unknown model status, rollback ineligible/conflict/pending/success/failure, and restricted administrator states.
...
5. **Apply the route-specific data contract:** RTC; models table caption names governed versions, primary column is model/version identity, lifecycle/status/date/primary metric/rollback stay reachable; pagination preserves state. No chart.
6. **Apply accessibility and responsive behavior:** AX; rollback uses destructive AlertDialog semantics, repeated controls name model/version, status is non-color, and focus returns to the invoking row. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/ai-admin/models` and `STORY_ID=171.2`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn"
npx vitest run "src/app/(dashboard)/analytics/ai-admin/models"
npm run test:e2e -- --grep "Story 171.2-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" diff --check
```
...
- **State matrix:** SC plus unknown model status, rollback ineligible/conflict/pending/success/failure, and restricted administrator states.
- **Route-specific table/chart contract:** RTC; models table caption names governed versions, primary column is model/version identity, lifecycle/status/date/primary metric/rollback stay reachable; pagination preserves state. No chart.
...
- **Required Story evidence:** VE plus restricted, rollback-ineligible, unknown status, pagination, and rollback lifecycle evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" commit \
  -m "feat(analytics): migrate Story 171.2 route to shadcn" \
  -m "Story 171.2-FE — Migrate AI Admin Model Governance. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn" push -u origin "cdx/epic-171-story-2-ai-admin-models-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-2-ai-admin-models-shadcn" \
  --title "[Story 171.2-FE] Migrate AI Admin Model Governance" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-2-ai-admin-models-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-2-ai-admin-models-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-2-ai-admin-models-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-2-ai-admin-models-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-2-ai-admin-models-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus unknown model status, rollback ineligible/conflict/pending/success/failure, and restricted administrator states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; models table caption names governed versions, primary column is model/version identity, lifecycle/status/date/primary metric/rollback stay reachable; pagination preserves state. No chart. |
| Keyboard/focus/contrast regression | Apply AX; rollback uses destructive AlertDialog semantics, repeated controls name model/version, status is non-color, and focus returns to the invoking row. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-2-ai-admin-models-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-2-ai-admin-models-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-2-ai-admin-models-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
