...
branch: cdx/epic-172-story-15-orders-fbo
temporaryWorktree: /private/tmp/wb-repricer-fe-172-15-orders-fbo
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.15 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_15["Story 172.15"]
  P2["Story 167.1"] --> S172_15["Story 172.15"]
  P3["Story 172.14"] --> S172_15["Story 172.15"]
```
...
- [ ] Confirm Story ID/title parity: `172.15` / `Migrate FBO Orders` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.15 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.15. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-15-orders-fbo`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-15-orders-fbo`
...
test ! -e "/private/tmp/wb-repricer-fe-172-15-orders-fbo"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-15-orders-fbo" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-15-orders-fbo" "/private/tmp/wb-repricer-fe-172-15-orders-fbo" main
git -C "/private/tmp/wb-repricer-fe-172-15-orders-fbo" status --short --branch
```
...
- Owned Surface and FBO fixtures/tests.
- Canonical owned surface: `src/app/(dashboard)/orders/fbo/**`, FBO-exclusive components/tests.
...
- foundation/AppShell, order shared owner files, API/hooks/types, integrity route.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.15.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty, stale/partial, error, populated.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/orders/fbo/**`, FBO-exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: FBO aggregates, filters, sales/orders table, statuses and actions.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty, stale/partial, error, populated.
6. Apply the Story contract for density and responsive behavior: fulfillment identifier/status/amount/date/action primary columns; server controls preserved.
7. Apply the Story accessibility contract: semantic table, sort/filter names, status text, dates and currency precision.
...
- `npm test -- --run 'src/app/(dashboard)/orders/fbo'`
- `npm run test:e2e -- e2e/orders-fbo.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-15-orders-fbo" diff --check
```
...
- `FBO table/filter tests and responsive/theme screenshots.`;
- canonical local-validation intent: targeted FBO tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.15/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** fulfillment identifier/status/amount/date/action primary columns; server controls preserved.
- **Story accessibility contract:** semantic table, sort/filter names, status text, dates and currency precision.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.15.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-15-orders-fbo diff --check
git -C /private/tmp/wb-repricer-fe-172-15-orders-fbo status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-15-orders-fbo" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-15-orders-fbo" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-15-orders-fbo" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-15-orders-fbo diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-15-orders-fbo commit \
  -m "feat(orders): migrate fbo orders" \
  -m "Story 172.15. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-15-orders-fbo push -u origin cdx/epic-172-story-15-orders-fbo
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-15-orders-fbo" \
  --title "Story 172.15: Migrate FBO Orders" \
  --body "Implements BMAD Story 172.15 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-15-orders-fbo"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-15-orders-fbo"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-15-orders-fbo"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-15-orders-fbo"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-15-orders-fbo"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-15-orders-fbo"
...
- `/private/tmp/wb-repricer-fe-172-15-orders-fbo` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.15 worktree;
- local and remote branch queries return no `cdx/epic-172-story-15-orders-fbo`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: completed lifecycle record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of FBO aggregates, filters, sales/orders table, statuses and actions. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: fulfillment identifier/status/amount/date/action primary columns; server controls preserved. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: semantic table, sort/filter names, status text, dates and currency precision. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty, stale/partial, error, populated. | Test each applicable fixture and block merge on an unhandled state. |
