---
storyId: "173.7"
storyTitle: "Migrate Tax Settings"
epic: "173-FE"
...
branch: cdx/epic-173-story-7-settings-tax
temporaryWorktree: /private/tmp/wb-repricer-fe-173-7-settings-tax
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.7 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 173.7 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** tax form, rate/type context, validation, warning, save/reset.
- **Required outcome:** Migrate Tax Settings. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 173.1 and financial presentation foundation.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_7["Story 173.7"]
  P2["Story 167.1"] --> S173_7["Story 173.7"]
  P3["Story 173.1"] --> S173_7["Story 173.7"]
```
...
- [ ] Confirm Story ID/title parity: `173.7` / `Migrate Tax Settings` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.7 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.7. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-7-settings-tax`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-7-settings-tax`
...
test ! -e "/private/tmp/wb-repricer-fe-173-7-settings-tax"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-7-settings-tax" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-7-settings-tax" "/private/tmp/wb-repricer-fe-173-7-settings-tax" main
git -C "/private/tmp/wb-repricer-fe-173-7-settings-tax" status --short --branch
```
...
- Owned Surface and tax fixtures/tests.
- Canonical owned surface: route directory and tax-exclusive components/tests.
...
- foundation/settings shell, tax API/hooks/types/calculations.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.7.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, pristine/dirty, valid, invalid, unusual warning, pending, success, failure.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and tax-exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: tax form, rate/type context, validation, warning, save/reset.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, pristine/dirty, valid, invalid, unusual warning, pending, success, failure.
6. Apply the Story contract for density and responsive behavior: focused form; percent/period units remain visible.
7. Apply the Story accessibility contract: labeled rate/type, warning confirmation, errors, focus/announcement, non-color consequence.
...
- `npm test -- --run 'src/app/(dashboard)/settings/tax' src/components/custom/settings/__tests__/TaxSettingsForm.test.tsx src/lib/__tests__/tax-display-helpers.test.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-7-settings-tax" diff --check
```
...
- `tax form boundary tests and responsive/theme screenshots.`;
- canonical local-validation intent: targeted tax tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.7/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** focused form; percent/period units remain visible.
- **Story accessibility contract:** labeled rate/type, warning confirmation, errors, focus/announcement, non-color consequence.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.7.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-7-settings-tax diff --check
git -C /private/tmp/wb-repricer-fe-173-7-settings-tax status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-7-settings-tax" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-7-settings-tax" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-7-settings-tax" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-7-settings-tax diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-7-settings-tax commit \
  -m "feat(settings): migrate tax settings" \
  -m "Story 173.7. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-7-settings-tax push -u origin cdx/epic-173-story-7-settings-tax
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-7-settings-tax" \
  --title "Story 173.7: Migrate Tax Settings" \
  --body "Implements BMAD Story 173.7 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-7-settings-tax"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-7-settings-tax"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-7-settings-tax"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-7-settings-tax"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-7-settings-tax"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-7-settings-tax"
...
- `/private/tmp/wb-repricer-fe-173-7-settings-tax` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.7 worktree;
- local and remote branch queries return no `cdx/epic-173-story-7-settings-tax`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: calculation-semantics proof and cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of tax form, rate/type context, validation, warning, save/reset. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 173.1 and financial presentation foundation. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: focused form; percent/period units remain visible. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: labeled rate/type, warning confirmation, errors, focus/announcement, non-color consequence. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, pristine/dirty, valid, invalid, unusual warning, pending, success, failure. | Test each applicable fixture and block merge on an unhandled state. |
