---
storyId: "171.3"
storyTitle: "Migrate AI Preferences"
epic: "171-FE"
...
branch: cdx/epic-171-story-3-ai-preferences-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.3: Migrate AI Preferences

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1713-migrate-ai-preferences` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As an authorized user, I want `/analytics/ai-admin/preferences` to explain and save AI preferences accessibly, so that I can configure allowed AI behavior and understand whether my changes were saved.
- **Resolved owned surface:** `/analytics/ai-admin/preferences`; its `page.tsx`, `components/AiPreferencesForm.tsx`, and colocated tests.
...
- **Exact branch:** `cdx/epic-171-story-3-ai-preferences-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-3-ai-preferences-shadcn" "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/ai-admin/preferences/**`.
- **Resolved owned files/tree:** `/analytics/ai-admin/preferences`; its `page.tsx`, `components/AiPreferencesForm.tsx`, and colocated tests.
...
- **Canonical prohibition:** FS and sibling AI-admin routes.
...
1. From `/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/ai-admin/preferences"
   ```
...
4. Capture deterministic baseline evidence for: SC plus pristine/dirty, validation error, submitting, save success/failure/conflict, and unsaved-change states.
...
5. **Apply the route-specific data contract:** RTC form clauses; focused form uses constrained readable width and full-width mobile actions where appropriate. No table/chart is introduced.
6. **Apply accessibility and responsive behavior:** AX; all controls have persistent visible labels/descriptions, grouped choices use native/appropriate semantics, errors are associated and summarized, success is announced without focus theft. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/ai-admin/preferences` and `STORY_ID=171.3`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn"
npx vitest run "src/app/(dashboard)/analytics/ai-admin/preferences"
npm run test:e2e -- --grep "Story 171.3-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" diff --check
```
...
- **State matrix:** SC plus pristine/dirty, validation error, submitting, save success/failure/conflict, and unsaved-change states.
- **Route-specific table/chart contract:** RTC form clauses; focused form uses constrained readable width and full-width mobile actions where appropriate. No table/chart is introduced.
...
- **Required Story evidence:** VE plus dirty, validation, restricted, conflict, save-failure-retained-input, and success evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" commit \
  -m "feat(analytics): migrate Story 171.3 route to shadcn" \
  -m "Story 171.3-FE — Migrate AI Preferences. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn" push -u origin "cdx/epic-171-story-3-ai-preferences-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-3-ai-preferences-shadcn" \
  --title "[Story 171.3-FE] Migrate AI Preferences" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-3-ai-preferences-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-3-ai-preferences-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-3-ai-preferences-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-3-ai-preferences-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-3-ai-preferences-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus pristine/dirty, validation error, submitting, save success/failure/conflict, and unsaved-change states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC form clauses; focused form uses constrained readable width and full-width mobile actions where appropriate. No table/chart is introduced. |
| Keyboard/focus/contrast regression | Apply AX; all controls have persistent visible labels/descriptions, grouped choices use native/appropriate semantics, errors are associated and summarized, success is announced without focus theft. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-3-ai-preferences-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-3-ai-preferences-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-3-ai-preferences-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
