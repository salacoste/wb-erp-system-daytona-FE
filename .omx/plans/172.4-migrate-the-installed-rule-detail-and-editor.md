---
storyId: "172.4"
storyTitle: "Migrate the Installed Rule Detail and Editor"
epic: "172-FE"
...
branch: cdx/epic-172-story-4-installed-rule-detail
temporaryWorktree: /private/tmp/wb-repricer-fe-172-4-installed-rule-detail
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.4 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.4 scope below takes precedence.
...
- **Required outcome:** Migrate the Installed Rule Detail and Editor. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 172.3 and foundation/AppShell prerequisites.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_4["Story 172.4"]
  P2["Story 167.1"] --> S172_4["Story 172.4"]
  P3["Story 172.3"] --> S172_4["Story 172.4"]
```
...
- [ ] Verify the merge SHA for **Story 172.3** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `172.4` / `Migrate the Installed Rule Detail and Editor` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.4 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.4. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-4-installed-rule-detail`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-4-installed-rule-detail`
...
test ! -e "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-4-installed-rule-detail" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-4-installed-rule-detail" "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail" main
git -C "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail" status --short --branch
```
...
- Owned Surface and editor E2E/visual fixtures.
- Canonical owned surface: `src/app/(dashboard)/automation/installed-rules/[id]/**` and exclusive editor components/tests.
...
- tokens/primitives/AppShell, automation service/API/types, canned/list ownership.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.4.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading entity, not found, permission, pristine/dirty form, validation error, warning acknowledgement, save pending/success/failure, conflict where supported.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/automation/installed-rules/[id]/**` and exclusive editor components/tests.
2. Deliver the user value and interaction surface exactly as recorded: dynamic installed-rule detail/editor; inspect, validate, acknowledge, save, and verify rule configuration.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading entity, not found, permission, pristine/dirty form, validation error, warning acknowledgement, save pending/success/failure, conflict where supported.
6. Apply the Story contract for density and responsive behavior: focused form layout, clear mobile action order; no table/chart requirement unless already rendered.
7. Apply the Story accessibility contract: labels/descriptions/errors, error summary, unsaved-change focus behavior, Dialog/AlertDialog lifecycle, status announcement.
...
- `npm test -- --run 'src/app/(dashboard)/automation/installed-rules/[id]' src/lib/api/__tests__/automation.test.ts`
- `npm run test:e2e -- e2e/automation/installed-rule-editor.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail" diff --check
```
...
- `editor validation/writeback tests plus narrow/desktop/theme screenshots.`;
- canonical local-validation intent: targeted dynamic-route and form tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.4/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** focused form layout, clear mobile action order; no table/chart requirement unless already rendered.
- **Story accessibility contract:** labels/descriptions/errors, error summary, unsaved-change focus behavior, Dialog/AlertDialog lifecycle, status announcement.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.4.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-4-installed-rule-detail diff --check
git -C /private/tmp/wb-repricer-fe-172-4-installed-rule-detail status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-4-installed-rule-detail diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-4-installed-rule-detail commit \
  -m "feat(automation): migrate the installed rule detail and editor" \
  -m "Story 172.4. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-4-installed-rule-detail push -u origin cdx/epic-172-story-4-installed-rule-detail
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-4-installed-rule-detail" \
  --title "Story 172.4: Migrate the Installed Rule Detail and Editor" \
  --body "Implements BMAD Story 172.4 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-4-installed-rule-detail"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-4-installed-rule-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-4-installed-rule-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-4-installed-rule-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-4-installed-rule-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-4-installed-rule-detail"
...
- `/private/tmp/wb-repricer-fe-172-4-installed-rule-detail` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.4 worktree;
- local and remote branch queries return no `cdx/epic-172-story-4-installed-rule-detail`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: merge, branch deletion, and worktree-removal evidence.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of dynamic installed-rule detail/editor; inspect, validate, acknowledge, save, and verify rule configuration. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 172.3 and foundation/AppShell prerequisites. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: focused form layout, clear mobile action order; no table/chart requirement unless already rendered. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: labels/descriptions/errors, error summary, unsaved-change focus behavior, Dialog/AlertDialog lifecycle, status announcement. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading entity, not found, permission, pristine/dirty form, validation error, warning acknowledgement, save pending/success/failure, conflict where supported. | Test each applicable fixture and block merge on an unhandled state. |
