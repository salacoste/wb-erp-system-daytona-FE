---
storyId: "171.6"
storyTitle: "Migrate Model Registry and Training Entry"
epic: "171-FE"
...
branch: cdx/epic-171-story-6-model-registry-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-6-model-registry-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.6: Migrate Model Registry and Training Entry

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1716-migrate-model-registry-and-training-entry` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As an authorized model user, I want `/analytics/models` to list model identity/status and expose training entry safely, so that I can find a model, inspect its lifecycle, and start only a valid training operation.
- **Resolved owned surface:** `/analytics/models`; its `page.tsx`, `components/ModelListSection.tsx`, `TrainModelButton.tsx`, `model-list-helpers.ts`, and colocated tests.
...
- **Exact branch:** `cdx/epic-171-story-6-model-registry-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-6-model-registry-shadcn" "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/models/page.tsx` and root `components/**`/tests; dynamic `[id]/**` routes are excluded.
- **Resolved owned files/tree:** `/analytics/models`; its `page.tsx`, `components/ModelListSection.tsx`, `TrainModelButton.tsx`, `model-list-helpers.ts`, and colocated tests.
...
- **Canonical prohibition:** FS, `ai-admin/models/**`, and `analytics/models/[id]/**`.
...
1. From `/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/models/components"
   ```
...
4. Capture deterministic baseline evidence for: SC plus unknown status, training unavailable/already-running/pending/success/failure, and restricted action states.
...
5. **Apply the route-specific data contract:** RTC; model list/table primary column is model identity, type/version/status/date/primary metric/action remain reachable; detail links preserve model ID. No chart.
6. **Apply accessibility and responsive behavior:** AX; training control names the model, status is non-color, confirmation/result focus is deliberate, and repeated detail actions are distinguishable. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/models/components` and `STORY_ID=171.6`; include the models root page test.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn"
npx vitest run "src/app/(dashboard)/analytics/models/components"
npm run test:e2e -- --grep "Story 171.6-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" diff --check
```
...
- **State matrix:** SC plus unknown status, training unavailable/already-running/pending/success/failure, and restricted action states.
- **Route-specific table/chart contract:** RTC; model list/table primary column is model identity, type/version/status/date/primary metric/action remain reachable; detail links preserve model ID. No chart.
...
- **Required Story evidence:** VE plus empty registry, unknown status, training conflict/pending/failure/success, and narrow list evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" commit \
  -m "feat(analytics): migrate Story 171.6 route to shadcn" \
  -m "Story 171.6-FE — Migrate Model Registry and Training Entry. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn" push -u origin "cdx/epic-171-story-6-model-registry-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-6-model-registry-shadcn" \
  --title "[Story 171.6-FE] Migrate Model Registry and Training Entry" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-6-model-registry-shadcn"
...
Canonical cleanup requirement: CE, including proof all dynamic model routes were untouched.

After the merge SHA is proven on `main`, execute in this order:

```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-6-model-registry-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-6-model-registry-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-6-model-registry-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-6-model-registry-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus unknown status, training unavailable/already-running/pending/success/failure, and restricted action states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; model list/table primary column is model identity, type/version/status/date/primary metric/action remain reachable; detail links preserve model ID. No chart. |
| Keyboard/focus/contrast regression | Apply AX; training control names the model, status is non-color, confirmation/result focus is deliberate, and repeated detail actions are distinguishable. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-6-model-registry-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-6-model-registry-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-6-model-registry-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
