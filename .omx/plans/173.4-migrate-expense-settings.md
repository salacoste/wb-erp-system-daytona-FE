---
storyId: "173.4"
storyTitle: "Migrate Expense Settings"
epic: "173-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-173-4-settings-expenses
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.4 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_4["Story 173.4"]
  P2["Story 167.1"] --> S173_4["Story 173.4"]
  P3["Story 173.1"] --> S173_4["Story 173.4"]
```
...
- [ ] Confirm Story ID/title parity: `173.4` / `Migrate Expense Settings` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.4 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.4. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-4-settings-expenses`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-4-settings-expenses`
...
test ! -e "/private/tmp/wb-repricer-fe-173-4-settings-expenses"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-4-settings-expenses" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-4-settings-expenses" "/private/tmp/wb-repricer-fe-173-4-settings-expenses" main
git -C "/private/tmp/wb-repricer-fe-173-4-settings-expenses" status --short --branch
```
...
- Owned Surface and expense fixtures/tests.
- Canonical owned surface: route directory and expense-settings exclusive components/tests.
...
- tokens/primitives/AppShell/settings shell, expense API/hooks/types/calculations.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.4.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, populated, validation error, save/delete pending/success/failure.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and expense-settings exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: expense cards/list, add/edit/delete dialogs, validation and result feedback.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, populated, validation error, save/delete pending/success/failure.
6. Apply the Story contract for density and responsive behavior: expense name/value/period/status/action retained; dialogs fit narrow viewports.
7. Apply the Story accessibility contract: currency/period labels, entity-specific actions, error association, AlertDialog focus return.
...
- `npm test -- --run 'src/app/(dashboard)/settings/expenses'`
- `npm run test:e2e -- e2e/expenses-page.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-4-settings-expenses" diff --check
```
...
- `CRUD regression plus responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted expense tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.4/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** expense name/value/period/status/action retained; dialogs fit narrow viewports.
- **Story accessibility contract:** currency/period labels, entity-specific actions, error association, AlertDialog focus return.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.4.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-4-settings-expenses diff --check
git -C /private/tmp/wb-repricer-fe-173-4-settings-expenses status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-4-settings-expenses" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-4-settings-expenses" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-4-settings-expenses" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-4-settings-expenses diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-4-settings-expenses commit \
  -m "feat(settings): migrate expense settings" \
  -m "Story 173.4. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-4-settings-expenses push -u origin cdx/epic-173-story-4-settings-expenses
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-4-settings-expenses" \
  --title "Story 173.4: Migrate Expense Settings" \
  --body "Implements BMAD Story 173.4 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-4-settings-expenses"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-4-settings-expenses"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-4-settings-expenses"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-4-settings-expenses"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-4-settings-expenses"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-4-settings-expenses"
...
- `/private/tmp/wb-repricer-fe-173-4-settings-expenses` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.4 worktree;
- local and remote branch queries return no `cdx/epic-173-story-4-settings-expenses`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: CRUD proof and branch/worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of expense cards/list, add/edit/delete dialogs, validation and result feedback. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: expense name/value/period/status/action retained; dialogs fit narrow viewports. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: currency/period labels, entity-specific actions, error association, AlertDialog focus return. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, populated, validation error, save/delete pending/success/failure. | Test each applicable fixture and block merge on an unhandled state. |
