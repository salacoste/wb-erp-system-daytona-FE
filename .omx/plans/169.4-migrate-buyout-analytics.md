---
storyId: "169.4"
storyTitle: "Migrate Buyout Analytics"
epic: "169-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-169-4-buyout-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.4: Migrate Buyout Analytics

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1694-migrate-buyout-analytics` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an operations user, I want `/analytics/buyout` to show buyout performance, decliners, trends, and product evidence consistently, so that I can identify and investigate declining buyout behavior.
- **Resolved owned surface:** `/analytics/buyout`; its `page.tsx`, full `components/**` tree (summary widget/subcomponents, decliners list, trend chart/legend/tooltip/config, table/cells/columns, comparison utilities), and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-4-buyout-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-4-buyout-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-4-buyout-shadcn" "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/buyout/**`.
- **Resolved owned files/tree:** `/analytics/buyout`; its `page.tsx`, full `components/**` tree (summary widget/subcomponents, decliners list, trend chart/legend/tooltip/config, table/cells/columns, comparison utilities), and colocated tests.
...
- **Canonical prohibition:** FS and `/analytics/buyout-reconciliation/**`.
...
1. From `/private/tmp/wb-repricer-fe-169-4-buyout-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/buyout"
   ```
...
4. Capture deterministic baseline evidence for: SC plus missing comparison, anomalous negative delta, and zero-buyout (valid zero) states.
...
5. **Apply the route-specific data contract:** RTC; table primary column is product/SKU, buyout and order values use aligned percentages/counts; decliner action stays reachable. Trend chart distinguishes current/comparison and provides daily data alternative; tooltip and table use identical precision.
6. **Apply accessibility and responsive behavior:** AX; delta/decliner severity is non-color and chart evidence is keyboard/touch independent. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/buyout` and `STORY_ID=169.4`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn"
npx vitest run "src/app/(dashboard)/analytics/buyout"
npm run test:e2e -- --grep "Story 169.4-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" diff --check
```
...
- **State matrix:** SC plus missing comparison, anomalous negative delta, and zero-buyout (valid zero) states.
- **Route-specific table/chart contract:** RTC; table primary column is product/SKU, buyout and order values use aligned percentages/counts; decliner action stays reachable. Trend chart distinguishes current/comparison and provides daily data alternative; tooltip and table use identical precision.
- **Accessibility contract:** AX; delta/decliner severity is non-color and chart evidence is keyboard/touch independent.
- **Required Story evidence:** VE plus decliner, missing-comparison, chart alternative, and mobile table evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" commit \
  -m "feat(analytics): migrate Story 169.4 route to shadcn" \
  -m "Story 169.4-FE — Migrate Buyout Analytics. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn" push -u origin "cdx/epic-169-story-4-buyout-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-4-buyout-shadcn" \
  --title "[Story 169.4-FE] Migrate Buyout Analytics" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-4-buyout-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-4-buyout-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-4-buyout-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-4-buyout-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-4-buyout-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-4-buyout-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-4-buyout-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus missing comparison, anomalous negative delta, and zero-buyout (valid zero) states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; table primary column is product/SKU, buyout and order values use aligned percentages/counts; decliner action stays reachable. Trend chart distinguishes current/comparison and provides daily data alternative; tooltip and table use identical precision. |
| Keyboard/focus/contrast regression | Apply AX; delta/decliner severity is non-color and chart evidence is keyboard/touch independent. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-4-buyout-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-4-buyout-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-4-buyout-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-4-buyout-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
