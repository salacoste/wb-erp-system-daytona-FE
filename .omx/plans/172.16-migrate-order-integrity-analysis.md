---
storyId: "172.16"
storyTitle: "Migrate Order Integrity Analysis"
epic: "172-FE"
...
branch: cdx/epic-172-story-16-order-integrity
temporaryWorktree: /private/tmp/wb-repricer-fe-172-16-order-integrity
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.16 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_16["Story 172.16"]
  P2["Story 167.1"] --> S172_16["Story 172.16"]
  P3["Story 172.14"] --> S172_16["Story 172.16"]
```
...
- [ ] Verify the merge SHA for **Story 172.14** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `172.16` / `Migrate Order Integrity Analysis` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.16 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.16. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-16-order-integrity`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-16-order-integrity`
...
test ! -e "/private/tmp/wb-repricer-fe-172-16-order-integrity"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-16-order-integrity" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-16-order-integrity" "/private/tmp/wb-repricer-fe-172-16-order-integrity" main
git -C "/private/tmp/wb-repricer-fe-172-16-order-integrity" status --short --branch
```
...
- Owned Surface and integrity fixtures/tests.
- Canonical owned surface: `src/app/(dashboard)/orders/integrity/**` and integrity-exclusive components/tests.
...
- tokens/primitives/AppShell, order shared owner files, integrity API/hooks/types.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.16.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **checking, healthy, warning, failed, partial, empty, stale, route error, corrective action lifecycle.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/orders/integrity/**` and integrity-exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: integrity summary/status cards, anomaly detail, filters, tables, actions/dialogs.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: checking, healthy, warning, failed, partial, empty, stale, route error, corrective action lifecycle.
6. Apply the Story contract for density and responsive behavior: exception-first hierarchy; anomaly identifier/severity/evidence/action primary columns.
7. Apply the Story accessibility contract: severity label/icon/order, Dialog focus, keyboard recovery actions, live status updates moderated.
...
- `npm test -- --run 'src/app/(dashboard)/orders/integrity'`
- `npm run test:e2e -- e2e/orders-integrity.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-16-order-integrity" diff --check
```
...
- `integrity-state/action tests and health matrix screenshots.`;
- canonical local-validation intent: targeted integrity tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.16/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** exception-first hierarchy; anomaly identifier/severity/evidence/action primary columns.
- **Story accessibility contract:** severity label/icon/order, Dialog focus, keyboard recovery actions, live status updates moderated.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.16.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-16-order-integrity diff --check
git -C /private/tmp/wb-repricer-fe-172-16-order-integrity status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-16-order-integrity" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-16-order-integrity" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-16-order-integrity" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-16-order-integrity diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-16-order-integrity commit \
  -m "feat(orders): migrate order integrity analysis" \
  -m "Story 172.16. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-16-order-integrity push -u origin cdx/epic-172-story-16-order-integrity
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-16-order-integrity" \
  --title "Story 172.16: Migrate Order Integrity Analysis" \
  --body "Implements BMAD Story 172.16 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-16-order-integrity"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-16-order-integrity"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-16-order-integrity"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-16-order-integrity"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-16-order-integrity"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-16-order-integrity"
...
- `/private/tmp/wb-repricer-fe-172-16-order-integrity` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.16 worktree;
- local and remote branch queries return no `cdx/epic-172-story-16-order-integrity`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: review/merge and worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of integrity summary/status cards, anomaly detail, filters, tables, actions/dialogs. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 172.14 plus foundation/AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: exception-first hierarchy; anomaly identifier/severity/evidence/action primary columns. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: severity label/icon/order, Dialog focus, keyboard recovery actions, live status updates moderated. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: checking, healthy, warning, failed, partial, empty, stale, route error, corrective action lifecycle. | Test each applicable fixture and block merge on an unhandled state. |
