---
storyId: "173.10"
storyTitle: "Migrate Shipment Box Types"
epic: "173-FE"
...
branch: cdx/epic-173-story-10-box-types
temporaryWorktree: /private/tmp/wb-repricer-fe-173-10-box-types
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.10 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_10["Story 173.10"]
  P2["Story 167.1"] --> S173_10["Story 173.10"]
  P3["Story 173.8"] --> S173_10["Story 173.10"]
```
...
- [ ] Confirm Story ID/title parity: `173.10` / `Migrate Shipment Box Types` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.10 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.10. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-10-box-types`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-10-box-types`
...
test ! -e "/private/tmp/wb-repricer-fe-173-10-box-types"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-10-box-types" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-10-box-types" "/private/tmp/wb-repricer-fe-173-10-box-types" main
git -C "/private/tmp/wb-repricer-fe-173-10-box-types" status --short --branch
```
...
- Owned Surface and CRUD fixtures/tests.
- Canonical owned surface: route entry and `src/components/custom/box-types/**` plus exclusive tests.
...
- foundation/AppShell, shipment shared owner, API/hooks/types/dimension rules.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.10.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, populated, validation, create/edit pending/success/failure, deactivate confirmation.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route entry and `src/components/custom/box-types/**` plus exclusive tests.
2. Deliver the user value and interaction surface exactly as recorded: box-types table, empty state, dimension fields, create/edit/deactivate dialogs.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, populated, validation, create/edit pending/success/failure, deactivate confirmation.
6. Apply the Story contract for density and responsive behavior: type/name/dimensions/status/action primary data; responsive table and dialogs.
7. Apply the Story accessibility contract: dimension units, field errors, named row actions, AlertDialog focus return.
...
- `npm test -- --run 'src/app/(dashboard)/shipments/box-types' src/components/custom/box-types`
- `npm run test:e2e -- e2e/box-types-page.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-10-box-types" diff --check
```
...
- `box CRUD/dimension tests and responsive/theme screenshots.`;
- canonical local-validation intent: targeted box-type tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.10/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** type/name/dimensions/status/action primary data; responsive table and dialogs.
- **Story accessibility contract:** dimension units, field errors, named row actions, AlertDialog focus return.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.10.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-10-box-types diff --check
git -C /private/tmp/wb-repricer-fe-173-10-box-types status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-10-box-types" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-10-box-types" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-10-box-types" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-10-box-types diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-10-box-types commit \
  -m "feat(shipments): migrate shipment box types" \
  -m "Story 173.10. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-10-box-types push -u origin cdx/epic-173-story-10-box-types
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-10-box-types" \
  --title "Story 173.10: Migrate Shipment Box Types" \
  --body "Implements BMAD Story 173.10 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-10-box-types"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-10-box-types"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-10-box-types"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-10-box-types"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-10-box-types"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-10-box-types"
...
- `/private/tmp/wb-repricer-fe-173-10-box-types` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.10 worktree;
- local and remote branch queries return no `cdx/epic-173-story-10-box-types`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: CRUD proof and cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of box-types table, empty state, dimension fields, create/edit/deactivate dialogs. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: type/name/dimensions/status/action primary data; responsive table and dialogs. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: dimension units, field errors, named row actions, AlertDialog focus return. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, populated, validation, create/edit pending/success/failure, deactivate confirmation. | Test each applicable fixture and block merge on an unhandled state. |
