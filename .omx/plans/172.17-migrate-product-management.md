---
storyId: "172.17"
storyTitle: "Migrate Product Management"
epic: "172-FE"
...
branch: cdx/epic-172-story-17-products
temporaryWorktree: /private/tmp/wb-repricer-fe-172-17-products
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.17 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.17 scope below takes precedence.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_17["Story 172.17"]
  P2["Story 167.1"] --> S172_17["Story 172.17"]
```
...
- [ ] Confirm Story ID/title parity: `172.17` / `Migrate Product Management` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.17 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.17. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-17-products`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-17-products`
...
test ! -e "/private/tmp/wb-repricer-fe-172-17-products"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-17-products" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-17-products" "/private/tmp/wb-repricer-fe-172-17-products" main
git -C "/private/tmp/wb-repricer-fe-172-17-products" status --short --branch
```
...
- Owned Surface and product fixtures/tests.
- Canonical owned surface: `src/app/(dashboard)/products/**`, product-exclusive custom components/tests.
...
- tokens/primitives/AppShell, product API/hooks/types/business lifecycle.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.17.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty where supported, active/inactive/status states, action pending/success/failure, destructive confirmation.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/products/**`, product-exclusive custom components/tests.
2. Deliver the user value and interaction surface exactly as recorded: product list/lifecycle, statuses, actions, confirmation dialogs, loading/empty/error behavior.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty where supported, active/inactive/status states, action pending/success/failure, destructive confirmation.
6. Apply the Story contract for density and responsive behavior: product identity/status/primary metric/action retained; explicit list/table narrow strategy.
7. Apply the Story accessibility contract: product-specific action names, status text/icons, AlertDialog lifecycle, restored focus and announced result.
...
- `npm test -- --run 'src/app/(dashboard)/products' src/components/custom/products`
- `npm run test:e2e -- e2e/products-assortment.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-17-products" diff --check
```
...
- `product lifecycle tests plus responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted product tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.17/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** product identity/status/primary metric/action retained; explicit list/table narrow strategy.
- **Story accessibility contract:** product-specific action names, status text/icons, AlertDialog lifecycle, restored focus and announced result.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.17.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-17-products diff --check
git -C /private/tmp/wb-repricer-fe-172-17-products status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-17-products" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-17-products" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-17-products" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-17-products diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-17-products commit \
  -m "feat(products): migrate product management" \
  -m "Story 172.17. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-17-products push -u origin cdx/epic-172-story-17-products
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-17-products" \
  --title "Story 172.17: Migrate Product Management" \
  --body "Implements BMAD Story 172.17 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-17-products"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-17-products"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-17-products"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-17-products"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-17-products"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-17-products"
...
- `/private/tmp/wb-repricer-fe-172-17-products` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.17 worktree;
- local and remote branch queries return no `cdx/epic-172-story-17-products`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: final Epic 172 route cleanup record and no remaining Story worktree.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of product list/lifecycle, statuses, actions, confirmation dialogs, loading/empty/error behavior. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: product identity/status/primary metric/action retained; explicit list/table narrow strategy. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: product-specific action names, status text/icons, AlertDialog lifecycle, restored focus and announced result. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty where supported, active/inactive/status states, action pending/success/failure, destructive confirmation. | Test each applicable fixture and block merge on an unhandled state. |
