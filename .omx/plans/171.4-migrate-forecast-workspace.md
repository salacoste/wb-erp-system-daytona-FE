---
storyId: "171.4"
storyTitle: "Migrate Forecast Workspace"
epic: "171-FE"
...
branch: cdx/epic-171-story-4-forecast-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-4-forecast-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.4: Migrate Forecast Workspace

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1714-migrate-forecast-workspace` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As an owner/operations user, I want `/analytics/forecast` to explain readiness, AI engine status, parameters, forecast metrics, series, SKU evidence, and collection progress, so that I can understand forecasts and their limitations before planning inventory.
- **Resolved owned surface:** `/analytics/forecast`; its `page.tsx`, complete `components/**` tree (page/header, engine status, preferences/model selectors, readiness/collecting states, parameters/metrics, forecast chart/helpers, forecast/top-SKU tables, query/readiness helpers), and colocated tests.
...
- **Exact branch:** `cdx/epic-171-story-4-forecast-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-4-forecast-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-4-forecast-shadcn" "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/forecast/**`.
- **Resolved owned files/tree:** `/analytics/forecast`; its `page.tsx`, complete `components/**` tree (page/header, engine status, preferences/model selectors, readiness/collecting states, parameters/metrics, forecast chart/helpers, forecast/top-SKU tables, query/readiness helpers), and colocated tests.
...
- **Canonical prohibition:** FS and `/analytics/forecast-accuracy/**` or model route trees.
...
1. From `/private/tmp/wb-repricer-fe-171-4-forecast-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/forecast"
   ```
...
4. Capture deterministic baseline evidence for: SC plus preference-required, collecting/progress, engine degraded/unavailable, forecast ready, missing confidence band, and polling failure states.
...
5. **Apply the route-specific data contract:** RTC; forecast and top-SKU tables prioritize SKU/date/horizon with actual/forecast/delta and status. Forecast chart names actual/forecast/target/confidence series, horizon and units, provides accessible summary/data alternative, and retains selection/drill-down context.
6. **Apply accessibility and responsive behavior:** AX; progress and engine status have measured announcements/non-color labels; selectors expose current value; confidence and forecast distinctions are not color-only. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/forecast` and `STORY_ID=171.4`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn"
npx vitest run "src/app/(dashboard)/analytics/forecast"
npm run test:e2e -- --grep "Story 171.4-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" diff --check
```
...
- **State matrix:** SC plus preference-required, collecting/progress, engine degraded/unavailable, forecast ready, missing confidence band, and polling failure states.
- **Route-specific table/chart contract:** RTC; forecast and top-SKU tables prioritize SKU/date/horizon with actual/forecast/delta and status. Forecast chart names actual/forecast/target/confidence series, horizon and units, provides accessible summary/data alternative, and retains selection/drill-down context.
...
- **Required Story evidence:** VE plus collecting, engine unavailable, missing confidence band, ready forecast, and chart/table equivalence evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" commit \
  -m "feat(analytics): migrate Story 171.4 route to shadcn" \
  -m "Story 171.4-FE — Migrate Forecast Workspace. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn" push -u origin "cdx/epic-171-story-4-forecast-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-4-forecast-shadcn" \
  --title "[Story 171.4-FE] Migrate Forecast Workspace" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-4-forecast-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-4-forecast-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-4-forecast-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-4-forecast-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-4-forecast-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-4-forecast-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-4-forecast-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus preference-required, collecting/progress, engine degraded/unavailable, forecast ready, missing confidence band, and polling failure states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; forecast and top-SKU tables prioritize SKU/date/horizon with actual/forecast/delta and status. Forecast chart names actual/forecast/target/confidence series, horizon and units, provides accessible summary/data alternative, and retains selection/drill-down context. |
| Keyboard/focus/contrast regression | Apply AX; progress and engine status have measured announcements/non-color labels; selectors expose current value; confidence and forecast distinctions are not color-only. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-4-forecast-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-4-forecast-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-4-forecast-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-4-forecast-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
