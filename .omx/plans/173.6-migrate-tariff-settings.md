---
storyId: "173.6"
storyTitle: "Migrate Tariff Settings"
epic: "173-FE"
...
branch: cdx/epic-173-story-6-settings-tariffs
temporaryWorktree: /private/tmp/wb-repricer-fe-173-6-settings-tariffs
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.6 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_6["Story 173.6"]
  P2["Story 167.1"] --> S173_6["Story 173.6"]
  P3["Story 173.1"] --> S173_6["Story 173.6"]
```
...
- [ ] Confirm Story ID/title parity: `173.6` / `Migrate Tariff Settings` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.6 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.6. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-6-settings-tariffs`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-6-settings-tariffs`
...
test ! -e "/private/tmp/wb-repricer-fe-173-6-settings-tariffs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-6-settings-tariffs" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-6-settings-tariffs" "/private/tmp/wb-repricer-fe-173-6-settings-tariffs" main
git -C "/private/tmp/wb-repricer-fe-173-6-settings-tariffs" status --short --branch
```
...
- Owned Surface and tariff fixtures/tests.
- Canonical owned surface: route directory and tariff-exclusive components/tests.
...
- foundation/settings shell, tariff API/hooks/types/calculations.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.6.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, valid/pristine/dirty, validation error, pending, success, failure, partial/unavailable values.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and tariff-exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: tariff form, current values, units, validation, save/reset.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, valid/pristine/dirty, validation error, pending, success, failure, partial/unavailable values.
6. Apply the Story contract for density and responsive behavior: readable focused form with stable action order.
7. Apply the Story accessibility contract: units and descriptions associated, error summary where needed, save announcement.
...
- `npm test -- --run 'src/app/(dashboard)/settings/tariffs' src/components/custom/tariffs-admin`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-6-settings-tariffs" diff --check
```
...
- `form regression and responsive/theme screenshots.`;
- canonical local-validation intent: targeted tariff tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.6/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** readable focused form with stable action order.
- **Story accessibility contract:** units and descriptions associated, error summary where needed, save announcement.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.6.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-6-settings-tariffs diff --check
git -C /private/tmp/wb-repricer-fe-173-6-settings-tariffs status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-6-settings-tariffs" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-6-settings-tariffs" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-6-settings-tariffs" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-6-settings-tariffs diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-6-settings-tariffs commit \
  -m "feat(settings): migrate tariff settings" \
  -m "Story 173.6. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-6-settings-tariffs push -u origin cdx/epic-173-story-6-settings-tariffs
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-6-settings-tariffs" \
  --title "Story 173.6: Migrate Tariff Settings" \
  --body "Implements BMAD Story 173.6 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-6-settings-tariffs"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-6-settings-tariffs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-6-settings-tariffs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-6-settings-tariffs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-6-settings-tariffs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-6-settings-tariffs"
...
- `/private/tmp/wb-repricer-fe-173-6-settings-tariffs` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.6 worktree;
- local and remote branch queries return no `cdx/epic-173-story-6-settings-tariffs`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: merge and worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of tariff form, current values, units, validation, save/reset. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: readable focused form with stable action order. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: units and descriptions associated, error summary where needed, save announcement. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, valid/pristine/dirty, validation error, pending, success, failure, partial/unavailable values. | Test each applicable fixture and block merge on an unhandled state. |
