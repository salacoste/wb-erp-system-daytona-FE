---
storyId: "169.13"
storyTitle: "Migrate Supply Planning"
epic: "169-FE"
...
branch: cdx/epic-169-story-13-supply-planning-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.13: Migrate Supply Planning

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-16913-migrate-supply-planning` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an operations manager, I want `/analytics/supply-planning` to prioritize stockout risk, supply metrics, SKU rows, cost/trend detail, pagination, and export, so that I can plan replenishment and investigate the highest-risk item without losing queue context.
- **Resolved owned surface:** `/analytics/supply-planning`; its `page.tsx`, full `components/**` tree (header, risk cards, metrics, loading/empty, table/header/row/cells/pagination/export/filter hooks, detail left/right/cost/trend sections and calculations), and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-13-supply-planning-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-13-supply-planning-shadcn" "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/supply-planning/**`.
- **Resolved owned files/tree:** `/analytics/supply-planning`; its `page.tsx`, full `components/**` tree (header, risk cards, metrics, loading/empty, table/header/row/cells/pagination/export/filter hooks, detail left/right/cost/trend sections and calculations), and colocated tests.
...
1. From `/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/supply-planning"
   ```
...
4. Capture deterministic baseline evidence for: SC plus no-risk, detail-loading/error, selected/stale entity, export lifecycle, and cost/trend partial states.
...
5. **Apply the route-specific data contract:** RTC; table primary column is product/SKU with stockout status/risk, stock/sales/recommended supply, selection/detail action; mobile uses explicit list-to-detail return. Detail trend chart names period/units/series and provides data alternative; pagination and export preserve filters.
6. **Apply accessibility and responsive behavior:** AX; risk uses label/icon/order, selection/expanded/current state is programmatic, detail transition has deliberate focus/return. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/supply-planning` and `STORY_ID=169.13`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn"
npx vitest run "src/app/(dashboard)/analytics/supply-planning"
npm run test:e2e -- --grep "Story 169.13-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" diff --check
```
...
- **State matrix:** SC plus no-risk, detail-loading/error, selected/stale entity, export lifecycle, and cost/trend partial states.
- **Route-specific table/chart contract:** RTC; table primary column is product/SKU with stockout status/risk, stock/sales/recommended supply, selection/detail action; mobile uses explicit list-to-detail return. Detail trend chart names period/units/series and provides data alternative; pagination and export preserve filters.
...
- **Required Story evidence:** VE plus no-risk versus failure, list/detail context preservation, partial trend, and export evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" commit \
  -m "feat(analytics): migrate Story 169.13 route to shadcn" \
  -m "Story 169.13-FE — Migrate Supply Planning. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn" push -u origin "cdx/epic-169-story-13-supply-planning-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-13-supply-planning-shadcn" \
  --title "[Story 169.13-FE] Migrate Supply Planning" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-13-supply-planning-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-13-supply-planning-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-13-supply-planning-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-13-supply-planning-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-13-supply-planning-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus no-risk, detail-loading/error, selected/stale entity, export lifecycle, and cost/trend partial states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; table primary column is product/SKU with stockout status/risk, stock/sales/recommended supply, selection/detail action; mobile uses explicit list-to-detail return. Detail trend chart names period/units/series and provides data alternative; pagination and export preserve filters. |
| Keyboard/focus/contrast regression | Apply AX; risk uses label/icon/order, selection/expanded/current state is programmatic, detail transition has deliberate focus/return. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-13-supply-planning-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-13-supply-planning-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-13-supply-planning-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
