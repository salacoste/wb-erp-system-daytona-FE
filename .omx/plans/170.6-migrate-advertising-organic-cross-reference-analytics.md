---
storyId: "170.6"
storyTitle: "Migrate Advertising–Organic Cross-Reference Analytics"
epic: "170-FE"
...
branch: cdx/epic-170-story-6-cross-reference-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn
productionScope: forbidden
---

# OMX Plan — Story 170.6: Migrate Advertising–Organic Cross-Reference Analytics

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1706-migrate-advertising-organic-cross-reference-analytics` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As a marketing/finance user, I want `/analytics/cross-reference` to connect advertising and organic overlap, cannibalization, spend/position relationships, insights, charts, and product evidence, so that I can validate cross-channel effects.
- **Resolved owned surface:** `/analytics/cross-reference`; its `page.tsx`, complete `components/**` and `utils/**` trees (state surfaces, summaries/insights, overlap/cross-reference tables, cannibalization, scatter and position/spend charts), and colocated tests.
...
- **Exact branch:** `cdx/epic-170-story-6-cross-reference-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-170-story-6-cross-reference-shadcn" "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/cross-reference/**`.
- **Resolved owned files/tree:** `/analytics/cross-reference`; its `page.tsx`, complete `components/**` and `utils/**` trees (state surfaces, summaries/insights, overlap/cross-reference tables, cannibalization, scatter and position/spend charts), and colocated tests.
...
- **Canonical prohibition:** FS and source advertising/search route trees.
...
1. From `/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/cross-reference"
   ```
...
4. Capture deterministic baseline evidence for: SC plus one-source partial, valid no-overlap, indeterminate correlation, selected chart point, and section-level error states.
...
5. **Apply the route-specific data contract:** RTC; overlap/cross-reference tables prioritize product/SKU with organic/ad metrics and relation status. Scatter and position/spend charts name axes/units/series, expose selected point and data alternatives, and reconcile with table values.
6. **Apply accessibility and responsive behavior:** AX; correlation/cannibalization is described in text, chart point interaction has keyboard/touch equivalent, and partial-source warnings are non-color. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/cross-reference` and `STORY_ID=170.6`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn"
npx vitest run "src/app/(dashboard)/analytics/cross-reference"
npm run test:e2e -- --grep "Story 170.6-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" diff --check
```
...
- **State matrix:** SC plus one-source partial, valid no-overlap, indeterminate correlation, selected chart point, and section-level error states.
- **Route-specific table/chart contract:** RTC; overlap/cross-reference tables prioritize product/SKU with organic/ad metrics and relation status. Scatter and position/spend charts name axes/units/series, expose selected point and data alternatives, and reconcile with table values.
...
- **Required Story evidence:** VE plus one-source partial, no-overlap, selected scatter point, and chart/table reconciliation evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" commit \
  -m "feat(analytics): migrate Story 170.6 route to shadcn" \
  -m "Story 170.6-FE — Migrate Advertising–Organic Cross-Reference Analytics. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn" push -u origin "cdx/epic-170-story-6-cross-reference-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-170-story-6-cross-reference-shadcn" \
  --title "[Story 170.6-FE] Migrate Advertising–Organic Cross-Reference Analytics" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-170-story-6-cross-reference-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-170-story-6-cross-reference-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-170-story-6-cross-reference-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-170-story-6-cross-reference-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-170-story-6-cross-reference-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus one-source partial, valid no-overlap, indeterminate correlation, selected chart point, and section-level error states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; overlap/cross-reference tables prioritize product/SKU with organic/ad metrics and relation status. Scatter and position/spend charts name axes/units/series, expose selected point and data alternatives, and reconcile with table values. |
| Keyboard/focus/contrast regression | Apply AX; correlation/cannibalization is described in text, chart point interaction has keyboard/touch equivalent, and partial-source warnings are non-color. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-170-story-6-cross-reference-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-170-story-6-cross-reference-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-170-6-cross-reference-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
