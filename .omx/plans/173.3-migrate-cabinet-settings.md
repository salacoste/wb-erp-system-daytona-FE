---
storyId: "173.3"
storyTitle: "Migrate Cabinet Settings"
epic: "173-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-173-3-settings-cabinet
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.3 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 173.3 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** cabinet information, token/config status, permitted edits and confirmations.
- **Required outcome:** Migrate Cabinet Settings. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 173.1 plus foundation/AppShell.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_3["Story 173.3"]
  P2["Story 167.1"] --> S173_3["Story 173.3"]
  P3["Story 173.1"] --> S173_3["Story 173.3"]
```
...
- [ ] Confirm Story ID/title parity: `173.3` / `Migrate Cabinet Settings` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.3 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.3. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-3-settings-cabinet`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-3-settings-cabinet`
...
test ! -e "/private/tmp/wb-repricer-fe-173-3-settings-cabinet"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-3-settings-cabinet" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-3-settings-cabinet" "/private/tmp/wb-repricer-fe-173-3-settings-cabinet" main
git -C "/private/tmp/wb-repricer-fe-173-3-settings-cabinet" status --short --branch
```
...
- Owned Surface and cabinet-settings fixtures/tests.
- Canonical owned surface: route directory and cabinet-settings exclusive components/tests.
...
- foundation, settings shell, cabinet store/API/hooks/types/auth semantics.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.3.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, unavailable/partial info, valid form, validation error, save pending/success/failure, restricted action.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and cabinet-settings exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: cabinet information, token/config status, permitted edits and confirmations.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, unavailable/partial info, valid form, validation error, save pending/success/failure, restricted action.
6. Apply the Story contract for density and responsive behavior: focused readable form/info cards; no table/chart unless existing.
7. Apply the Story accessibility contract: explicit cabinet context, labels/errors, confirmation focus, announced save outcome.
...
- `npm test -- --run 'src/app/(dashboard)/settings/cabinet' src/components/custom/settings/__tests__/CabinetInfoCard.test.tsx`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-3-settings-cabinet" diff --check
```
...
- `cabinet form regression plus responsive/theme screenshots.`;
- canonical local-validation intent: targeted cabinet tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.3/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** focused readable form/info cards; no table/chart unless existing.
- **Story accessibility contract:** explicit cabinet context, labels/errors, confirmation focus, announced save outcome.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.3.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-3-settings-cabinet diff --check
git -C /private/tmp/wb-repricer-fe-173-3-settings-cabinet status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-3-settings-cabinet" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-3-settings-cabinet" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-3-settings-cabinet" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-3-settings-cabinet diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-3-settings-cabinet commit \
  -m "feat(settings): migrate cabinet settings" \
  -m "Story 173.3. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-3-settings-cabinet push -u origin cdx/epic-173-story-3-settings-cabinet
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-3-settings-cabinet" \
  --title "Story 173.3: Migrate Cabinet Settings" \
  --body "Implements BMAD Story 173.3 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-3-settings-cabinet"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-3-settings-cabinet"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-3-settings-cabinet"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-3-settings-cabinet"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-3-settings-cabinet"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-3-settings-cabinet"
...
- `/private/tmp/wb-repricer-fe-173-3-settings-cabinet` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.3 worktree;
- local and remote branch queries return no `cdx/epic-173-story-3-settings-cabinet`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: behavior proof and mandatory cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of cabinet information, token/config status, permitted edits and confirmations. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 173.1 plus foundation/AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: focused readable form/info cards; no table/chart unless existing. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: explicit cabinet context, labels/errors, confirmation focus, announced save outcome. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, unavailable/partial info, valid form, validation error, save pending/success/failure, restricted action. | Test each applicable fixture and block merge on an unhandled state. |
