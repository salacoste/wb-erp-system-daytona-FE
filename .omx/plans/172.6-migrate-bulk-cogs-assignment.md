---
storyId: "172.6"
storyTitle: "Migrate Bulk COGS Assignment"
epic: "172-FE"
...
branch: cdx/epic-172-story-6-cogs-bulk
temporaryWorktree: /private/tmp/wb-repricer-fe-172-6-cogs-bulk
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.6 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.6 scope below takes precedence.
...
- **Required outcome:** Migrate Bulk COGS Assignment. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 172.5 plus foundation/AppShell.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_6["Story 172.6"]
  P2["Story 167.1"] --> S172_6["Story 172.6"]
  P3["Story 172.5"] --> S172_6["Story 172.6"]
```
...
- [ ] Verify the merge SHA for **Story 172.5** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `172.6` / `Migrate Bulk COGS Assignment` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.6 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.6. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-6-cogs-bulk`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-6-cogs-bulk`
...
test ! -e "/private/tmp/wb-repricer-fe-172-6-cogs-bulk"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-6-cogs-bulk" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-6-cogs-bulk" "/private/tmp/wb-repricer-fe-172-6-cogs-bulk" main
git -C "/private/tmp/wb-repricer-fe-172-6-cogs-bulk" status --short --branch
```
...
- Owned Surface, bulk COGS fixtures/tests.
- Canonical owned surface: route directory; `src/components/custom/bulk-cogs/**`, `BulkCogsForm.tsx`, and exclusive tests.
...
- tokens/primitives/AppShell, API/hooks/types, single-COGS owner components except through approved interfaces.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.6.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **no selection, validation errors, preview, submitting, all success, partial success, all failed, stale/conflicting row where supported.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory; `src/components/custom/bulk-cogs/**`, `BulkCogsForm.tsx`, and exclusive tests.
2. Deliver the user value and interaction surface exactly as recorded: `/cogs/bulk`; select, enter, validate, preview, submit, inspect, and retry bulk costs.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: no selection, validation errors, preview, submitting, all success, partial success, all failed, stale/conflicting row where supported.
6. Apply the Story contract for density and responsive behavior: persistent selected count; product/value/error/actions retained; focused mobile workflow; no chart.
7. Apply the Story accessibility contract: labeled selection, focusable multi-row error summary, row errors, confirmation focus, polite progress, explicit partial-result headings.
...
- `npm test -- --run 'src/app/(dashboard)/cogs/bulk' src/components/custom/bulk-cogs`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-6-cogs-bulk" diff --check
```
...
- `selection/preview/partial-retry tests and multi-state responsive/theme screenshots.`;
- canonical local-validation intent: targeted bulk tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.6/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** persistent selected count; product/value/error/actions retained; focused mobile workflow; no chart.
- **Story accessibility contract:** labeled selection, focusable multi-row error summary, row errors, confirmation focus, polite progress, explicit partial-result headings.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.6.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-6-cogs-bulk diff --check
git -C /private/tmp/wb-repricer-fe-172-6-cogs-bulk status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-6-cogs-bulk" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-6-cogs-bulk" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-6-cogs-bulk" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-6-cogs-bulk diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-6-cogs-bulk commit \
  -m "feat(cogs): migrate bulk cogs assignment" \
  -m "Story 172.6. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-6-cogs-bulk push -u origin cdx/epic-172-story-6-cogs-bulk
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-6-cogs-bulk" \
  --title "Story 172.6: Migrate Bulk COGS Assignment" \
  --body "Implements BMAD Story 172.6 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-6-cogs-bulk"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-6-cogs-bulk"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-6-cogs-bulk"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-6-cogs-bulk"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-6-cogs-bulk"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-6-cogs-bulk"
...
- `/private/tmp/wb-repricer-fe-172-6-cogs-bulk` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.6 worktree;
- local and remote branch queries return no `cdx/epic-172-story-6-cogs-bulk`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: branch/worktree removal and partial-result verification recorded.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of `/cogs/bulk`; select, enter, validate, preview, submit, inspect, and retry bulk costs. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 172.5 plus foundation/AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: persistent selected count; product/value/error/actions retained; focused mobile workflow; no chart. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: labeled selection, focusable multi-row error summary, row errors, confirmation focus, polite progress, explicit partial-result headings. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: no selection, validation errors, preview, submitting, all success, partial success, all failed, stale/conflicting row where supported. | Test each applicable fixture and block merge on an unhandled state. |
