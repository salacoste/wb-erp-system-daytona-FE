---
storyId: "169.8"
storyTitle: "Migrate Funnel Analytics"
epic: "169-FE"
...
branch: cdx/epic-169-story-8-funnel-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-8-funnel-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.8: Migrate Funnel Analytics

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1698-migrate-funnel-analytics` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an operations/analytics user, I want `/analytics/funnel` to compare funnel stages, anomalies, products, and periods consistently, so that I can locate conversion losses and supporting product evidence.
- **Resolved owned surface:** `/analytics/funnel`; its `page.tsx`, full `components/**` tree (summary, product filter, funnel and overlay charts/tooltips/config, anomaly/delta indicators, alerts/sync banner, table/cells/columns/rows, export hook/utilities), and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-8-funnel-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-8-funnel-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-8-funnel-shadcn" "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/funnel/**`.
- **Resolved owned files/tree:** `/analytics/funnel`; its `page.tsx`, full `components/**` tree (summary, product filter, funnel and overlay charts/tooltips/config, anomaly/delta indicators, alerts/sync banner, table/cells/columns/rows, export hook/utilities), and colocated tests.
...
1. From `/private/tmp/wb-repricer-fe-169-8-funnel-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/funnel"
   ```
...
4. Capture deterministic baseline evidence for: SC plus sync-gap, overlay unavailable, missing comparison, stage-level partial, and anomaly states.
...
5. **Apply the route-specific data contract:** RTC; product table primary column is product/SKU with stage metrics aligned; funnel and overlay charts name stage/series/period, use non-color distinctions, expose data alternatives, and preserve selected product/filter context.
6. **Apply accessibility and responsive behavior:** AX; anomaly/delta and sync meaning use text/icons; chart selection, filter, sort, and export are keyboard complete. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/funnel` and `STORY_ID=169.8`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn"
npx vitest run "src/app/(dashboard)/analytics/funnel"
npm run test:e2e -- --grep "Story 169.8-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" diff --check
```
...
- **State matrix:** SC plus sync-gap, overlay unavailable, missing comparison, stage-level partial, and anomaly states.
- **Route-specific table/chart contract:** RTC; product table primary column is product/SKU with stage metrics aligned; funnel and overlay charts name stage/series/period, use non-color distinctions, expose data alternatives, and preserve selected product/filter context.
...
- **Required Story evidence:** VE plus sync-gap, partial-stage, overlay comparison, and chart/table equivalence evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" commit \
  -m "feat(analytics): migrate Story 169.8 route to shadcn" \
  -m "Story 169.8-FE — Migrate Funnel Analytics. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn" push -u origin "cdx/epic-169-story-8-funnel-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-8-funnel-shadcn" \
  --title "[Story 169.8-FE] Migrate Funnel Analytics" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-8-funnel-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-8-funnel-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-8-funnel-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-8-funnel-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-8-funnel-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-8-funnel-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-8-funnel-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus sync-gap, overlay unavailable, missing comparison, stage-level partial, and anomaly states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; product table primary column is product/SKU with stage metrics aligned; funnel and overlay charts name stage/series/period, use non-color distinctions, expose data alternatives, and preserve selected product/filter context. |
| Keyboard/focus/contrast regression | Apply AX; anomaly/delta and sync meaning use text/icons; chart selection, filter, sort, and export are keyboard complete. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-8-funnel-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-8-funnel-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-8-funnel-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-8-funnel-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
