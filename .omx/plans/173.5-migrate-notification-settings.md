---
storyId: "173.5"
storyTitle: "Migrate Notification Settings"
epic: "173-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-173-5-settings-notifications
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.5 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 173.5 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** notification panels, Telegram binding, quiet hours, tabs/settings, status and writeback.
- **Required outcome:** Migrate Notification Settings. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 173.1, semantic external-brand token from 166-FE, AppShell.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_5["Story 173.5"]
  P2["Story 167.1"] --> S173_5["Story 173.5"]
  P3["Story 173.1"] --> S173_5["Story 173.5"]
```
...
- [ ] Verify the merge SHA for **Story 173.1** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `173.5` / `Migrate Notification Settings` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.5 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.5. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-5-settings-notifications`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-5-settings-notifications`
...
test ! -e "/private/tmp/wb-repricer-fe-173-5-settings-notifications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-5-settings-notifications" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-5-settings-notifications" "/private/tmp/wb-repricer-fe-173-5-settings-notifications" main
git -C "/private/tmp/wb-repricer-fe-173-5-settings-notifications" status --short --branch
```
...
- Owned Surface and notification visual/E2E fixtures.
- Canonical owned surface: route directory and notification-exclusive components/tests including binding modal.
...
- tokens except consumed variables, primitives, settings shell, notification API/hooks/types.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.5.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, bound/unbound, connecting, verification pending, save success/failure, unavailable Telegram, quiet-hours validation.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and notification-exclusive components/tests including binding modal.
2. Deliver the user value and interaction surface exactly as recorded: notification panels, Telegram binding, quiet hours, tabs/settings, status and writeback.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, bound/unbound, connecting, verification pending, save success/failure, unavailable Telegram, quiet-hours validation.
6. Apply the Story contract for density and responsive behavior: settings panels and modal reflow; no table/chart requirement.
7. Apply the Story accessibility contract: switch labels/state, Telegram status text/icon, time-field errors, modal focus lifecycle, both themes.
...
- `npm test -- --run 'src/app/(dashboard)/settings/notifications' src/components/notifications`
- `npm run test:e2e -- tests/e2e/telegram-notifications.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-5-settings-notifications" diff --check
```
...
- `binding/quiet-hours regression and light/dark responsive screenshots.`;
- canonical local-validation intent: targeted notification tests/E2E plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.5/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** settings panels and modal reflow; no table/chart requirement.
- **Story accessibility contract:** switch labels/state, Telegram status text/icon, time-field errors, modal focus lifecycle, both themes.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.5.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-5-settings-notifications diff --check
git -C /private/tmp/wb-repricer-fe-173-5-settings-notifications status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-5-settings-notifications" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-5-settings-notifications" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-5-settings-notifications" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-5-settings-notifications diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-5-settings-notifications commit \
  -m "feat(settings): migrate notification settings" \
  -m "Story 173.5. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-5-settings-notifications push -u origin cdx/epic-173-story-5-settings-notifications
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-5-settings-notifications" \
  --title "Story 173.5: Migrate Notification Settings" \
  --body "Implements BMAD Story 173.5 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-5-settings-notifications"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-5-settings-notifications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-5-settings-notifications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-5-settings-notifications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-5-settings-notifications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-5-settings-notifications"
...
- `/private/tmp/wb-repricer-fe-173-5-settings-notifications` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.5 worktree;
- local and remote branch queries return no `cdx/epic-173-story-5-settings-notifications`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: external-brand/theme proof and cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of notification panels, Telegram binding, quiet hours, tabs/settings, status and writeback. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 173.1, semantic external-brand token from 166-FE, AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: settings panels and modal reflow; no table/chart requirement. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: switch labels/state, Telegram status text/icon, time-field errors, modal focus lifecycle, both themes. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, bound/unbound, connecting, verification pending, save success/failure, unavailable Telegram, quiet-hours validation. | Test each applicable fixture and block merge on an unhandled state. |
