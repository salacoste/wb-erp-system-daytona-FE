---
storyId: "173.12"
storyTitle: "Migrate Supplies List"
epic: "173-FE"
...
branch: cdx/epic-173-story-12-supplies
temporaryWorktree: /private/tmp/wb-repricer-fe-173-12-supplies
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.12 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_12["Story 173.12"]
  P2["Story 167.1"] --> S173_12["Story 173.12"]
```
...
- [ ] Confirm Story ID/title parity: `173.12` / `Migrate Supplies List` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.12 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.12. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-12-supplies`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-12-supplies`
...
test ! -e "/private/tmp/wb-repricer-fe-173-12-supplies"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-12-supplies" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-12-supplies" "/private/tmp/wb-repricer-fe-173-12-supplies" main
git -C "/private/tmp/wb-repricer-fe-173-12-supplies" status --short --branch
```
...
- owned supply list/shared files and tests.
- Canonical owned surface: route entry and list/shared files in `src/components/custom/supplies/**` explicitly inventoried by this Story, plus tests.
...
- foundation/AppShell, supply API/hooks/types, detail-exclusive files.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.12.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty, lifecycle statuses, stale/partial, create pending/success/failure, error.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route entry and list/shared files in `src/components/custom/supplies/**` explicitly inventoried by this Story, plus tests.
2. Deliver the user value and interaction surface exactly as recorded: supply list, filters/status, create/open actions, empty/loading/error.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty, lifecycle statuses, stale/partial, create pending/success/failure, error.
6. Apply the Story contract for density and responsive behavior: supply identifier/status/date/action retained; explicit narrow queue strategy.
7. Apply the Story accessibility contract: named actions, status label/icon, create overlay focus/errors, keyboard navigation.
...
- `npm test -- --run 'src/app/(dashboard)/supplies/__tests__' src/components/custom/supplies/__tests__/SuppliesFilters.test.tsx src/components/custom/supplies/__tests__/SuppliesTable.test.tsx`
- `npm run test:e2e -- e2e/supplies/supplies-list.spec.ts e2e/supplies/supplies-a11y.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-12-supplies" diff --check
```
...
- `list/status/create tests and responsive/theme screenshots.`;
- canonical local-validation intent: targeted supplies tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.12/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** supply identifier/status/date/action retained; explicit narrow queue strategy.
- **Story accessibility contract:** named actions, status label/icon, create overlay focus/errors, keyboard navigation.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.12.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-12-supplies diff --check
git -C /private/tmp/wb-repricer-fe-173-12-supplies status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-12-supplies" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-12-supplies" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-12-supplies" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-12-supplies diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-12-supplies commit \
  -m "feat(supplies): migrate supplies list" \
  -m "Story 173.12. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-12-supplies push -u origin cdx/epic-173-story-12-supplies
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-12-supplies" \
  --title "Story 173.12: Migrate Supplies List" \
  --body "Implements BMAD Story 173.12 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-12-supplies"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-12-supplies"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-12-supplies"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-12-supplies"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-12-supplies"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-12-supplies"
...
- `/private/tmp/wb-repricer-fe-173-12-supplies` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.12 worktree;
- local and remote branch queries return no `cdx/epic-173-story-12-supplies`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: supply shared-owner manifest and cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of supply list, filters/status, create/open actions, empty/loading/error. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because foundation/AppShell; owner for supply-shared list/status compositions used by 173.13. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: supply identifier/status/date/action retained; explicit narrow queue strategy. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: named actions, status label/icon, create overlay focus/errors, keyboard navigation. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty, lifecycle statuses, stale/partial, create pending/success/failure, error. | Test each applicable fixture and block merge on an unhandled state. |
