---
storyId: "172.2"
storyTitle: "Migrate the Canned Automation Rules Gallery"
epic: "172-FE"
...
branch: cdx/epic-172-story-2-canned-rules
temporaryWorktree: /private/tmp/wb-repricer-fe-172-2-canned-rules
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.2 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.2 scope below takes precedence.
...
- **Required outcome:** Migrate the Canned Automation Rules Gallery. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Epics 166-FE and Story 167.1.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_2["Story 172.2"]
  P2["Story 167.1"] --> S172_2["Story 172.2"]
```
...
- [ ] Confirm Story ID/title parity: `172.2` / `Migrate the Canned Automation Rules Gallery` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.2 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.2. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-2-canned-rules`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-2-canned-rules`
...
test ! -e "/private/tmp/wb-repricer-fe-172-2-canned-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-2-canned-rules" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-2-canned-rules" "/private/tmp/wb-repricer-fe-172-2-canned-rules" main
git -C "/private/tmp/wb-repricer-fe-172-2-canned-rules" status --short --branch
```
...
- Owned Surface and route-specific fixtures/tests.
- Canonical owned surface: `src/app/(dashboard)/automation/canned-rules/**`, `src/components/custom/automation/CannedRulesGallery.tsx`, exclusive automation gallery components/tests.
...
- tokens, primitives, AppShell, shared navigation, automation API/hooks/types, installed-rule components.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.2.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, no rules, restricted/unavailable rule, install pending, success, error.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/automation/canned-rules/**`, `src/components/custom/automation/CannedRulesGallery.tsx`, exclusive automation gallery components/tests.
2. Deliver the user value and interaction surface exactly as recorded: `/automation/canned-rules`; discover, inspect, and initiate installation of supported canned rules.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, no rules, restricted/unavailable rule, install pending, success, error.
6. Apply the Story contract for density and responsive behavior: responsive gallery with preserved readable rule summaries and 44×44 touch actions; no table/chart requirement.
7. Apply the Story accessibility contract: semantic list/cards, explicit install action labels, keyboard flow, warnings not color-only, Dialog focus lifecycle.
...
- `npm test -- --run 'src/app/(dashboard)/automation/canned-rules' src/components/custom/automation/__tests__/CannedRulesGallery.test.tsx`
- `npm run test:e2e -- e2e/automation/canned-rules.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-2-canned-rules" diff --check
```
...
- `rule-gallery interaction tests and responsive/theme screenshots.`;
- canonical local-validation intent: targeted automation tests plus Universal Local Validation Contract.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.2/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** responsive gallery with preserved readable rule summaries and 44×44 touch actions; no table/chart requirement.
- **Story accessibility contract:** semantic list/cards, explicit install action labels, keyboard flow, warnings not color-only, Dialog focus lifecycle.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.2.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-2-canned-rules diff --check
git -C /private/tmp/wb-repricer-fe-172-2-canned-rules status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-2-canned-rules" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-2-canned-rules" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-2-canned-rules" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-2-canned-rules diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-2-canned-rules commit \
  -m "feat(automation): migrate the canned automation rules gallery" \
  -m "Story 172.2. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-2-canned-rules push -u origin cdx/epic-172-story-2-canned-rules
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-2-canned-rules" \
  --title "Story 172.2: Migrate the Canned Automation Rules Gallery" \
  --body "Implements BMAD Story 172.2 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-2-canned-rules"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-2-canned-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-2-canned-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-2-canned-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-2-canned-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-2-canned-rules"
...
- `/private/tmp/wb-repricer-fe-172-2-canned-rules` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.2 worktree;
- local and remote branch queries return no `cdx/epic-172-story-2-canned-rules`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: merged SHA, branch deletion, and absent worktree entry.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of `/automation/canned-rules`; discover, inspect, and initiate installation of supported canned rules. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Epics 166-FE and Story 167.1. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: responsive gallery with preserved readable rule summaries and 44×44 touch actions; no table/chart requirement. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: semantic list/cards, explicit install action labels, keyboard flow, warnings not color-only, Dialog focus lifecycle. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, no rules, restricted/unavailable rule, install pending, success, error. | Test each applicable fixture and block merge on an unhandled state. |
