---
storyId: "172.14"
storyTitle: "Migrate the Orders Overview"
epic: "172-FE"
...
branch: cdx/epic-172-story-14-orders
temporaryWorktree: /private/tmp/wb-repricer-fe-172-14-orders
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.14 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.14 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** order filters, aggregate cards, orders table, statuses, pagination, and actions.
- **Required outcome:** Migrate the Orders Overview. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** foundation/AppShell; this Story owns order-shared presentation used by 172.15–172.16 where documented.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_14["Story 172.14"]
  P2["Story 167.1"] --> S172_14["Story 172.14"]
```
...
- [ ] Confirm Story ID/title parity: `172.14` / `Migrate the Orders Overview` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.14 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.14. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-14-orders`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-14-orders`
...
test ! -e "/private/tmp/wb-repricer-fe-172-14-orders"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-14-orders" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-14-orders" "/private/tmp/wb-repricer-fe-172-14-orders" main
git -C "/private/tmp/wb-repricer-fe-172-14-orders" status --short --branch
```
...
- Owned Surface and shared order UI explicitly inventoried in the Story worktree.
- Canonical owned surface: `src/app/(dashboard)/orders/page.tsx`, route-local overview components, order-shared components explicitly assigned to this Story, tests.
...
- tokens/primitives/AppShell, order API/hooks/types, FBO/integrity exclusive components.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.14.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty, stale/partial, error, action pending/success/failure.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/orders/page.tsx`, route-local overview components, order-shared components explicitly assigned to this Story, tests.
2. Deliver the user value and interaction surface exactly as recorded: order filters, aggregate cards, orders table, statuses, pagination, and actions.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty, stale/partial, error, action pending/success/failure.
6. Apply the Story contract for density and responsive behavior: order identifier/status/amount/action primary columns; filters/pagination preserved; no unintended mobile clipping.
7. Apply the Story accessibility contract: accessible table/sort/actions, status text/icons, full values and dates, keyboard filters.
...
- `npm test -- --run 'src/app/(dashboard)/orders/__tests__' src/components/custom/orders`
- `npm run test:e2e -- e2e/orders.spec.ts e2e/orders-accessibility.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-14-orders" diff --check
```
...
- `orders overview tests and responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted orders tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.14/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** order identifier/status/amount/action primary columns; filters/pagination preserved; no unintended mobile clipping.
- **Story accessibility contract:** accessible table/sort/actions, status text/icons, full values and dates, keyboard filters.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.14.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-14-orders diff --check
git -C /private/tmp/wb-repricer-fe-172-14-orders status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-14-orders" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-14-orders" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-14-orders" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-14-orders diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-14-orders commit \
  -m "feat(orders): migrate the orders overview" \
  -m "Story 172.14. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-14-orders push -u origin cdx/epic-172-story-14-orders
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-14-orders" \
  --title "Story 172.14: Migrate the Orders Overview" \
  --body "Implements BMAD Story 172.14 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-14-orders"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-14-orders"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-14-orders"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-14-orders"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-14-orders"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-14-orders"
...
- `/private/tmp/wb-repricer-fe-172-14-orders` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.14 worktree;
- local and remote branch queries return no `cdx/epic-172-story-14-orders`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: shared-owner inventory plus branch/worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of order filters, aggregate cards, orders table, statuses, pagination, and actions. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because foundation/AppShell; this Story owns order-shared presentation used by 172.15–172.16 where documented. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: order identifier/status/amount/action primary columns; filters/pagination preserved; no unintended mobile clipping. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: accessible table/sort/actions, status text/icons, full values and dates, keyboard filters. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty, stale/partial, error, action pending/success/failure. | Test each applicable fixture and block merge on an unhandled state. |
