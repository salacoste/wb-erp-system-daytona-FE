---
storyId: "172.9"
storyTitle: "Migrate Communications Workspace"
epic: "172-FE"
...
branch: cdx/epic-172-story-9-communications
temporaryWorktree: /private/tmp/wb-repricer-fe-172-9-communications
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.9 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.9 scope below takes precedence.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_9["Story 172.9"]
  P2["Story 167.1"] --> S172_9["Story 172.9"]
```
...
- [ ] Confirm Story ID/title parity: `172.9` / `Migrate Communications Workspace` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.9 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.9. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-9-communications`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-9-communications`
...
test ! -e "/private/tmp/wb-repricer-fe-172-9-communications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-9-communications" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-9-communications" "/private/tmp/wb-repricer-fe-172-9-communications" main
git -C "/private/tmp/wb-repricer-fe-172-9-communications" status --short --branch
```
...
- Owned Surface plus communications E2E/visual fixtures.
- Canonical owned surface: route directory and communications-exclusive components/tests under the current domain paths.
...
- tokens/primitives/AppShell, communication hooks/API/types/writeback semantics.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.9.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading per section, empty, unread, draft, sending, success, partial/retryable writeback, session/network uncertainty, error.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and communications-exclusive components/tests under the current domain paths.
2. Deliver the user value and interaction surface exactly as recorded: feedback, questions, chats, claims, pinned reviews, reply/composer workflows and unread states.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading per section, empty, unread, draft, sending, success, partial/retryable writeback, session/network uncertainty, error.
6. Apply the Story contract for density and responsive behavior: tabs and queue/detail behavior preserve selection; mobile uses explicit list/detail transition; no chart requirement.
7. Apply the Story accessibility contract: tab semantics, named conversation actions, composer labels, retained focus/draft, live-region moderation, Dialog/Sheet lifecycle.
...
- `npm test -- --run 'src/app/(dashboard)/communications' src/lib/__tests__/communications-writeback-utils.test.ts`
- `npm run test:e2e -- e2e/communications.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-9-communications" diff --check
```
...
- `existing writeback/retry regression tests plus responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted communications tests and integration-sensitive E2E plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.9/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** tabs and queue/detail behavior preserve selection; mobile uses explicit list/detail transition; no chart requirement.
- **Story accessibility contract:** tab semantics, named conversation actions, composer labels, retained focus/draft, live-region moderation, Dialog/Sheet lifecycle.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.9.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-9-communications diff --check
git -C /private/tmp/wb-repricer-fe-172-9-communications status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-9-communications" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-9-communications" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-9-communications" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-9-communications diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-9-communications commit \
  -m "feat(communications): migrate communications workspace" \
  -m "Story 172.9. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-9-communications push -u origin cdx/epic-172-story-9-communications
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-9-communications" \
  --title "Story 172.9: Migrate Communications Workspace" \
  --body "Implements BMAD Story 172.9 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-9-communications"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-9-communications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-9-communications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-9-communications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-9-communications"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-9-communications"
...
- `/private/tmp/wb-repricer-fe-172-9-communications` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.9 worktree;
- local and remote branch queries return no `cdx/epic-172-story-9-communications`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: writeback regression and mandatory repository cleanup evidence.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of feedback, questions, chats, claims, pinned reviews, reply/composer workflows and unread states. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: tabs and queue/detail behavior preserve selection; mobile uses explicit list/detail transition; no chart requirement. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: tab semantics, named conversation actions, composer labels, retained focus/draft, live-region moderation, Dialog/Sheet lifecycle. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading per section, empty, unread, draft, sending, success, partial/retryable writeback, session/network uncertainty, error. | Test each applicable fixture and block merge on an unhandled state. |
