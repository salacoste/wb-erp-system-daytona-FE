---
storyId: "171.8"
storyTitle: "Migrate Evaluation SKU Accuracy Detail"
epic: "171-FE"
...
branch: cdx/epic-171-story-8-model-sku-accuracy-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.8: Migrate Evaluation SKU Accuracy Detail

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1718-migrate-evaluation-sku-accuracy-detail` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As a model analyst, I want `/analytics/models/[id]/evaluations/sku-accuracy` to show SKU accuracy overview and row-level evidence for the selected evaluation, so that I can identify the SKUs driving model error and return without losing evaluation context.
- **Resolved owned surface:** `/analytics/models/[id]/evaluations/sku-accuracy`; dynamic route `src/app/(dashboard)/analytics/models/[id]/evaluations/sku-accuracy/**`; its `page.tsx`, `SkuAccuracyOverview.tsx`, `SkuAccuracyDetail.tsx`, `SkuAccuracyTable.tsx`, helpers, and colocated tests.
...
- Story 171.7 is merged into current `main`; record its merge SHA before creating this Story's branch.
...
flowchart TD
  ...
  S167_1 --> THIS["Story 171.8"]
  S168_1 --> THIS
  S171_7["Story 171.7"] --> THIS
```
...
- **Exact branch:** `cdx/epic-171-story-8-model-sku-accuracy-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-8-model-sku-accuracy-shadcn" "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" main
...
- **Canonical allowance:** Only the nested SKU-accuracy route tree.
- **Resolved owned files/tree:** `/analytics/models/[id]/evaluations/sku-accuracy`; dynamic route `src/app/(dashboard)/analytics/models/[id]/evaluations/sku-accuracy/**`; its `page.tsx`, `SkuAccuracyOverview.tsx`, `SkuAccuracyDetail.tsx`, `SkuAccuracyTable.tsx`, helpers, and colocated tests.
...
- **Canonical prohibition:** FS and parent evaluations/model performance/registry trees.
...
1. From `/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/models/[id]/evaluations/sku-accuracy"
   ```
...
4. Capture deterministic baseline evidence for: SC plus missing/invalid evaluation parameter, no observations, undefined versus zero metric, and row-level partial states.
...
5. **Apply the route-specific data contract:** RTC; SKU accuracy table caption names model/evaluation, primary column is SKU/product, actual/forecast/error/accuracy/sample columns align with explicit units, active sort and pagination remain visible. No chart.
6. **Apply accessibility and responsive behavior:** AX; back link is semantic and context-rich, metric direction is sign/text based, table actions name SKU, and overview does not replace the single `h1` hierarchy. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/models/[id]/evaluations/sku-accuracy` and `STORY_ID=171.8`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn"
npx vitest run "src/app/(dashboard)/analytics/models/[id]/evaluations/sku-accuracy"
npm run test:e2e -- --grep "Story 171.8-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" diff --check
```
...
- **State matrix:** SC plus missing/invalid evaluation parameter, no observations, undefined versus zero metric, and row-level partial states.
- **Route-specific table/chart contract:** RTC; SKU accuracy table caption names model/evaluation, primary column is SKU/product, actual/forecast/error/accuracy/sample columns align with explicit units, active sort and pagination remain visible. No chart.
...
- **Required Story evidence:** VE plus invalid context, no observations, undefined versus zero, partial row, and back-context evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" commit \
  -m "feat(analytics): migrate Story 171.8 route to shadcn" \
  -m "Story 171.8-FE — Migrate Evaluation SKU Accuracy Detail. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn" push -u origin "cdx/epic-171-story-8-model-sku-accuracy-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-8-model-sku-accuracy-shadcn" \
  --title "[Story 171.8-FE] Migrate Evaluation SKU Accuracy Detail" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-8-model-sku-accuracy-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-8-model-sku-accuracy-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-8-model-sku-accuracy-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-8-model-sku-accuracy-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-8-model-sku-accuracy-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus missing/invalid evaluation parameter, no observations, undefined versus zero metric, and row-level partial states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; SKU accuracy table caption names model/evaluation, primary column is SKU/product, actual/forecast/error/accuracy/sample columns align with explicit units, active sort and pagination remain visible. No chart. |
| Keyboard/focus/contrast regression | Apply AX; back link is semantic and context-rich, metric direction is sign/text based, table actions name SKU, and overview does not replace the single `h1` hierarchy. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-8-model-sku-accuracy-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-8-model-sku-accuracy-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-8-model-sku-accuracy-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
