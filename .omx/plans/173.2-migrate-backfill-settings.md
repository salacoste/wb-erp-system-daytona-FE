---
storyId: "173.2"
storyTitle: "Migrate Backfill Settings"
epic: "173-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-173-2-settings-backfill
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.2 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_2["Story 173.2"]
  P2["Story 167.1"] --> S173_2["Story 173.2"]
  P3["Story 173.1"] --> S173_2["Story 173.2"]
```
...
- [ ] Confirm Story ID/title parity: `173.2` / `Migrate Backfill Settings` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.2 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.2. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-2-settings-backfill`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-2-settings-backfill`
...
test ! -e "/private/tmp/wb-repricer-fe-173-2-settings-backfill"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-2-settings-backfill" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-2-settings-backfill" "/private/tmp/wb-repricer-fe-173-2-settings-backfill" main
git -C "/private/tmp/wb-repricer-fe-173-2-settings-backfill" status --short --branch
```
...
- Owned Surface and backfill fixtures/tests.
- Canonical owned surface: route directory and backfill-exclusive components/tests.
...
- foundation, settings shell, backfill API/hooks/types/job semantics.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.2.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, queued/running, safe-to-leave, success, partial/failure, retry.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and backfill-exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: backfill table, dialog, status, progress, and recovery.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, queued/running, safe-to-leave, success, partial/failure, retry.
6. Apply the Story contract for density and responsive behavior: backfill item/status/time/action primary columns; explicit mobile detail.
7. Apply the Story accessibility contract: named actions, progress announcement, Dialog focus, durable result summary.
...
- `npm test -- --run 'src/app/(dashboard)/settings/backfill' src/lib/__tests__/backfill-utils.test.ts`
- `npm run test:e2e -- e2e/backfill-page.spec.ts e2e/settings/backfill-a11y.spec.ts e2e/settings/backfill-admin.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-2-settings-backfill" diff --check
```
...
- `job/dialog regression and responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted backfill tests/E2E plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.2/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** backfill item/status/time/action primary columns; explicit mobile detail.
- **Story accessibility contract:** named actions, progress announcement, Dialog focus, durable result summary.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.2.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-2-settings-backfill diff --check
git -C /private/tmp/wb-repricer-fe-173-2-settings-backfill status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-2-settings-backfill" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-2-settings-backfill" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-2-settings-backfill" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-2-settings-backfill diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-2-settings-backfill commit \
  -m "feat(settings): migrate backfill settings" \
  -m "Story 173.2. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-2-settings-backfill push -u origin cdx/epic-173-story-2-settings-backfill
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-2-settings-backfill" \
  --title "Story 173.2: Migrate Backfill Settings" \
  --body "Implements BMAD Story 173.2 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-2-settings-backfill"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-2-settings-backfill"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-2-settings-backfill"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-2-settings-backfill"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-2-settings-backfill"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-2-settings-backfill"
...
- `/private/tmp/wb-repricer-fe-173-2-settings-backfill` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.2 worktree;
- local and remote branch queries return no `cdx/epic-173-story-2-settings-backfill`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: job verification and branch/worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of backfill table, dialog, status, progress, and recovery. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: backfill item/status/time/action primary columns; explicit mobile detail. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: named actions, progress announcement, Dialog focus, durable result summary. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, queued/running, safe-to-leave, success, partial/failure, retry. | Test each applicable fixture and block merge on an unhandled state. |
