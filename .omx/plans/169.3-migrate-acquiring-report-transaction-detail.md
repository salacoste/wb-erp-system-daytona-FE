---
storyId: "169.3"
storyTitle: "Migrate Acquiring Report Transaction Detail"
epic: "169-FE"
...
branch: cdx/epic-169-story-3-acquiring-report-detail-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.3: Migrate Acquiring Report Transaction Detail

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1693-migrate-acquiring-report-transaction-detail` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As a finance user, I want `/analytics/acquiring/reports/[id]` to expose report totals and transaction evidence accessibly, so that I can trace reconciliation differences to exact transactions.
- **Resolved owned surface:** Dynamic route `/analytics/acquiring/reports/[id]`; its `page.tsx`, `components/AcquiringReportDetailPage.tsx`, `AcquiringReportDetailSummary.tsx`, `AcquiringTransactionsTable.tsx`, and colocated tests.
...
- Story 169.1 is merged into current `main`; record its merge SHA before creating this Story's branch.
...
flowchart TD
  ...
  S167_1 --> THIS["Story 169.3"]
  S168_1 --> THIS
  S169_1["Story 169.1"] --> THIS
```
...
- **Exact branch:** `cdx/epic-169-story-3-acquiring-report-detail-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-3-acquiring-report-detail-shadcn" "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/acquiring/reports/[id]/**`.
- **Resolved owned files/tree:** Dynamic route `/analytics/acquiring/reports/[id]`; its `page.tsx`, `components/AcquiringReportDetailPage.tsx`, `AcquiringReportDetailSummary.tsx`, `AcquiringTransactionsTable.tsx`, and colocated tests.
...
- **Canonical prohibition:** FS plus acquiring index/period trees and global not-found handling.
...
1. From `/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/acquiring/reports/[id]"
   ```
...
4. Capture deterministic baseline evidence for: SC plus invalid ID, `notFound`, report-level success with transaction-section failure, and rate-limit states.
...
5. **Apply the route-specific data contract:** RTC; transactions table caption includes report identity, primary column is transaction/operation identifier, dates and RUB columns remain aligned/full precision, row details/actions remain reachable. No chart is introduced.
6. **Apply accessibility and responsive behavior:** AX; dynamic route title names the report, not-found is semantically distinct, and transaction row actions include entity identity. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/acquiring/reports/[id]` and `STORY_ID=169.3`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn"
npx vitest run "src/app/(dashboard)/analytics/acquiring/reports/[id]"
npm run test:e2e -- --grep "Story 169.3-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" diff --check
```
...
- **State matrix:** SC plus invalid ID, `notFound`, report-level success with transaction-section failure, and rate-limit states.
- **Route-specific table/chart contract:** RTC; transactions table caption includes report identity, primary column is transaction/operation identifier, dates and RUB columns remain aligned/full precision, row details/actions remain reachable. No chart is introduced.
...
- **Required Story evidence:** VE plus valid/invalid ID, partial transaction section, and dense-table evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" commit \
  -m "feat(analytics): migrate Story 169.3 route to shadcn" \
  -m "Story 169.3-FE — Migrate Acquiring Report Transaction Detail. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn" push -u origin "cdx/epic-169-story-3-acquiring-report-detail-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-3-acquiring-report-detail-shadcn" \
  --title "[Story 169.3-FE] Migrate Acquiring Report Transaction Detail" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-3-acquiring-report-detail-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-3-acquiring-report-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-3-acquiring-report-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-3-acquiring-report-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-3-acquiring-report-detail-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus invalid ID, `notFound`, report-level success with transaction-section failure, and rate-limit states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; transactions table caption includes report identity, primary column is transaction/operation identifier, dates and RUB columns remain aligned/full precision, row details/actions remain reachable. No chart is introduced. |
| Keyboard/focus/contrast regression | Apply AX; dynamic route title names the report, not-found is semantically distinct, and transaction row actions include entity identity. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-3-acquiring-report-detail-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-3-acquiring-report-detail-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-3-acquiring-report-detail-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
