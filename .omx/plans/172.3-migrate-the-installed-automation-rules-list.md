---
storyId: "172.3"
storyTitle: "Migrate the Installed Automation Rules List"
epic: "172-FE"
...
branch: cdx/epic-172-story-3-installed-rules
temporaryWorktree: /private/tmp/wb-repricer-fe-172-3-installed-rules
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.3 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.3 scope below takes precedence.
...
- **Required outcome:** Migrate the Installed Automation Rules List. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** 172.2 for shared automation presentation if reused; Epics 166-FE and 167.1.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_3["Story 172.3"]
  P2["Story 167.1"] --> S172_3["Story 172.3"]
  P3["Story 172.2 when its shared automation presentation is reused"] --> S172_3["Story 172.3"]
```
...
- [ ] Verify the merge SHA for **Story 172.2 when its shared automation presentation is reused** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `172.3` / `Migrate the Installed Automation Rules List` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.3 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.3. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-3-installed-rules`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-3-installed-rules`
...
test ! -e "/private/tmp/wb-repricer-fe-172-3-installed-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-3-installed-rules" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-3-installed-rules" "/private/tmp/wb-repricer-fe-172-3-installed-rules" main
git -C "/private/tmp/wb-repricer-fe-172-3-installed-rules" status --short --branch
```
...
- Owned Surface and automation-list tests/fixtures.
- Canonical owned surface: route directory, `InstalledRulesList.tsx`, `InstalledRuleRow.tsx`, `PostInstallBanner.tsx`, exclusive tests.
...
- foundation, AppShell, API/hooks/types, dynamic editor owned by 172.4.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.3.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, populated, stale status, pending update, error, restricted action.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory, `InstalledRulesList.tsx`, `InstalledRuleRow.tsx`, `PostInstallBanner.tsx`, exclusive tests.
2. Deliver the user value and interaction surface exactly as recorded: `/automation/installed-rules`; review installed rules, lifecycle status, and detail navigation.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, populated, stale status, pending update, error, restricted action.
6. Apply the Story contract for density and responsive behavior: primary rule name/status/action retained on narrow width; secondary metadata progressively disclosed.
7. Apply the Story accessibility contract: named row actions, status text/icons, keyboard detail navigation, focus preservation.
...
- `npm test -- --run 'src/app/(dashboard)/automation/installed-rules' src/components/custom/automation/__tests__/InstalledRulesList.test.tsx`
- `npm run test:e2e -- e2e/automation/installed-rules.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-3-installed-rules" diff --check
```
...
- `installed-list tests and state/responsive/theme screenshots.`;
- canonical local-validation intent: targeted tests plus Universal Local Validation Contract.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.3/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** primary rule name/status/action retained on narrow width; secondary metadata progressively disclosed.
- **Story accessibility contract:** named row actions, status text/icons, keyboard detail navigation, focus preservation.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.3.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-3-installed-rules diff --check
git -C /private/tmp/wb-repricer-fe-172-3-installed-rules status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-3-installed-rules" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-3-installed-rules" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-3-installed-rules" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-3-installed-rules diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-3-installed-rules commit \
  -m "feat(automation): migrate the installed automation rules list" \
  -m "Story 172.3. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-3-installed-rules push -u origin cdx/epic-172-story-3-installed-rules
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-3-installed-rules" \
  --title "Story 172.3: Migrate the Installed Automation Rules List" \
  --body "Implements BMAD Story 172.3 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-3-installed-rules"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-3-installed-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-3-installed-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-3-installed-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-3-installed-rules"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-3-installed-rules"
...
- `/private/tmp/wb-repricer-fe-172-3-installed-rules` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.3 worktree;
- local and remote branch queries return no `cdx/epic-172-story-3-installed-rules`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: completed branch and worktree cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of `/automation/installed-rules`; review installed rules, lifecycle status, and detail navigation. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because 172.2 for shared automation presentation if reused; Epics 166-FE and 167.1. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: primary rule name/status/action retained on narrow width; secondary metadata progressively disclosed. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: named row actions, status text/icons, keyboard detail navigation, focus preservation. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, populated, stale status, pending update, error, restricted action. | Test each applicable fixture and block merge on an unhandled state. |
