...
- **Required outcome:** Prove exact one-to-one parity among the 92 canonical BMAD Stories, 92 per-Story OMX plans, 76 source routes, 76 route-ledger rows, ownership records, prerequisites, and evidence schema. This Story validates planning/tracking truth and does not migrate runtime UI.
...
3. Prove `92 BMAD Stories = 92 OMX plans` and `76 source routes = 76 ledger routes = 76 route Stories`, with every item mapped exactly once.
4. Validate Story 167.8's exact backend repository, branch, and worktree contract against `/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new`; validate all other Story plans against their declared repository. Cross-repository parity may read both repositories but may not mutate backend product code.
...
- `machine check: 76 source routes = 76 ledger routes = 76 route Stories; 92 BMAD Stories = 92 OMX plans; no duplicates.`;
...
| --- | --- |
| Planning work omits or misclassifies one of 76 routes or 92 Stories, including the cross-repository Story 167.8 contract. | Compare canonical IDs/titles, repository/branch/worktree fields, route ownership, and exact counts; rerun parity proof after the final diff. |
