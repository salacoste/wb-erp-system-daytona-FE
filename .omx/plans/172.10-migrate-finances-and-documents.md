---
storyId: "172.10"
storyTitle: "Migrate Finances and Documents"
epic: "172-FE"
...
branch: cdx/epic-172-story-10-finances
temporaryWorktree: /private/tmp/wb-repricer-fe-172-10-finances
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.10 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_10["Story 172.10"]
  P2["Story 167.1"] --> S172_10["Story 172.10"]
```
...
- [ ] Confirm Story ID/title parity: `172.10` / `Migrate Finances and Documents` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.10 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.10. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-10-finances`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-10-finances`
...
test ! -e "/private/tmp/wb-repricer-fe-172-10-finances"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-10-finances" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-10-finances" "/private/tmp/wb-repricer-fe-172-10-finances" main
git -C "/private/tmp/wb-repricer-fe-172-10-finances" status --short --branch
```
...
- Owned Surface and download/filter tests.
- Canonical owned surface: route directory and finance-exclusive components/tests.
...
- tokens/primitives/AppShell, finance API/hooks/types/download contracts.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.10.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, no documents, filtered empty, balance unavailable/partial, download pending/ready/failed, route error.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and finance-exclusive components/tests.
2. Deliver the user value and interaction surface exactly as recorded: balance summary, document filters, table, pagination, and downloads.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, no documents, filtered empty, balance unavailable/partial, download pending/ready/failed, route error.
6. Apply the Story contract for density and responsive behavior: document identity/type/period/action primary columns; server filter/pagination preserved; full balance precision.
7. Apply the Story accessibility contract: accessible table, named downloads, pending status announcement, non-color document/status meaning.
...
- `npm test -- --run 'src/app/(dashboard)/finances' src/lib/finances`
- `npm run test:e2e -- e2e/finances.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-10-finances" diff --check
```
...
- `filters/pagination/download regressions plus responsive/theme screenshots.`;
- canonical local-validation intent: targeted finance tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.10/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** document identity/type/period/action primary columns; server filter/pagination preserved; full balance precision.
- **Story accessibility contract:** accessible table, named downloads, pending status announcement, non-color document/status meaning.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.10.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-10-finances diff --check
git -C /private/tmp/wb-repricer-fe-172-10-finances status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-10-finances" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-10-finances" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-10-finances" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-10-finances diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-10-finances commit \
  -m "feat(finances): migrate finances and documents" \
  -m "Story 172.10. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-10-finances push -u origin cdx/epic-172-story-10-finances
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-10-finances" \
  --title "Story 172.10: Migrate Finances and Documents" \
  --body "Implements BMAD Story 172.10 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-10-finances"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-10-finances"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-10-finances"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-10-finances"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-10-finances"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-10-finances"
...
- `/private/tmp/wb-repricer-fe-172-10-finances` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.10 worktree;
- local and remote branch queries return no `cdx/epic-172-story-10-finances`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: completed lifecycle and removed worktree evidence.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of balance summary, document filters, table, pagination, and downloads. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: document identity/type/period/action primary columns; server filter/pagination preserved; full balance precision. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: accessible table, named downloads, pending status announcement, non-color document/status meaning. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, no documents, filtered empty, balance unavailable/partial, download pending/ready/failed, route error. | Test each applicable fixture and block merge on an unhandled state. |
