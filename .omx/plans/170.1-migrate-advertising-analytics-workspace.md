---
storyId: "170.1"
storyTitle: "Migrate Advertising Analytics Workspace"
epic: "170-FE"
...
branch: cdx/epic-170-story-1-advertising-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-170-1-advertising-shadcn
productionScope: forbidden
---

# OMX Plan — Story 170.1: Migrate Advertising Analytics Workspace

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1701-migrate-advertising-analytics-workspace` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As a marketing/finance user, I want `/analytics/advertising` to connect campaign selection, efficiency, attribution, spend discrepancies, trends, product performance, and sync gaps consistently, so that I can compare advertising outcomes and reach trustworthy evidence.
...
- **Exact branch:** `cdx/epic-170-story-1-advertising-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-170-1-advertising-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-170-story-1-advertising-shadcn" "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" main
...
- **Canonical allowance:** Only the advertising root route files/components/utils/tests; no nested campaign-detail files.
- **Resolved owned files/tree:** `/analytics/advertising`; root `page.tsx`, `loading.tsx`, `error.tsx`, complete root `components/**` and `utils/**` trees (header/filters/selectors, sync/efficiency/attribution banners, summary/campaign list, daily trend, discrepancy/cannibalization sections, merged/performance tables and helpers), and colocated tests. The nested `campaigns/[advertId]/**` route is excluded.
...
- **Canonical prohibition:** FS plus `advertising/campaigns/**` and shared custom components not exclusively owned here.
...
1. From `/private/tmp/wb-repricer-fe-170-1-advertising-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/advertising"
   ```
...
4. Capture deterministic baseline evidence for: SC plus sync-gap, over-attribution, multi-campaign warning, discrepancy, partial comparison/daily-series, and route-boundary error states.
...
5. **Apply the route-specific data contract:** RTC; performance/merged tables use campaign or product/SKU as primary column, expose selection/grouping/sort and aligned spend/revenue/ratio metrics. Daily/discrepancy charts name period, units, actual/comparison series and provide data alternatives; chart/table values reconcile.
6. **Apply accessibility and responsive behavior:** AX; filter/toggle/campaign selectors expose current state; warnings and efficiency are non-color; charts and dense tables are keyboard/touch complete. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/advertising` and `STORY_ID=170.1`, excluding nested campaign tests from ownership assertions.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn"
npx vitest run "src/app/(dashboard)/analytics/advertising"
npm run test:e2e -- --grep "Story 170.1-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" diff --check
```
...
- **State matrix:** SC plus sync-gap, over-attribution, multi-campaign warning, discrepancy, partial comparison/daily-series, and route-boundary error states.
- **Route-specific table/chart contract:** RTC; performance/merged tables use campaign or product/SKU as primary column, expose selection/grouping/sort and aligned spend/revenue/ratio metrics. Daily/discrepancy charts name period, units, actual/comparison series and provide data alternatives; chart/table values reconcile.
...
- **Required Story evidence:** VE plus sync-gap, over-attribution, grouped table, discrepancy, daily comparison, and mixed-partial evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" commit \
  -m "feat(analytics): migrate Story 170.1 route to shadcn" \
  -m "Story 170.1-FE — Migrate Advertising Analytics Workspace. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn" push -u origin "cdx/epic-170-story-1-advertising-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-170-story-1-advertising-shadcn" \
  --title "[Story 170.1-FE] Migrate Advertising Analytics Workspace" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-170-story-1-advertising-shadcn"
...
Canonical cleanup requirement: CE, including proof the nested campaign route and its exclusive custom card were untouched.

After the merge SHA is proven on `main`, execute in this order:

```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-170-story-1-advertising-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-170-1-advertising-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-170-story-1-advertising-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-170-story-1-advertising-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-170-story-1-advertising-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-170-1-advertising-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus sync-gap, over-attribution, multi-campaign warning, discrepancy, partial comparison/daily-series, and route-boundary error states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; performance/merged tables use campaign or product/SKU as primary column, expose selection/grouping/sort and aligned spend/revenue/ratio metrics. Daily/discrepancy charts name period, units, actual/comparison series and provide data alternatives; chart/table values reconcile. |
| Keyboard/focus/contrast regression | Apply AX; filter/toggle/campaign selectors expose current state; warnings and efficiency are non-color; charts and dense tables are keyboard/touch complete. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-170-story-1-advertising-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-170-1-advertising-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-170-story-1-advertising-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-170-1-advertising-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
