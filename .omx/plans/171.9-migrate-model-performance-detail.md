---
storyId: "171.9"
storyTitle: "Migrate Model Performance Detail"
epic: "171-FE"
...
branch: cdx/epic-171-story-9-model-performance-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-9-model-performance-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.9: Migrate Model Performance Detail

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1719-migrate-model-performance-detail` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As a model analyst, I want `/analytics/models/[id]/performance` to connect model identity, performance metrics, MAPE trend, and evaluation history, so that I can judge model quality and trace changes to evaluation runs.
- **Resolved owned surface:** `/analytics/models/[id]/performance`; dynamic route `src/app/(dashboard)/analytics/models/[id]/performance/**`; its `page.tsx`, `ModelPerformanceDetail.tsx`, `MapeTrendChart.tsx`, `EvaluationHistoryTable.tsx`, helpers, and colocated tests.
...
- **Exact branch:** `cdx/epic-171-story-9-model-performance-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-9-model-performance-shadcn" "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" main
...
- **Canonical allowance:** Only the model performance route tree.
- **Resolved owned files/tree:** `/analytics/models/[id]/performance`; dynamic route `src/app/(dashboard)/analytics/models/[id]/performance/**`; its `page.tsx`, `ModelPerformanceDetail.tsx`, `MapeTrendChart.tsx`, `EvaluationHistoryTable.tsx`, helpers, and colocated tests.
...
- **Canonical prohibition:** FS and model registry/evaluations/AI-admin trees.
...
1. From `/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/models/[id]/performance"
   ```
...
4. Capture deterministic baseline evidence for: SC plus invalid/not-found model, no history, insufficient sample, undefined versus zero metric, and mixed summary/chart/table partial states.
...
5. **Apply the route-specific data contract:** RTC; evaluation history table caption names the model, primary column is evaluation/run timestamp or ID with metric/sample/destination. MAPE chart names period/percentage units/series, provides summary/data alternative, and reconciles with history rows.
6. **Apply accessibility and responsive behavior:** AX; metric quality/direction is non-color, chart point access has keyboard/touch equivalent, and dynamic route title names the model. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/models/[id]/performance` and `STORY_ID=171.9`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn"
npx vitest run "src/app/(dashboard)/analytics/models/[id]/performance"
npm run test:e2e -- --grep "Story 171.9-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" diff --check
```
...
- **State matrix:** SC plus invalid/not-found model, no history, insufficient sample, undefined versus zero metric, and mixed summary/chart/table partial states.
- **Route-specific table/chart contract:** RTC; evaluation history table caption names the model, primary column is evaluation/run timestamp or ID with metric/sample/destination. MAPE chart names period/percentage units/series, provides summary/data alternative, and reconciles with history rows.
...
- **Required Story evidence:** VE plus invalid model, insufficient sample, mixed partial section, undefined versus zero, and chart/table reconciliation evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" commit \
  -m "feat(analytics): migrate Story 171.9 route to shadcn" \
  -m "Story 171.9-FE — Migrate Model Performance Detail. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn" push -u origin "cdx/epic-171-story-9-model-performance-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-9-model-performance-shadcn" \
  --title "[Story 171.9-FE] Migrate Model Performance Detail" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-9-model-performance-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-9-model-performance-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-9-model-performance-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-9-model-performance-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-9-model-performance-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus invalid/not-found model, no history, insufficient sample, undefined versus zero metric, and mixed summary/chart/table partial states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; evaluation history table caption names the model, primary column is evaluation/run timestamp or ID with metric/sample/destination. MAPE chart names period/percentage units/series, provides summary/data alternative, and reconciles with history rows. |
| Keyboard/focus/contrast regression | Apply AX; metric quality/direction is non-color, chart point access has keyboard/touch equivalent, and dynamic route title names the model. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-9-model-performance-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-9-model-performance-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-9-model-performance-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
