---
storyId: "170.2"
storyTitle: "Migrate Advertising Campaign Bid-Recommendation Detail"
epic: "170-FE"
...
branch: cdx/epic-170-story-2-advertising-campaign-detail-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn
productionScope: forbidden
---

# OMX Plan — Story 170.2: Migrate Advertising Campaign Bid-Recommendation Detail

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1702-migrate-advertising-campaign-bid-recommendation-detail` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As a marketing user, I want `/analytics/advertising/campaigns/[advertId]` to show campaign identity and bid recommendations with clear context and recovery, so that I can review recommendations without losing the originating campaign/product selection.
...
- Story 170.1 is merged into current `main`; record its merge SHA before creating this Story's branch.
...
flowchart TD
  ...
  S167_1 --> THIS["Story 170.2"]
  S168_1 --> THIS
  S170_1["Story 170.1"] --> THIS
```
...
- **Exact branch:** `cdx/epic-170-story-2-advertising-campaign-detail-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-170-story-2-advertising-campaign-detail-shadcn" "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" main
...
- **Canonical allowance:** Dynamic route page/tests and the exclusive `BidRecommendationsCard`/tests only.
- **Resolved owned files/tree:** `/analytics/advertising/campaigns/[advertId]`; dynamic route `src/app/(dashboard)/analytics/advertising/campaigns/[advertId]/**`, plus exclusively consumed `src/components/custom/advertising/BidRecommendationsCard.tsx` and its tests. Its API hook/client/type contracts remain forbidden dependencies.
...
- **Canonical prohibition:** FS plus advertising root route, recommendation hooks/API/types, and global route/not-found definitions.
...
1. From `/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/advertising/campaigns/[advertId]"
   ```
...
4. Capture deterministic baseline evidence for: SC plus invalid `advertId`, absent/invalid `nmId`, cabinet hydration, no recommendation, and recommendation partial/error states.
...
5. **Apply the route-specific data contract:** RTC; recommendation evidence prioritizes recommendation/entity, current/recommended bid, rationale/status, and action. Any tabular list has accessible name and bounded narrow strategy; no chart is added unless already inside the exclusive card.
6. **Apply accessibility and responsive behavior:** AX; back link is a semantic link without nested interactive semantics, campaign title is the `h1`, repeated actions name campaign/product, and any confirmation overlay restores focus. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/advertising/campaigns/[advertId]` and `STORY_ID=170.2`; also run `npx vitest run "src/components/custom/advertising/__tests__/BidRecommendationsCard.test.tsx"`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn"
npx vitest run "src/app/(dashboard)/analytics/advertising/campaigns/[advertId]"
npm run test:e2e -- --grep "Story 170.2-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" diff --check
```
...
- **State matrix:** SC plus invalid `advertId`, absent/invalid `nmId`, cabinet hydration, no recommendation, and recommendation partial/error states.
- **Route-specific table/chart contract:** RTC; recommendation evidence prioritizes recommendation/entity, current/recommended bid, rationale/status, and action. Any tabular list has accessible name and bounded narrow strategy; no chart is added unless already inside the exclusive card.
...
- **Required Story evidence:** VE plus invalid ID, missing cabinet, empty/partial recommendation, and deep-link/back-context evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" commit \
  -m "feat(analytics): migrate Story 170.2 route to shadcn" \
  -m "Story 170.2-FE — Migrate Advertising Campaign Bid-Recommendation Detail. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn" push -u origin "cdx/epic-170-story-2-advertising-campaign-detail-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-170-story-2-advertising-campaign-detail-shadcn" \
  --title "[Story 170.2-FE] Migrate Advertising Campaign Bid-Recommendation Detail" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-170-story-2-advertising-campaign-detail-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-170-story-2-advertising-campaign-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-170-story-2-advertising-campaign-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-170-story-2-advertising-campaign-detail-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-170-story-2-advertising-campaign-detail-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus invalid `advertId`, absent/invalid `nmId`, cabinet hydration, no recommendation, and recommendation partial/error states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; recommendation evidence prioritizes recommendation/entity, current/recommended bid, rationale/status, and action. Any tabular list has accessible name and bounded narrow strategy; no chart is added unless already inside the exclusive card. |
| Keyboard/focus/contrast regression | Apply AX; back link is a semantic link without nested interactive semantics, campaign title is the `h1`, repeated actions name campaign/product, and any confirmation overlay restores focus. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-170-story-2-advertising-campaign-detail-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-170-story-2-advertising-campaign-detail-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-170-2-advertising-campaign-detail-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
