---
storyId: "169.9"
storyTitle: "Migrate Analytics Gaps Triage"
epic: "169-FE"
...
branch: cdx/epic-169-story-9-gaps-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-9-gaps-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.9: Migrate Analytics Gaps Triage

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1699-migrate-analytics-gaps-triage` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an operations user, I want `/analytics/gaps` to prioritize data/operational gaps and explain each affected entity, so that I can investigate and hand off the correct recovery action.
- **Resolved owned surface:** `/analytics/gaps`; its `page.tsx`, `components/GapsPageContent.tsx`, `GapsSummaryCards.tsx`, `GapsTable.tsx`, `GapAnalysisDialog.tsx`, `useGapsPageState.ts`, and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-9-gaps-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-9-gaps-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-9-gaps-shadcn" "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/gaps/**`.
- **Resolved owned files/tree:** `/analytics/gaps`; its `page.tsx`, `components/GapsPageContent.tsx`, `GapsSummaryCards.tsx`, `GapsTable.tsx`, `GapAnalysisDialog.tsx`, `useGapsPageState.ts`, and colocated tests.
...
1. From `/private/tmp/wb-repricer-fe-169-9-gaps-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/gaps"
   ```
...
4. Capture deterministic baseline evidence for: SC plus valid no-gaps, unknown classification, analyzed/pending, and dialog detail-error states.
...
5. **Apply the route-specific data contract:** RTC; gaps table primary column is affected entity, priority/status/reason/action remain reachable; narrow detail may use the owned dialog/Sheet behavior without losing queue context. No chart.
6. **Apply accessibility and responsive behavior:** AX; severity is label/icon/order based, repeated detail actions name the entity, and dialog focus lifecycle is verified. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/gaps` and `STORY_ID=169.9`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn"
npx vitest run "src/app/(dashboard)/analytics/gaps"
npm run test:e2e -- --grep "Story 169.9-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" diff --check
```
...
- **State matrix:** SC plus valid no-gaps, unknown classification, analyzed/pending, and dialog detail-error states.
- **Route-specific table/chart contract:** RTC; gaps table primary column is affected entity, priority/status/reason/action remain reachable; narrow detail may use the owned dialog/Sheet behavior without losing queue context. No chart.
...
- **Required Story evidence:** VE plus no-gaps versus failed-analysis and dialog lifecycle evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" commit \
  -m "feat(analytics): migrate Story 169.9 route to shadcn" \
  -m "Story 169.9-FE — Migrate Analytics Gaps Triage. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn" push -u origin "cdx/epic-169-story-9-gaps-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-9-gaps-shadcn" \
  --title "[Story 169.9-FE] Migrate Analytics Gaps Triage" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-9-gaps-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-9-gaps-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-9-gaps-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-9-gaps-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-9-gaps-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-9-gaps-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-9-gaps-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus valid no-gaps, unknown classification, analyzed/pending, and dialog detail-error states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; gaps table primary column is affected entity, priority/status/reason/action remain reachable; narrow detail may use the owned dialog/Sheet behavior without losing queue context. No chart. |
| Keyboard/focus/contrast regression | Apply AX; severity is label/icon/order based, repeated detail actions name the entity, and dialog focus lifecycle is verified. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-9-gaps-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-9-gaps-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-9-gaps-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-9-gaps-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
