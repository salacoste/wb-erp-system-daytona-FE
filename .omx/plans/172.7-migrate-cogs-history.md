---
storyId: "172.7"
storyTitle: "Migrate COGS History"
epic: "172-FE"
...
branch: cdx/epic-172-story-7-cogs-history
temporaryWorktree: /private/tmp/wb-repricer-fe-172-7-cogs-history
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.7 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_7["Story 172.7"]
  P2["Story 167.1"] --> S172_7["Story 172.7"]
  P3["Story 172.5"] --> S172_7["Story 172.7"]
```
...
- [ ] Confirm Story ID/title parity: `172.7` / `Migrate COGS History` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.7 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.7. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-7-cogs-history`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-7-cogs-history`
...
test ! -e "/private/tmp/wb-repricer-fe-172-7-cogs-history"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-7-cogs-history" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-7-cogs-history" "/private/tmp/wb-repricer-fe-172-7-cogs-history" main
git -C "/private/tmp/wb-repricer-fe-172-7-cogs-history" status --short --branch
```
...
- Owned Surface and history tests/fixtures.
- Canonical owned surface: route directory; `CogsHistoryMeta*`, `CogsHistoryTable*`, `CogsHistoryPagination`, exclusive tests.
...
- tokens/primitives/AppShell, history API/hooks/types, other COGS routes.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.7.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty, populated, stale/partial, error.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory; `CogsHistoryMeta*`, `CogsHistoryTable*`, `CogsHistoryPagination`, exclusive tests.
2. Deliver the user value and interaction surface exactly as recorded: COGS history search, metadata, table, and pagination.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty, populated, stale/partial, error.
6. Apply the Story contract for density and responsive behavior: product/change/date/actor context, numeric precision, semantic headers, server pagination and narrow-width row detail.
7. Apply the Story accessibility contract: accessible table name, announced sort/pagination where present, full currency precision and change direction text.
...
- `npm test -- --run 'src/app/(dashboard)/cogs/history'`
- `npm run test:e2e -- e2e/cogs-pages.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-7-cogs-history" diff --check
```
...
- `table/pagination regression plus responsive/theme screenshots.`;
- canonical local-validation intent: targeted history tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.7/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** product/change/date/actor context, numeric precision, semantic headers, server pagination and narrow-width row detail.
- **Story accessibility contract:** accessible table name, announced sort/pagination where present, full currency precision and change direction text.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.7.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-7-cogs-history diff --check
git -C /private/tmp/wb-repricer-fe-172-7-cogs-history status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-7-cogs-history" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-7-cogs-history" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-7-cogs-history" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-7-cogs-history diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-7-cogs-history commit \
  -m "feat(cogs): migrate cogs history" \
  -m "Story 172.7. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-7-cogs-history push -u origin cdx/epic-172-story-7-cogs-history
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-7-cogs-history" \
  --title "Story 172.7: Migrate COGS History" \
  --body "Implements BMAD Story 172.7 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-7-cogs-history"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-7-cogs-history"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-7-cogs-history"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-7-cogs-history"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-7-cogs-history"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-7-cogs-history"
...
- `/private/tmp/wb-repricer-fe-172-7-cogs-history` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.7 worktree;
- local and remote branch queries return no `cdx/epic-172-story-7-cogs-history`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: merge and repository cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of COGS history search, metadata, table, and pagination. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
...
| Responsive or dense-data treatment loses evidence: product/change/date/actor context, numeric precision, semantic headers, server pagination and narrow-width row detail. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: accessible table name, announced sort/pagination where present, full currency precision and change direction text. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty, populated, stale/partial, error. | Test each applicable fixture and block merge on an unhandled state. |
