---
storyId: "174.4"
storyTitle: "Complete Full Local Functional and Backend-Contract Regression"
epic: "174-FE"
...
branch: cdx/epic-174-story-4-full-local-regression
temporaryWorktree: /private/tmp/wb-repricer-fe-174-4-full-local-regression
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 174.4 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 174.4 scope below takes precedence.
...
- **Required outcome:** Prove the fully migrated frontend against the pinned local frontend/backend environment, complete Vitest and relevant Playwright suites, critical journeys, and request/contract probes without changing backend, production, or public contracts.
- **Canonical shared dependency statement:** all route Stories and Story 174.3 merged.
...
```mermaid
flowchart LR
  P1["Story 174.3"] --> S174_4["Story 174.4"]
  P2["All route migrations merged"] --> S174_4["Story 174.4"]
```

- [ ] Verify the merge SHA for **Story 174.3** is reachable from current local `main`; record that SHA in the Story delivery record.
...
- [ ] Confirm Story ID/title parity: `174.4` / `Complete Full Local Functional and Backend-Contract Regression` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 174.4 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 174.4. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-174-story-4-full-local-regression`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-174-4-full-local-regression`
...
test ! -e "/private/tmp/wb-repricer-fe-174-4-full-local-regression"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-174-story-4-full-local-regression" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-174-story-4-full-local-regression" "/private/tmp/wb-repricer-fe-174-4-full-local-regression" main
git -C "/private/tmp/wb-repricer-fe-174-4-full-local-regression" status --short --branch
```
...
- tests/evidence and reviewed fixes within explicit route ownership.
- Canonical owned surface: full regression configuration/evidence and narrowly scoped owner-coordinated fixes.
...
- backend code, production/deployment config, contract changes, required CI gates.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 174.4.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **authentication/onboarding, COGS single/bulk, analytics, writeback, exports, long jobs, dynamic entities, session/error recovery.**
...
1. Verify Node/npm pins and local frontend/backend readiness; preserve complete preflight and failure output.
2. Run the complete Vitest suite, relevant full Playwright suites, lint, type-check, max-lines, and production build in dependency order.
3. Exercise critical journeys for authentication/onboarding, single/bulk COGS, analytics, writeback/retry, exports/downloads, long jobs, dynamic entities, and session/error recovery.
4. Capture request/contract probes for URLs, headers, query behavior, request counts, calculations, formatting, authorization, mutation outcomes, and cache behavior.
5. File failures against the owning route/domain. Apply only reviewed fixes within explicit ownership, then rerun the smallest affected proof followed by the relevant full suite.
6. Treat every unresolved regression or unavailable mandatory environment as a completion blocker; no production action is an alternative validation path.
7. Publish the regression report and inspect the final diff for weakened assertions, hidden contract changes, or scope expansion.
...
npm run build
git -C "/private/tmp/wb-repricer-fe-174-4-full-local-regression" diff --check
```
...
- `full Vitest, relevant Playwright suites, request/contract probes, build, lint, type-check, max-lines.`;
- canonical local-validation intent: complete pinned-runtime local suite with preserved failure output.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-174.4/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** verify no request duplication/performance regression caused by responsive/composition changes.
- **Story accessibility contract:** critical journey keyboard and status-announcement smoke remains part of functional verification.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 174.4.
...
```bash
git -C /private/tmp/wb-repricer-fe-174-4-full-local-regression diff --check
git -C /private/tmp/wb-repricer-fe-174-4-full-local-regression status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-174-4-full-local-regression" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-174-4-full-local-regression" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-174-4-full-local-regression" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-174-4-full-local-regression diff --cached --stat
git -C /private/tmp/wb-repricer-fe-174-4-full-local-regression commit \
  -m "test(migration): complete full local functional and backend contract regression" \
  -m "Story 174.4. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-174-4-full-local-regression push -u origin cdx/epic-174-story-4-full-local-regression
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-174-story-4-full-local-regression" \
  --title "Story 174.4: Complete Full Local Functional and Backend-Contract Regression" \
  --body "Implements BMAD Story 174.4 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-174-story-4-full-local-regression"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-174-story-4-full-local-regression"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-174-4-full-local-regression"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-174-story-4-full-local-regression"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-174-story-4-full-local-regression"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-174-story-4-full-local-regression"
...
- `/private/tmp/wb-repricer-fe-174-4-full-local-regression` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 174.4 worktree;
- local and remote branch queries return no `cdx/epic-174-story-4-full-local-regression`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: regression report, resolved findings, branch/worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of all routes and critical journeys on `localhost:3100` with backend `localhost:3000`. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because all route Stories and Story 174.3 merged. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: verify no request duplication/performance regression caused by responsive/composition changes. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: critical journey keyboard and status-announcement smoke remains part of functional verification. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: authentication/onboarding, COGS single/bulk, analytics, writeback, exports, long jobs, dynamic entities, session/error recovery. | Test each applicable fixture and block merge on an unhandled state. |
