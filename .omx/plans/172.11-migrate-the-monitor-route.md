---
storyId: "172.11"
storyTitle: "Migrate the Monitor Route"
epic: "172-FE"
...
branch: cdx/epic-172-story-11-monitor
temporaryWorktree: /private/tmp/wb-repricer-fe-172-11-monitor
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.11 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.11 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR27
- **Route/User Value:** monitor KPIs, trends, status surfaces, tables, and actions.
- **Required outcome:** Migrate the Monitor Route. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** foundation/AppShell; no implicit ownership of `/monitoring` components.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_11["Story 172.11"]
  P2["Story 167.1"] --> S172_11["Story 172.11"]
```
...
- [ ] Confirm Story ID/title parity: `172.11` / `Migrate the Monitor Route` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.11 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.11. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-11-monitor`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-11-monitor`
...
test ! -e "/private/tmp/wb-repricer-fe-172-11-monitor"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-11-monitor" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-11-monitor" "/private/tmp/wb-repricer-fe-172-11-monitor" main
git -C "/private/tmp/wb-repricer-fe-172-11-monitor" status --short --branch
```
...
- Owned Surface and route-specific tests/visuals.
- Canonical owned surface: `src/app/(dashboard)/monitor/**` and exclusive custom components/tests.
...
- tokens/primitives/AppShell, monitoring APIs/hooks/types, `/monitoring` owned files.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.11.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, stale, partial, degraded, healthy, error, action pending where applicable.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/monitor/**` and exclusive custom components/tests.
2. Deliver the user value and interaction surface exactly as recorded: monitor KPIs, trends, status surfaces, tables, and actions.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, stale, partial, degraded, healthy, error, action pending where applicable.
6. Apply the Story contract for density and responsive behavior: exception-first hierarchy; metric/table/chart contracts with units, periods, primary columns, and accessible summaries.
7. Apply the Story accessibility contract: severity by text/icon/order, keyboard actions, chart alternative, theme contrast.
...
- `npm test -- --run 'src/app/(dashboard)/monitor'`
- `npm run test:e2e -- e2e/monitor.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-11-monitor" diff --check
```
...
- `monitor regression tests and representative health/state screenshots.`;
- canonical local-validation intent: targeted monitor tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.11/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** exception-first hierarchy; metric/table/chart contracts with units, periods, primary columns, and accessible summaries.
- **Story accessibility contract:** severity by text/icon/order, keyboard actions, chart alternative, theme contrast.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.11.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-11-monitor diff --check
git -C /private/tmp/wb-repricer-fe-172-11-monitor status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-11-monitor" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-11-monitor" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-11-monitor" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-11-monitor diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-11-monitor commit \
  -m "feat(monitor): migrate the monitor route" \
  -m "Story 172.11. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-11-monitor push -u origin cdx/epic-172-story-11-monitor
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-11-monitor" \
  --title "Story 172.11: Migrate the Monitor Route" \
  --body "Implements BMAD Story 172.11 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-11-monitor"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-11-monitor"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-11-monitor"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-11-monitor"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-11-monitor"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-11-monitor"
...
- `/private/tmp/wb-repricer-fe-172-11-monitor` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.11 worktree;
- local and remote branch queries return no `cdx/epic-172-story-11-monitor`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: merge and worktree cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of monitor KPIs, trends, status surfaces, tables, and actions. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because foundation/AppShell; no implicit ownership of `/monitoring` components. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: exception-first hierarchy; metric/table/chart contracts with units, periods, primary columns, and accessible summaries. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: severity by text/icon/order, keyboard actions, chart alternative, theme contrast. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, stale, partial, degraded, healthy, error, action pending where applicable. | Test each applicable fixture and block merge on an unhandled state. |
