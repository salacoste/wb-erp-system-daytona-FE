---
storyId: "170.5"
storyTitle: "Migrate Category Margin Analytics"
epic: "170-FE"
...
branch: cdx/epic-170-story-5-category-margin-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-170-5-category-margin-shadcn
productionScope: forbidden
---

# OMX Plan — Story 170.5: Migrate Category Margin Analytics

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1705-migrate-category-margin-analytics` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As an owner/finance user, I want `/analytics/category` to compare margin by category with filters, summaries, storage context, missing-COGS disclosure, and export, so that I can identify category-level profitability drivers.
- **Resolved owned surface:** `/analytics/category`; its `page.tsx`, `components/CategoryHelpSection.tsx`, exclusively consumed `src/components/custom/MarginByCategoryTable.tsx` and route-specific tests. Cross-route margin compositions and `ExportDialog` remain shared dependencies.
...
- **Exact branch:** `cdx/epic-170-story-5-category-margin-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-170-story-5-category-margin-shadcn" "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" main
...
- **Canonical allowance:** Category route files/tests and exclusive `MarginByCategoryTable`/tests only.
- **Resolved owned files/tree:** `/analytics/category`; its `page.tsx`, `components/CategoryHelpSection.tsx`, exclusively consumed `src/components/custom/MarginByCategoryTable.tsx` and route-specific tests. Cross-route margin compositions and `ExportDialog` remain shared dependencies.
...
- **Canonical prohibition:** FS, especially `analytics/shared/**`, ExportDialog, hooks/types/calculations, and brand route.
...
1. From `/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/category"
   ```
...
4. Capture deterministic baseline evidence for: SC plus unknown category, missing-COGS, negative margin, storage-comparison unavailable, and export lifecycle.
...
5. **Apply the route-specific data contract:** RTC; table caption names category margin analysis, primary column is category, numeric columns align with full RUB/percent precision, row actions/export scope remain reachable. No chart is introduced.
6. **Apply accessibility and responsive behavior:** AX; unknown categories use neutral labeled fallback, financial direction is non-color, and help/export are keyboard complete. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/category` and `STORY_ID=170.5`; include targeted `MarginByCategoryTable` tests.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn"
npx vitest run "src/app/(dashboard)/analytics/category"
npm run test:e2e -- --grep "Story 170.5-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" diff --check
```
...
- **State matrix:** SC plus unknown category, missing-COGS, negative margin, storage-comparison unavailable, and export lifecycle.
- **Route-specific table/chart contract:** RTC; table caption names category margin analysis, primary column is category, numeric columns align with full RUB/percent precision, row actions/export scope remain reachable. No chart is introduced.
...
- **Required Story evidence:** VE plus unknown category, missing COGS, negative value, and dense category-table evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" commit \
  -m "feat(analytics): migrate Story 170.5 route to shadcn" \
  -m "Story 170.5-FE — Migrate Category Margin Analytics. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn" push -u origin "cdx/epic-170-story-5-category-margin-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-170-story-5-category-margin-shadcn" \
  --title "[Story 170.5-FE] Migrate Category Margin Analytics" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-170-story-5-category-margin-shadcn"
...
Canonical cleanup requirement: CE, including proof shared margin compositions were untouched.

After the merge SHA is proven on `main`, execute in this order:

```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-170-story-5-category-margin-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-170-story-5-category-margin-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-170-story-5-category-margin-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-170-story-5-category-margin-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus unknown category, missing-COGS, negative margin, storage-comparison unavailable, and export lifecycle. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; table caption names category margin analysis, primary column is category, numeric columns align with full RUB/percent precision, row actions/export scope remain reachable. No chart is introduced. |
| Keyboard/focus/contrast regression | Apply AX; unknown categories use neutral labeled fallback, financial direction is non-color, and help/export are keyboard complete. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-170-story-5-category-margin-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-170-story-5-category-margin-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-170-5-category-margin-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
