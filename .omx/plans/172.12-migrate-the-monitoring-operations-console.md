...
- **Canonical shared dependency statement:** foundation/AppShell; Story 172.11 only for explicitly shared owner interfaces.
...
- tokens/primitives/AppShell, monitoring API/hooks/types, `/monitor` exclusive files.
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: `src/app/(dashboard)/monitoring/**` and exclusive monitoring components/tests.
...
| Shared ownership is crossed because foundation/AppShell; Story 172.11 only for explicitly shared owner interfaces. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
