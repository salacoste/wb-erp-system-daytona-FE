---
storyId: "169.1"
storyTitle: "Migrate Acquiring Report Index"
epic: "169-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-169-1-acquiring-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.1: Migrate Acquiring Report Index

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1691-migrate-acquiring-report-index` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As a finance/operations user, I want `/analytics/acquiring` to present acquiring totals, VAT anomalies, rate limits, and report rows consistently, so that I can identify the report and period needing investigation.
...
- **Exact branch:** `cdx/epic-169-story-1-acquiring-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-1-acquiring-shadcn" "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" main
...
- **Canonical allowance:** Only the owned files above and new route-focused tests/evidence fixtures colocated under the acquiring root.
- **Resolved owned files/tree:** `/analytics/acquiring`; `src/app/(dashboard)/analytics/acquiring/page.tsx`, root `components/AcquiringPageContent.tsx`, `AcquiringSummaryCards.tsx`, `AcquiringReportsTable.tsx`, `components/shared/AcquiringRateLimitBanner.tsx`, `components/shared/AnomalyVatIndicator.tsx`, and colocated tests. Nested `period/**` and `reports/**` are excluded. This Story owns the two acquiring-shared presentation components for Stories 169.2–169.3.
...
- **Canonical prohibition:** FS, plus nested `acquiring/period/**` and `acquiring/reports/**`.
...
1. From `/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/acquiring"
   ```
...
4. Capture deterministic baseline evidence for: SC plus rate-limited, VAT-anomaly, report-processing, and invalid/unknown report-status states.
...
5. **Apply the route-specific data contract:** RTC; reports table caption names acquiring reports, primary column is report period/identifier, financial totals use tabular RUB precision, anomaly and action columns stay reachable through bounded horizontal scroll or row detail. No chart is introduced.
6. **Apply accessibility and responsive behavior:** AX; anomaly and rate-limit meaning uses text/icon, report links name their target, and sortable headers expose direction. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/acquiring` and `STORY_ID=169.1`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn"
npx vitest run "src/app/(dashboard)/analytics/acquiring"
npm run test:e2e -- --grep "Story 169.1-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" diff --check
```
...
- **State matrix:** SC plus rate-limited, VAT-anomaly, report-processing, and invalid/unknown report-status states.
- **Route-specific table/chart contract:** RTC; reports table caption names acquiring reports, primary column is report period/identifier, financial totals use tabular RUB precision, anomaly and action columns stay reachable through bounded horizontal scroll or row detail. No chart is introduced.
...
- **Required Story evidence:** VE plus rate-limit banner, VAT anomaly, and reports-table narrow-width evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" commit \
  -m "feat(analytics): migrate Story 169.1 route to shadcn" \
  -m "Story 169.1-FE — Migrate Acquiring Report Index. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn" push -u origin "cdx/epic-169-story-1-acquiring-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-1-acquiring-shadcn" \
  --title "[Story 169.1-FE] Migrate Acquiring Report Index" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-1-acquiring-shadcn"
...
Canonical cleanup requirement: CE, including proof that nested acquiring detail routes were untouched.

After the merge SHA is proven on `main`, execute in this order:

```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-1-acquiring-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-1-acquiring-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-1-acquiring-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-1-acquiring-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus rate-limited, VAT-anomaly, report-processing, and invalid/unknown report-status states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; reports table caption names acquiring reports, primary column is report period/identifier, financial totals use tabular RUB precision, anomaly and action columns stay reachable through bounded horizontal scroll or row detail. No chart is introduced. |
| Keyboard/focus/contrast regression | Apply AX; anomaly and rate-limit meaning uses text/icon, report links name their target, and sortable headers expose direction. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-1-acquiring-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-1-acquiring-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-1-acquiring-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
