---
storyId: "174.5"
storyTitle: "Finalize Documentation and Repository Cleanup"
epic: "174-FE"
...
branch: cdx/epic-174-story-5-docs-cleanup
temporaryWorktree: /private/tmp/wb-repricer-fe-174-5-docs-cleanup
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 174.5 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 174.5 scope below takes precedence.
...
- **Required outcome:** Finalize canonical design-system and migration documentation, prove every ledger/evidence link and accepted exception is current, and remove only completed migration branches/worktrees so the repository hands off cleanly without changing runtime product behavior.
- **Canonical shared dependency statement:** Stories 174.1–174.4 complete with no unresolved migration blocker.
...
```mermaid
flowchart LR
  P1["Story 174.1"] --> S174_5["Story 174.5"]
  P2["Story 174.2"] --> S174_5["Story 174.5"]
  P3["Story 174.3"] --> S174_5["Story 174.5"]
  P4["Story 174.4"] --> S174_5["Story 174.5"]
```
...
- [ ] Verify the merge SHA for **Story 174.4** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `174.5` / `Finalize Documentation and Repository Cleanup` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 174.5 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 174.5. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-174-story-5-docs-cleanup`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-174-5-docs-cleanup`
...
test ! -e "/private/tmp/wb-repricer-fe-174-5-docs-cleanup"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-174-story-5-docs-cleanup" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-174-story-5-docs-cleanup" "/private/tmp/wb-repricer-fe-174-5-docs-cleanup" main
git -C "/private/tmp/wb-repricer-fe-174-5-docs-cleanup" status --short --branch
```
...
- documentation/tracking and safe Git cleanup for already merged migration resources.
- Canonical owned surface: canonical frontend/design-system docs, route ledger final status, delivery manifest, retrospective, cleanup report.
...
- runtime product behavior; unrelated branches/worktrees; production operations.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 174.5.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **all ledger rows `verified`; all exception records resolved or explicitly accepted by owner.**
...
1. Reconcile canonical frontend/design-system documentation with the merged token, primitive, composition, ownership, responsive, accessibility, and Story-delivery contracts.
2. Mark route-ledger rows verified only when their linked implementation, validation, visual/accessibility, review, merge, and cleanup evidence resolves.
3. Validate every documentation/evidence link, final delivery-manifest entry, retrospective item, and exception disposition.
4. Audit migration branches and worktrees against merged PRs. Remove only resources proven completed; preserve and report unrelated or unmerged work.
5. Delete completed remote branches, remove their explicit temporary worktrees, delete local branches, run `git worktree prune`, and attach final absence evidence.
6. Run final Story/route/evidence parity and documentation checks; block completion on an unresolved migration blocker or broken canonical reference.
7. Publish the cleanup report and verify the final diff contains documentation/tracking only, with no runtime product change.
...
npm run build
git -C "/private/tmp/wb-repricer-fe-174-5-docs-cleanup" diff --check
```
...
- `doc links resolve; Story/route/evidence parity passes; `git diff --check`; clean intended worktree state.`;
- canonical local-validation intent: documentation/parity checks plus final repository status/worktree/branch audit.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-174.5/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** final documentation records reusable contracts and remaining justified exceptions.
- **Story accessibility contract:** final report records achieved evidence and any explicitly accepted environment gaps.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 174.5.
...
```bash
git -C /private/tmp/wb-repricer-fe-174-5-docs-cleanup diff --check
git -C /private/tmp/wb-repricer-fe-174-5-docs-cleanup status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-174-5-docs-cleanup" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-174-5-docs-cleanup" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-174-5-docs-cleanup" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-174-5-docs-cleanup diff --cached --stat
git -C /private/tmp/wb-repricer-fe-174-5-docs-cleanup commit \
  -m "docs(migration): finalize documentation and repository cleanup" \
  -m "Story 174.5. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-174-5-docs-cleanup push -u origin cdx/epic-174-story-5-docs-cleanup
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-174-story-5-docs-cleanup" \
  --title "Story 174.5: Finalize Documentation and Repository Cleanup" \
  --body "Implements BMAD Story 174.5 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-174-story-5-docs-cleanup"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-174-story-5-docs-cleanup"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-174-5-docs-cleanup"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-174-story-5-docs-cleanup"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-174-story-5-docs-cleanup"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-174-story-5-docs-cleanup"
...
- `/private/tmp/wb-repricer-fe-174-5-docs-cleanup` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 174.5 worktree;
- local and remote branch queries return no `cdx/epic-174-story-5-docs-cleanup`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: all completed migration remote/local branches deleted, all temporary migration worktrees removed, final `git worktree list` and branch audit attached.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of complete product; reliable future maintenance and no stale implementation artifacts. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Stories 174.1–174.4 complete with no unresolved migration blocker. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: final documentation records reusable contracts and remaining justified exceptions. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: final report records achieved evidence and any explicitly accepted environment gaps. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: all ledger rows `verified`; all exception records resolved or explicitly accepted by owner. | Test each applicable fixture and block merge on an unhandled state. |
