---
storyId: "169.2"
storyTitle: "Migrate Acquiring Period Detail"
epic: "169-FE"
...
branch: cdx/epic-169-story-2-acquiring-period-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.2: Migrate Acquiring Period Detail

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1692-migrate-acquiring-period-detail` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As a finance user, I want `/analytics/acquiring/period` to explain one acquiring period and its reconciliation summary clearly, so that I can validate the period without losing report context.
- **Resolved owned surface:** `/analytics/acquiring/period`; `src/app/(dashboard)/analytics/acquiring/period/page.tsx`, `components/AcquiringPeriodDetailPage.tsx`, `components/AcquiringPeriodSummary.tsx`, and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-2-acquiring-period-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-2-acquiring-period-shadcn" "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" main
...
- **Canonical allowance:** Only the period route tree and its tests.
- **Resolved owned files/tree:** `/analytics/acquiring/period`; `src/app/(dashboard)/analytics/acquiring/period/page.tsx`, `components/AcquiringPeriodDetailPage.tsx`, `components/AcquiringPeriodSummary.tsx`, and colocated tests.
...
- **Canonical prohibition:** FS plus the acquiring index and `reports/[id]/**` trees.
...
1. From `/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/acquiring/period"
   ```
...
4. Capture deterministic baseline evidence for: SC plus invalid/missing period, rate-limit, and period-not-calculated states.
...
5. **Apply the route-specific data contract:** RTC; summary evidence keeps period, units, definition, and full precision visible; any detail collection uses period/item as primary identifier. No chart is added unless already owned by the route.
6. **Apply accessibility and responsive behavior:** AX; breadcrumbs/back action are named, period context is not tooltip-only, and anomalies are non-color. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/acquiring/period` and `STORY_ID=169.2`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn"
npx vitest run "src/app/(dashboard)/analytics/acquiring/period"
npm run test:e2e -- --grep "Story 169.2-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" diff --check
```
...
- **State matrix:** SC plus invalid/missing period, rate-limit, and period-not-calculated states.
- **Route-specific table/chart contract:** RTC; summary evidence keeps period, units, definition, and full precision visible; any detail collection uses period/item as primary identifier. No chart is added unless already owned by the route.
...
- **Required Story evidence:** VE plus valid/invalid period and full-precision summary evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" commit \
  -m "feat(analytics): migrate Story 169.2 route to shadcn" \
  -m "Story 169.2-FE — Migrate Acquiring Period Detail. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn" push -u origin "cdx/epic-169-story-2-acquiring-period-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-2-acquiring-period-shadcn" \
  --title "[Story 169.2-FE] Migrate Acquiring Period Detail" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-2-acquiring-period-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-2-acquiring-period-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-2-acquiring-period-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-2-acquiring-period-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-2-acquiring-period-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus invalid/missing period, rate-limit, and period-not-calculated states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; summary evidence keeps period, units, definition, and full precision visible; any detail collection uses period/item as primary identifier. No chart is added unless already owned by the route. |
| Keyboard/focus/contrast regression | Apply AX; breadcrumbs/back action are named, period context is not tooltip-only, and anomalies are non-color. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-2-acquiring-period-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-2-acquiring-period-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-2-acquiring-period-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
