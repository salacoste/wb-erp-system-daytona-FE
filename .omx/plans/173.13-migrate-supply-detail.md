---
storyId: "173.13"
storyTitle: "Migrate Supply Detail"
epic: "173-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-173-13-supply-detail
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.13 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 173.13 scope below takes precedence.
...
- **Required outcome:** Migrate Supply Detail. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 173.12 plus foundation/AppShell.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_13["Story 173.13"]
  P2["Story 167.1"] --> S173_13["Story 173.13"]
  P3["Story 173.12"] --> S173_13["Story 173.13"]
```
...
- [ ] Verify the merge SHA for **Story 173.12** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `173.13` / `Migrate Supply Detail` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.13 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.13. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-13-supply-detail`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-13-supply-detail`
...
test ! -e "/private/tmp/wb-repricer-fe-173-13-supply-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-13-supply-detail" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-13-supply-detail" "/private/tmp/wb-repricer-fe-173-13-supply-detail" main
git -C "/private/tmp/wb-repricer-fe-173-13-supply-detail" status --short --branch
```
...
- detail-owned files and route fixtures/tests.
- Canonical owned surface: route directory and detail-exclusive `src/components/custom/supplies/**` files/tests.
...
- supply shared owner, tokens/primitives/AppShell, API/hooks/types/lifecycle rules.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.13.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, not found, partial, lifecycle states, document pending/error, picker states, close pending/success/failure.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and detail-exclusive `src/components/custom/supplies/**` files/tests.
2. Deliver the user value and interaction surface exactly as recorded: dynamic supply detail, status stepper, orders table, documents, order-picker drawer, close dialog, sticker and acceptance-act flows.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, not found, partial, lifecycle states, document pending/error, picker states, close pending/success/failure.
6. Apply the Story contract for density and responsive behavior: status and primary action remain visible; orders table primary columns; Drawer/mobile detail contract.
7. Apply the Story accessibility contract: stepper text/current state, table semantics, Drawer/Dialog focus return, named document/actions, announcements.
...
- `npm test -- --run 'src/app/(dashboard)/supplies/[id]' src/components/custom/supplies`
- `npm run test:e2e -- e2e/supplies/supply-detail.spec.ts e2e/supplies/supply-lifecycle.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-13-supply-detail" diff --check
```
...
- `dynamic lifecycle/orders/documents/close regression and responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted supply detail tests/E2E plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.13/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** status and primary action remain visible; orders table primary columns; Drawer/mobile detail contract.
- **Story accessibility contract:** stepper text/current state, table semantics, Drawer/Dialog focus return, named document/actions, announcements.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.13.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-13-supply-detail diff --check
git -C /private/tmp/wb-repricer-fe-173-13-supply-detail status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-13-supply-detail" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-13-supply-detail" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-13-supply-detail" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-13-supply-detail diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-13-supply-detail commit \
  -m "feat(supplies): migrate supply detail" \
  -m "Story 173.13. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-13-supply-detail push -u origin cdx/epic-173-story-13-supply-detail
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-13-supply-detail" \
  --title "Story 173.13: Migrate Supply Detail" \
  --body "Implements BMAD Story 173.13 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-13-supply-detail"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-13-supply-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-13-supply-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-13-supply-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-13-supply-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-13-supply-detail"
...
- `/private/tmp/wb-repricer-fe-173-13-supply-detail` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.13 worktree;
- local and remote branch queries return no `cdx/epic-173-story-13-supply-detail`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: final Epic 173 branch/worktree cleanup and not-found proof.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of dynamic supply detail, status stepper, orders table, documents, order-picker drawer, close dialog, sticker and acceptance-act flows. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 173.12 plus foundation/AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: status and primary action remain visible; orders table primary columns; Drawer/mobile detail contract. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: stepper text/current state, table semantics, Drawer/Dialog focus return, named document/actions, announcements. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, not found, partial, lifecycle states, document pending/error, picker states, close pending/success/failure. | Test each applicable fixture and block merge on an unhandled state. |
