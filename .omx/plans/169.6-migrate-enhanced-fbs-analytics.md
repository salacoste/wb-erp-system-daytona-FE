---
storyId: "169.6"
storyTitle: "Migrate Enhanced FBS Analytics"
epic: "169-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.6: Migrate Enhanced FBS Analytics

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1696-migrate-enhanced-fbs-analytics` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an operations manager, I want `/analytics/fbs-enhanced` to present FBS orders, funnel, calculated metrics, regional evidence, and stock risk coherently, so that I can triage fulfillment issues without losing operational context.
- **Resolved owned surface:** `/analytics/fbs-enhanced`; its `page.tsx`, full `components/**` tree (page content, calculated metrics, order stats, funnel section/chart, regional section/tooltip, stock section), and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-6-fbs-enhanced-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-6-fbs-enhanced-shadcn" "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/fbs-enhanced/**`.
- **Resolved owned files/tree:** `/analytics/fbs-enhanced`; its `page.tsx`, full `components/**` tree (page content, calculated metrics, order stats, funnel section/chart, regional section/tooltip, stock section), and colocated tests.
...
- **Canonical prohibition:** FS and `/analytics/fbs-stock/**`.
...
1. From `/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/fbs-enhanced"
   ```
...
4. Capture deterministic baseline evidence for: SC plus per-section partial/error, funnel not-calculated, no regional data, and stock-risk states.
...
5. **Apply the route-specific data contract:** RTC; FBS funnel chart names stages/counts/conversion and exposes a data alternative; regional/stock collections prioritize region or SKU, operational status, primary count, and action.
6. **Apply accessibility and responsive behavior:** AX; funnel stage meaning and stock severity are non-color, tooltip content has non-hover access, and section failures have named regions. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/fbs-enhanced` and `STORY_ID=169.6`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn"
npx vitest run "src/app/(dashboard)/analytics/fbs-enhanced"
npm run test:e2e -- --grep "Story 169.6-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" diff --check
```
...
- **State matrix:** SC plus per-section partial/error, funnel not-calculated, no regional data, and stock-risk states.
- **Route-specific table/chart contract:** RTC; FBS funnel chart names stages/counts/conversion and exposes a data alternative; regional/stock collections prioritize region or SKU, operational status, primary count, and action.
...
- **Required Story evidence:** VE plus mixed partial-section, funnel alternative, and tablet operational-priority evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" commit \
  -m "feat(analytics): migrate Story 169.6 route to shadcn" \
  -m "Story 169.6-FE — Migrate Enhanced FBS Analytics. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn" push -u origin "cdx/epic-169-story-6-fbs-enhanced-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-6-fbs-enhanced-shadcn" \
  --title "[Story 169.6-FE] Migrate Enhanced FBS Analytics" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-6-fbs-enhanced-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-6-fbs-enhanced-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-6-fbs-enhanced-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-6-fbs-enhanced-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-6-fbs-enhanced-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus per-section partial/error, funnel not-calculated, no regional data, and stock-risk states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; FBS funnel chart names stages/counts/conversion and exposes a data alternative; regional/stock collections prioritize region or SKU, operational status, primary count, and action. |
| Keyboard/focus/contrast regression | Apply AX; funnel stage meaning and stock severity are non-color, tooltip content has non-hover access, and section failures have named regions. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-6-fbs-enhanced-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-6-fbs-enhanced-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-6-fbs-enhanced-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
