...
- **Unassigned shared boundary prerequisite:** before Story 169.11 can become `ready-for-dev`, an explicit owner Story must preserve an unknown return-reason discriminator and neutral Russian label through `src/lib/api/return-analytics-normalizer.ts` and `src/types/analytics-returns.ts`. Current shared behavior coerces every unknown category to `return_after_receipt`, which makes the canonical unknown-reason acceptance criterion impossible inside this route-only Story. This plan does not authorize that shared-file change.
...
flowchart TD
  ...
  S168_1 --> THIS
  UNKNOWN["Shared unknown-reason boundary prerequisite merged"] --> THIS

```

**Stop gate:** do not create the branch while the unknown-reason boundary prerequisite is unassigned or unmerged, if any prerequisite SHA is absent from current `main`, the Allowed Change Surface overlaps an active Story, or implementation would require a Forbidden Shared File. A route-only neutral color fallback does not satisfy the blocker because the current normalizer has already mislabeled the category as `return_after_receipt`.
