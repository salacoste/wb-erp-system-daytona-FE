---
storyId: "172.8"
storyTitle: "Migrate the COGS Price Calculator"
epic: "172-FE"
...
branch: cdx/epic-172-story-8-price-calculator
temporaryWorktree: /private/tmp/wb-repricer-fe-172-8-price-calculator
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.8 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.8 scope below takes precedence.

## Outcome and Requirements

- **Requirements:** FR12, FR27
- **Route/User Value:** price-calculator form, validation, assumptions, result, and reset/recalculation.
- **Required outcome:** Migrate the COGS Price Calculator. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** Story 172.5 formatting/presentation plus foundation/AppShell.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_8["Story 172.8"]
  P2["Story 167.1"] --> S172_8["Story 172.8"]
  P3["Story 172.5"] --> S172_8["Story 172.8"]
```
...
- [ ] Confirm Story ID/title parity: `172.8` / `Migrate the COGS Price Calculator` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.8 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.8. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-8-price-calculator`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-8-price-calculator`
...
test ! -e "/private/tmp/wb-repricer-fe-172-8-price-calculator"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-8-price-calculator" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-8-price-calculator" "/private/tmp/wb-repricer-fe-172-8-price-calculator" main
git -C "/private/tmp/wb-repricer-fe-172-8-price-calculator" status --short --branch
```
...
- Owned Surface and calculator tests/fixtures.
- Canonical owned surface: route directory and exclusive price-calculator components/tests.
...
- tokens/primitives/AppShell, calculation helpers/business formulas/API contracts unless explicitly proven presentation-only.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.8.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **pristine, valid input, field error, unusual warning, calculating, result, zero/negative result, failure.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and exclusive price-calculator components/tests.
2. Deliver the user value and interaction surface exactly as recorded: price-calculator form, validation, assumptions, result, and reset/recalculation.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: pristine, valid input, field error, unusual warning, calculating, result, zero/negative result, failure.
6. Apply the Story contract for density and responsive behavior: readable focused form and result on mobile/desktop; preserve any existing breakdown table contract.
7. Apply the Story accessibility contract: visible units, associated errors, logical result heading, announced completion, non-color profit direction.
...
- `npm run test:e2e -- e2e/price-calculator-visual.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-8-price-calculator" diff --check
```
...
- `formula-boundary regression fixtures and form/result screenshots.`;
- canonical local-validation intent: targeted calculator tests plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.8/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** readable focused form and result on mobile/desktop; preserve any existing breakdown table contract.
- **Story accessibility contract:** visible units, associated errors, logical result heading, announced completion, non-color profit direction.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.8.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-8-price-calculator diff --check
git -C /private/tmp/wb-repricer-fe-172-8-price-calculator status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-8-price-calculator" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-8-price-calculator" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-8-price-calculator" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-8-price-calculator diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-8-price-calculator commit \
  -m "feat(cogs): migrate the cogs price calculator" \
  -m "Story 172.8. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-8-price-calculator push -u origin cdx/epic-172-story-8-price-calculator
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-8-price-calculator" \
  --title "Story 172.8: Migrate the COGS Price Calculator" \
  --body "Implements BMAD Story 172.8 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-8-price-calculator"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-8-price-calculator"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-8-price-calculator"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-8-price-calculator"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-8-price-calculator"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-8-price-calculator"
...
- `/private/tmp/wb-repricer-fe-172-8-price-calculator` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.8 worktree;
- local and remote branch queries return no `cdx/epic-172-story-8-price-calculator`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: formula-preservation evidence plus branch/worktree cleanup.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of price-calculator form, validation, assumptions, result, and reset/recalculation. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 172.5 formatting/presentation plus foundation/AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: readable focused form and result on mobile/desktop; preserve any existing breakdown table contract. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: visible units, associated errors, logical result heading, announced completion, non-color profit direction. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: pristine, valid input, field error, unusual warning, calculating, result, zero/negative result, failure. | Test each applicable fixture and block merge on an unhandled state. |
