---
storyId: "169.5"
storyTitle: "Migrate Buyout Reconciliation"
epic: "169-FE"
...
temporaryWorktree: /private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.5: Migrate Buyout Reconciliation

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1695-migrate-buyout-reconciliation` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As a finance/operations user, I want `/analytics/buyout-reconciliation` to expose reconciliation controls, anomalies, and row outcomes clearly, so that I can distinguish matched, mismatched, pending, and failed reconciliation evidence.
- **Resolved owned surface:** `/analytics/buyout-reconciliation`; its `page.tsx`, `components/BuyoutReconciliationPageContent.tsx`, `ReconciliationControls.tsx`, `ReconciliationStateMachine.tsx`, `ReconciliationTable.tsx`, `AnomalyIndicator.tsx`, and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-5-buyout-reconciliation-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-5-buyout-reconciliation-shadcn" "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/buyout-reconciliation/**`.
- **Resolved owned files/tree:** `/analytics/buyout-reconciliation`; its `page.tsx`, `components/BuyoutReconciliationPageContent.tsx`, `ReconciliationControls.tsx`, `ReconciliationStateMachine.tsx`, `ReconciliationTable.tsx`, `AnomalyIndicator.tsx`, and colocated tests.
...
- **Canonical prohibition:** FS and `/analytics/buyout/**`.
...
1. From `/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/buyout-reconciliation"
   ```
...
4. Capture deterministic baseline evidence for: SC plus not-started, processing, matched, mismatched, partial-source, and state-machine failure states.
...
5. **Apply the route-specific data contract:** RTC; reconciliation table caption names the period, primary column is SKU/entity, compared values and delta are aligned with full precision, anomaly and action columns remain reachable; no new chart.
6. **Apply accessibility and responsive behavior:** AX; state machine transitions use measured live announcements, anomalies are text/icon based, and controls expose current value. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/buyout-reconciliation` and `STORY_ID=169.5`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn"
npx vitest run "src/app/(dashboard)/analytics/buyout-reconciliation"
npm run test:e2e -- --grep "Story 169.5-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" diff --check
```
...
- **State matrix:** SC plus not-started, processing, matched, mismatched, partial-source, and state-machine failure states.
- **Route-specific table/chart contract:** RTC; reconciliation table caption names the period, primary column is SKU/entity, compared values and delta are aligned with full precision, anomaly and action columns remain reachable; no new chart.
...
- **Required Story evidence:** VE plus matched/mismatched/processing/partial-source evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" commit \
  -m "feat(analytics): migrate Story 169.5 route to shadcn" \
  -m "Story 169.5-FE — Migrate Buyout Reconciliation. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn" push -u origin "cdx/epic-169-story-5-buyout-reconciliation-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-5-buyout-reconciliation-shadcn" \
  --title "[Story 169.5-FE] Migrate Buyout Reconciliation" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-5-buyout-reconciliation-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-5-buyout-reconciliation-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-5-buyout-reconciliation-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-5-buyout-reconciliation-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-5-buyout-reconciliation-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus not-started, processing, matched, mismatched, partial-source, and state-machine failure states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; reconciliation table caption names the period, primary column is SKU/entity, compared values and delta are aligned with full precision, anomaly and action columns remain reachable; no new chart. |
| Keyboard/focus/contrast regression | Apply AX; state machine transitions use measured live announcements, anomalies are text/icon based, and controls expose current value. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-5-buyout-reconciliation-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-5-buyout-reconciliation-shadcn`.
