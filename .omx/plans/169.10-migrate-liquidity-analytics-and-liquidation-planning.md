---
storyId: "169.10"
storyTitle: "Migrate Liquidity Analytics and Liquidation Planning"
epic: "169-FE"
...
branch: cdx/epic-169-story-10-liquidity-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-10-liquidity-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.10: Migrate Liquidity Analytics and Liquidation Planning

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-16910-migrate-liquidity-analytics-and-liquidation-planning` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an owner/operations user, I want `/analytics/liquidity` to connect inventory liquidity distribution, benchmarks, trends, SKU detail, and liquidation scenarios, so that I can identify tied-up capital and evaluate an action safely.
- **Resolved owned surface:** `/analytics/liquidity`; its `page.tsx`, full `components/**` tree (header, cards, distribution/trend charts, summary/benchmarks, table/header/rows/expanded row, loading/empty, planner modal and scenario cards), and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-10-liquidity-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-10-liquidity-shadcn" "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/liquidity/**`.
- **Resolved owned files/tree:** `/analytics/liquidity`; its `page.tsx`, full `components/**` tree (header, cards, distribution/trend charts, summary/benchmarks, table/header/rows/expanded row, loading/empty, planner modal and scenario cards), and colocated tests.
...
1. From `/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/liquidity"
   ```
...
4. Capture deterministic baseline evidence for: SC plus benchmark unavailable, expanded-row loading/error, planner validating/preview/submitting/success/failure, and partial trend states.
...
5. **Apply the route-specific data contract:** RTC; liquidity table primary column is product/SKU with category/status, capital/stock metrics, expansion and action reachable; distribution/trend charts expose units, category/period series, summaries/data alternatives; planner uses focused responsive form/dialog, not a clipped desktop fragment.
6. **Apply accessibility and responsive behavior:** AX; scenario form has persistent labels/error summary, overlays restore focus, and liquidity status/direction is non-color. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/liquidity` and `STORY_ID=169.10`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn"
npx vitest run "src/app/(dashboard)/analytics/liquidity"
npm run test:e2e -- --grep "Story 169.10-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" diff --check
```
...
- **State matrix:** SC plus benchmark unavailable, expanded-row loading/error, planner validating/preview/submitting/success/failure, and partial trend states.
- **Route-specific table/chart contract:** RTC; liquidity table primary column is product/SKU with category/status, capital/stock metrics, expansion and action reachable; distribution/trend charts expose units, category/period series, summaries/data alternatives; planner uses focused responsive form/dialog, not a clipped desktop fragment.
...
- **Required Story evidence:** VE plus expanded row, partial chart, planner validation/preview/result, and large-negative-value evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" commit \
  -m "feat(analytics): migrate Story 169.10 route to shadcn" \
  -m "Story 169.10-FE — Migrate Liquidity Analytics and Liquidation Planning. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn" push -u origin "cdx/epic-169-story-10-liquidity-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-10-liquidity-shadcn" \
  --title "[Story 169.10-FE] Migrate Liquidity Analytics and Liquidation Planning" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-10-liquidity-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-10-liquidity-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-10-liquidity-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-10-liquidity-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-10-liquidity-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus benchmark unavailable, expanded-row loading/error, planner validating/preview/submitting/success/failure, and partial trend states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; liquidity table primary column is product/SKU with category/status, capital/stock metrics, expansion and action reachable; distribution/trend charts expose units, category/period series, summaries/data alternatives; planner uses focused responsive form/dialog, not a clipped desktop fragment. |
| Keyboard/focus/contrast regression | Apply AX; scenario form has persistent labels/error summary, overlays restore focus, and liquidity status/direction is non-color. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-10-liquidity-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-10-liquidity-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-10-liquidity-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
