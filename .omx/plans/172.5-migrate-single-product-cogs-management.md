---
storyId: "172.5"
storyTitle: "Migrate Single-Product COGS Management"
epic: "172-FE"
...
branch: cdx/epic-172-story-5-cogs
temporaryWorktree: /private/tmp/wb-repricer-fe-172-5-cogs
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 172.5 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
This plan inherits the master protocol, but the narrower Story 172.5 scope below takes precedence.
...
- **Required outcome:** Migrate Single-Product COGS Management. Migrate the complete Story-owned render tree—not only a route entry—to approved merged shadcn primitives and semantic product compositions while preserving all current backend, query, calculation, formatting, authorization, navigation, URL/search, cache, and mutation semantics.
- **Canonical shared dependency statement:** foundation/AppShell; this Story is owner for shared single-COGS presentation used by 172.6–172.8 where applicable.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S172_5["Story 172.5"]
  P2["Story 167.1"] --> S172_5["Story 172.5"]
```
...
- [ ] Confirm Story ID/title parity: `172.5` / `Migrate Single-Product COGS Management` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 172.5 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 172.5. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-172-story-5-cogs`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-172-5-cogs`
...
test ! -e "/private/tmp/wb-repricer-fe-172-5-cogs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-172-story-5-cogs" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-172-story-5-cogs" "/private/tmp/wb-repricer-fe-172-5-cogs" main
git -C "/private/tmp/wb-repricer-fe-172-5-cogs" status --short --branch
```
...
- Owned Surface, exclusive COGS custom components, tests and fixtures.
- Canonical owned surface: route directory; `ProductList*`, `SingleCogsForm`, `CogsEditDialog`, `CogsDeleteDialog`, COGS/margin cells and exclusive tests.
...
- tokens/primitives/AppShell, COGS API/hooks/types/calculations, bulk-only components.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 172.5.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, empty, filtered empty, missing COGS, valid zero, edit/delete pending, save failure, saved-margin-pending, margin ready/error.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory; `ProductList*`, `SingleCogsForm`, `CogsEditDialog`, `CogsDeleteDialog`, COGS/margin cells and exclusive tests.
2. Deliver the user value and interaction surface exactly as recorded: `/cogs`; product discovery, single COGS add/edit/delete, coverage state, and margin feedback.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, empty, filtered empty, missing COGS, valid zero, edit/delete pending, save failure, saved-margin-pending, margin ready/error.
6. Apply the Story contract for density and responsive behavior: product identity/status/cost/margin/action primary columns; explicit narrow row-detail strategy and pagination preservation.
7. Apply the Story accessibility contract: product-specific action names, currency/unit labels, numeric validation, warning confirmation, focus return, non-color margin direction.
...
- `npm test -- --run 'src/app/(dashboard)/cogs' src/components/custom/single-cogs src/components/custom/cogs-edit-helpers.test.ts`
- `npm run test:e2e -- e2e/cogs-assignment.spec.ts e2e/cogs-pages.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-172-5-cogs" diff --check
```
...
- `single-COGS regression tests, keyboard flow, and state/responsive/theme screenshots.`;
- canonical local-validation intent: targeted COGS tests and Universal Local Validation Contract.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-172.5/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** product identity/status/cost/margin/action primary columns; explicit narrow row-detail strategy and pagination preservation.
- **Story accessibility contract:** product-specific action names, currency/unit labels, numeric validation, warning confirmation, focus return, non-color margin direction.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 172.5.
...
```bash
git -C /private/tmp/wb-repricer-fe-172-5-cogs diff --check
git -C /private/tmp/wb-repricer-fe-172-5-cogs status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-5-cogs" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-172-5-cogs" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-172-5-cogs" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-172-5-cogs diff --cached --stat
git -C /private/tmp/wb-repricer-fe-172-5-cogs commit \
  -m "feat(cogs): migrate single product cogs management" \
  -m "Story 172.5. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-172-5-cogs push -u origin cdx/epic-172-story-5-cogs
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-172-story-5-cogs" \
  --title "Story 172.5: Migrate Single-Product COGS Management" \
  --body "Implements BMAD Story 172.5 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-172-story-5-cogs"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-172-story-5-cogs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-172-5-cogs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-172-story-5-cogs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-172-story-5-cogs"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-172-story-5-cogs"
...
- `/private/tmp/wb-repricer-fe-172-5-cogs` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 172.5 worktree;
- local and remote branch queries return no `cdx/epic-172-story-5-cogs`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: detailed merge and mandatory cleanup evidence.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of `/cogs`; product discovery, single COGS add/edit/delete, coverage state, and margin feedback. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because foundation/AppShell; this Story is owner for shared single-COGS presentation used by 172.6–172.8 where applicable. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: product identity/status/cost/margin/action primary columns; explicit narrow row-detail strategy and pagination preservation. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: product-specific action names, currency/unit labels, numeric validation, warning confirmation, focus return, non-color margin direction. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, empty, filtered empty, missing COGS, valid zero, edit/delete pending, save failure, saved-margin-pending, margin ready/error. | Test each applicable fixture and block merge on an unhandled state. |
