---
storyId: "171.1"
storyTitle: "Migrate AI Anomaly Triage"
epic: "171-FE"
...
branch: cdx/epic-171-story-1-ai-anomalies-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn
productionScope: forbidden
---

# OMX Plan — Story 171.1: Migrate AI Anomaly Triage

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1711-migrate-ai-anomaly-triage` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As an authorized AI administrator, I want `/analytics/ai-admin/anomalies` to present anomaly identity, severity, evidence, status, and resolution safely, so that I can triage and resolve anomalies with an auditable outcome.
- **Resolved owned surface:** `/analytics/ai-admin/anomalies`; its `page.tsx`, `components/AnomaliesList.tsx`, `ResolveAnomalyDialog.tsx`, `anomalies-helpers.ts`, and colocated tests.
...
- **Exact branch:** `cdx/epic-171-story-1-ai-anomalies-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-171-story-1-ai-anomalies-shadcn" "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/ai-admin/anomalies/**`.
- **Resolved owned files/tree:** `/analytics/ai-admin/anomalies`; its `page.tsx`, `components/AnomaliesList.tsx`, `ResolveAnomalyDialog.tsx`, `anomalies-helpers.ts`, and colocated tests.
...
- **Canonical prohibition:** FS and sibling `ai-admin/models`/`preferences` trees.
...
1. From `/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/ai-admin/anomalies"
   ```
...
4. Capture deterministic baseline evidence for: SC plus unknown anomaly type, already-resolved/conflict, resolve validating/submitting/success/failure, and restricted administrator states.
...
5. **Apply the route-specific data contract:** RTC; anomaly list/table caption names AI anomalies, primary column is anomaly/entity identifier, severity/status/timestamp/action remain reachable; mobile may use explicit detail/resolve transition. No chart is introduced.
6. **Apply accessibility and responsive behavior:** AX; severity/status use text/icon/order, repeated resolve controls name the anomaly, dialog has error summary and correct focus return, progress is announced once. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/ai-admin/anomalies` and `STORY_ID=171.1`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn"
npx vitest run "src/app/(dashboard)/analytics/ai-admin/anomalies"
npm run test:e2e -- --grep "Story 171.1-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" diff --check
```
...
- **State matrix:** SC plus unknown anomaly type, already-resolved/conflict, resolve validating/submitting/success/failure, and restricted administrator states.
- **Route-specific table/chart contract:** RTC; anomaly list/table caption names AI anomalies, primary column is anomaly/entity identifier, severity/status/timestamp/action remain reachable; mobile may use explicit detail/resolve transition. No chart is introduced.
...
- **Required Story evidence:** VE plus restricted, unknown-status, already-resolved, and full resolve lifecycle evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" commit \
  -m "feat(analytics): migrate Story 171.1 route to shadcn" \
  -m "Story 171.1-FE — Migrate AI Anomaly Triage. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn" push -u origin "cdx/epic-171-story-1-ai-anomalies-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-171-story-1-ai-anomalies-shadcn" \
  --title "[Story 171.1-FE] Migrate AI Anomaly Triage" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-171-story-1-ai-anomalies-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-171-story-1-ai-anomalies-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-171-story-1-ai-anomalies-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-171-story-1-ai-anomalies-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-171-story-1-ai-anomalies-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus unknown anomaly type, already-resolved/conflict, resolve validating/submitting/success/failure, and restricted administrator states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; anomaly list/table caption names AI anomalies, primary column is anomaly/entity identifier, severity/status/timestamp/action remain reachable; mobile may use explicit detail/resolve transition. No chart is introduced. |
| Keyboard/focus/contrast regression | Apply AX; severity/status use text/icon/order, repeated resolve controls name the anomaly, dialog has error summary and correct focus return, progress is announced once. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-171-story-1-ai-anomalies-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-171-story-1-ai-anomalies-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-171-1-ai-anomalies-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
