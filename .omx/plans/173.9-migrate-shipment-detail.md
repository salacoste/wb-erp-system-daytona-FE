---
storyId: "173.9"
storyTitle: "Migrate Shipment Detail"
epic: "173-FE"
...
branch: cdx/epic-173-story-9-shipment-detail
temporaryWorktree: /private/tmp/wb-repricer-fe-173-9-shipment-detail
...
1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md` — canonical Story 173.9 ID/title, requirements, ownership, states, evidence, and acceptance criteria.
...
```mermaid
flowchart LR
  P1["Epic 166-FE"] --> S173_9["Story 173.9"]
  P2["Story 167.1"] --> S173_9["Story 173.9"]
  P3["Story 173.8"] --> S173_9["Story 173.9"]
```
...
- [ ] Verify the merge SHA for **Story 173.8** is reachable from current local `main`; record that SHA in the Story delivery record.
- [ ] Confirm Story ID/title parity: `173.9` / `Migrate Shipment Detail` in BMAD, this plan, the branch delivery record, and PR.
- [ ] Record current `main` base SHA after prerequisites merge; do not base this Story on another unmerged feature branch.
- [ ] Inventory every consumer of each proposed shared component. Edit a multi-route component only when Story 173.9 is its explicit owner; otherwise stop that edit and route it to the named prerequisite owner.
- [ ] Confirm the route-ledger row(s), if any, are owned exactly once by Story 173.9. Epic 174 audit Stories verify rather than take silent ownership of route implementation.

## Exact Branch and Isolated Worktree

- **Branch:** `cdx/epic-173-story-9-shipment-detail`
- **Temporary worktree:** `/private/tmp/wb-repricer-fe-173-9-shipment-detail`
...
test ! -e "/private/tmp/wb-repricer-fe-173-9-shipment-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" show-ref --verify --quiet "refs/heads/cdx/epic-173-story-9-shipment-detail" && exit 1 || true
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-173-story-9-shipment-detail" "/private/tmp/wb-repricer-fe-173-9-shipment-detail" main
git -C "/private/tmp/wb-repricer-fe-173-9-shipment-detail" status --short --branch
```
...
- detail-owned files and fixtures/tests.
- Canonical owned surface: route directory and detail-exclusive files under `src/components/custom/shipments/**`, plus tests.
...
- shared list owner, tokens/primitives/AppShell, shipment API/hooks/types/calculations.
...
- If a forbidden shared edit becomes necessary, stop that edit and escalate to the orchestrator/owner Story; do not absorb it into Story 173.9.

## Behavior Lock Before Presentation Changes

Canonical state matrix: **loading, not found, partial calculation, validation warning/error, action pending/success/failure, completed lifecycle.**
...
1. Work only inside the explicit allowed manifest and migrate the complete owned surface: route directory and detail-exclusive files under `src/components/custom/shipments/**`, plus tests.
2. Deliver the user value and interaction surface exactly as recorded: dynamic detail header, results, pallet accordion, validation panels, actions and state.
...
5. Implement every applicable canonical state without hiding meaning behind color, hover, truncation, or viewport width: loading, not found, partial calculation, validation warning/error, action pending/success/failure, completed lifecycle.
6. Apply the Story contract for density and responsive behavior: contextual detail; pallet accordion/table primary fields; mobile detail remains navigable.
7. Apply the Story accessibility contract: entity heading, Accordion semantics, validation summary, action confirmation/focus, status announcement.
...
- `npm test -- --run 'src/app/(dashboard)/shipments/[id]' src/components/custom/shipments`
- `npm run test:e2e -- e2e/shipments/shipments-detail.spec.ts e2e/shipments/shipments-lifecycle.spec.ts`
...
npm run build
git -C "/private/tmp/wb-repricer-fe-173-9-shipment-detail" diff --check
```
...
- `dynamic detail/calculation/lifecycle regressions and responsive/theme/state screenshots.`;
- canonical local-validation intent: targeted shipment detail tests/E2E plus universal checks.
...
- Capture Story-specific evidence under a unique `test-results/shadcn/story-173.9/` run directory or the repository's established equivalent.
...
- **Story responsive/data contract:** contextual detail; pallet accordion/table primary fields; mobile detail remains navigable.
- **Story accessibility contract:** entity heading, Accordion semantics, validation summary, action confirmation/focus, status announcement.
...
After the author completes implementation and validation, assign a reviewer who did not author Story 173.9.
...
```bash
git -C /private/tmp/wb-repricer-fe-173-9-shipment-detail diff --check
git -C /private/tmp/wb-repricer-fe-173-9-shipment-detail status --short
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-9-shipment-detail" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-173-9-shipment-detail" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-173-9-shipment-detail" add -- $EXPLICIT_ALLOWED_FILES_AND_DIRECT_TESTS
git -C /private/tmp/wb-repricer-fe-173-9-shipment-detail diff --cached --stat
git -C /private/tmp/wb-repricer-fe-173-9-shipment-detail commit \
  -m "feat(shipments): migrate shipment detail" \
  -m "Story 173.9. Migrate only the canonical owned surface, preserve established contracts and states, and record targeted, local, visual, accessibility, and independent-review evidence."
git -C /private/tmp/wb-repricer-fe-173-9-shipment-detail push -u origin cdx/epic-173-story-9-shipment-detail
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "cdx/epic-173-story-9-shipment-detail" \
  --title "Story 173.9: Migrate Shipment Detail" \
  --body "Implements BMAD Story 173.9 within its declared owner surface. Includes prerequisite/base SHA, behavior-lock evidence, exact validation results, visual/responsive/accessibility proof, independent-review disposition, risks, and cleanup checklist. No production or backend-contract work.")"
test -n "$STORY_PR_URL"
...
```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-173-story-9-shipment-detail"
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-173-story-9-shipment-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-173-9-shipment-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-173-story-9-shipment-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree list
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-173-story-9-shipment-detail"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-173-story-9-shipment-detail"
...
- `/private/tmp/wb-repricer-fe-173-9-shipment-detail` was removed;
- `git worktree prune` ran;
- `git worktree list` has no Story 173.9 worktree;
- local and remote branch queries return no `cdx/epic-173-story-9-shipment-detail`;
- intended primary-checkout status is recorded; and
- canonical cleanup intent is satisfied: calculation preservation and cleanup record.
...
| --- | --- |
| Presentation work changes the established meaning or behavior of dynamic detail header, results, pallet accordion, validation panels, actions and state. | Behavior-lock the canonical states and request/query/calculation/navigation/mutation semantics before editing; rerun targeted proof after the final diff. |
| Shared ownership is crossed because Story 173.8 plus foundation/AppShell. | Record prerequisite SHAs and a consumer/file manifest; reject forbidden edits and route shared needs to their owner. |
| Responsive or dense-data treatment loses evidence: contextual detail; pallet accordion/table primary fields; mobile detail remains navigable. | Capture the full width/theme matrix and prove primary identity, precision, actions, and alternatives remain available. |
| Keyboard, focus, status, or non-color meaning regresses: entity heading, Accordion semantics, validation summary, action confirmation/focus, status announcement. | Require manual keyboard/focus notes, semantic assertions, axe output, contrast evidence, and independent review. |
| Async or edge states are visually omitted: loading, not found, partial calculation, validation warning/error, action pending/success/failure, completed lifecycle. | Test each applicable fixture and block merge on an unhandled state. |
