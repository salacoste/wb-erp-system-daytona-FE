...
branch: cdx/epic-169-story-12-storage-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-12-storage-shadcn
...
- **Exact branch:** `cdx/epic-169-story-12-storage-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-12-storage-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-12-storage-shadcn" "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" main
...
1. From `/private/tmp/wb-repricer-fe-169-12-storage-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
...
4. Capture deterministic baseline evidence for: SC plus week-filter mismatch, alert, import validation/uploading/processing/partial/success/failure, and per-section error states.
...
5. **Apply the route-specific data contract:** RTC; SKU table primary column is product/SKU, storage cost/volume/days align with full precision, warehouse badges/actions stay reachable; trend chart exposes week/units/series and data alternative; import dialog uses focusable error/result summary.
6. **Apply accessibility and responsive behavior:** AX; import progress/result uses bounded live announcements, badges/alerts are non-color, dialog focus returns to import trigger. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
```bash
cd "/private/tmp/wb-repricer-fe-169-12-storage-shadcn"
npx vitest run "src/app/(dashboard)/analytics/storage"
...
npm run build
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" diff --check
```
...
- **State matrix:** SC plus week-filter mismatch, alert, import validation/uploading/processing/partial/success/failure, and per-section error states.
...
- **Required Story evidence:** VE plus alert, filtered-empty, partial section, and full import lifecycle evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" commit \
  -m "feat(analytics): migrate Story 169.12 route to shadcn" \
  -m "Story 169.12-FE — Migrate Storage Analytics and Paid-Storage Import. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-12-storage-shadcn" push -u origin "cdx/epic-169-story-12-storage-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-12-storage-shadcn" \
  --title "[Story 169.12-FE] Migrate Storage Analytics and Paid-Storage Import" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-12-storage-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-12-storage-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-12-storage-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-12-storage-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-12-storage-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-12-storage-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-12-storage-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus week-filter mismatch, alert, import validation/uploading/processing/partial/success/failure, and per-section error states. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-12-storage-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-12-storage-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-12-storage-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-12-storage-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
