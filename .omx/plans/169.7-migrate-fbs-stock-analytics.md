---
storyId: "169.7"
storyTitle: "Migrate FBS Stock Analytics"
epic: "169-FE"
...
branch: cdx/epic-169-story-7-fbs-stock-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn
productionScope: forbidden
---

# OMX Plan — Story 169.7: Migrate FBS Stock Analytics

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1697-migrate-fbs-stock-analytics` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Outcome/user value:** As an operations manager, I want `/analytics/fbs-stock` to compare stock by group, region, and size with a trustworthy export, so that I can find availability gaps and hand off corrective work.
- **Resolved owned surface:** `/analytics/fbs-stock`; its `page.tsx`, full `components/**` tree (`FbsStockPageContent`, groups/regions/sizes sections, export button), and colocated tests.
...
- **Exact branch:** `cdx/epic-169-story-7-fbs-stock-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-169-story-7-fbs-stock-shadcn" "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/fbs-stock/**`.
- **Resolved owned files/tree:** `/analytics/fbs-stock`; its `page.tsx`, full `components/**` tree (`FbsStockPageContent`, groups/regions/sizes sections, export button), and colocated tests.
...
- **Canonical prohibition:** FS and `/analytics/fbs-enhanced/**`.
...
1. From `/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/fbs-stock"
   ```
...
4. Capture deterministic baseline evidence for: SC plus valid zero-stock, missing availability, expanded group, and export queued/ready/failed states.
...
5. **Apply the route-specific data contract:** RTC; each collection has an accessible name and primary identifier (group/region/size), quantities align with units, expansion is programmatic, and narrow widths use deliberate disclosure or bounded scroll. No chart is introduced.
6. **Apply accessibility and responsive behavior:** AX; group expansion exposes state, export lifecycle is announced once, and availability is non-color. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/fbs-stock` and `STORY_ID=169.7`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn"
npx vitest run "src/app/(dashboard)/analytics/fbs-stock"
npm run test:e2e -- --grep "Story 169.7-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" diff --check
```
...
- **State matrix:** SC plus valid zero-stock, missing availability, expanded group, and export queued/ready/failed states.
- **Route-specific table/chart contract:** RTC; each collection has an accessible name and primary identifier (group/region/size), quantities align with units, expansion is programmatic, and narrow widths use deliberate disclosure or bounded scroll. No chart is introduced.
- **Accessibility contract:** AX; group expansion exposes state, export lifecycle is announced once, and availability is non-color.
- **Required Story evidence:** VE plus zero/missing stock, expanded group, and export lifecycle evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" commit \
  -m "feat(analytics): migrate Story 169.7 route to shadcn" \
  -m "Story 169.7-FE — Migrate FBS Stock Analytics. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn" push -u origin "cdx/epic-169-story-7-fbs-stock-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-169-story-7-fbs-stock-shadcn" \
  --title "[Story 169.7-FE] Migrate FBS Stock Analytics" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-169-story-7-fbs-stock-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-169-story-7-fbs-stock-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-169-story-7-fbs-stock-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-169-story-7-fbs-stock-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-169-story-7-fbs-stock-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus valid zero-stock, missing availability, expanded group, and export queued/ready/failed states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; each collection has an accessible name and primary identifier (group/region/size), quantities align with units, expansion is programmatic, and narrow widths use deliberate disclosure or bounded scroll. No chart is introduced. |
| Keyboard/focus/contrast regression | Apply AX; group expansion exposes state, export lifecycle is announced once, and availability is non-color. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-169-story-7-fbs-stock-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-169-story-7-fbs-stock-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-169-7-fbs-stock-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
