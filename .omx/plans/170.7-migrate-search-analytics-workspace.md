---
storyId: "170.7"
storyTitle: "Migrate Search Analytics Workspace"
epic: "170-FE"
...
branch: cdx/epic-170-story-7-search-analytics-shadcn
temporaryWorktree: /private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn
productionScope: forbidden
---

# OMX Plan — Story 170.7: Migrate Search Analytics Workspace

## Authoritative references

1. `_bmad-output/planning-artifacts/epics-166-174-fe-shadcn-migration.md#story-1707-migrate-search-analytics-workspace` — canonical ID, exact title, requirements, route value, owned/allowed/forbidden surfaces, BDD acceptance criteria, branch, validation, and cleanup contract.
...
- **Requirements:** FR27.
- **Outcome/user value:** As a marketing user, I want `/analytics/search` to analyze by query, product, orders, and position trends through consistent tabs, filters, charts, and tables, so that I can trace search demand and visibility to affected products and orders.
- **Resolved owned surface:** `/analytics/search`; its `page.tsx`, complete `components/**` tree (page content, product combobox, query/product/orders/trends tabs, summary/share/cards, order/position charts, query/product/orders/movers tables, seller badge, sort button, chart shell/helpers), and colocated tests.
...
- **Exact branch:** `cdx/epic-170-story-7-search-analytics-shadcn`
- **Unique temporary worktree:** `/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn`
...
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree add -b "cdx/epic-170-story-7-search-analytics-shadcn" "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" main
...
- **Canonical allowance:** Only `src/app/(dashboard)/analytics/search/**`.
- **Resolved owned files/tree:** `/analytics/search`; its `page.tsx`, complete `components/**` tree (page content, product combobox, query/product/orders/trends tabs, summary/share/cards, order/position charts, query/product/orders/movers tables, seller badge, sort button, chart shell/helpers), and colocated tests.
...
1. From `/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn`, inventory the complete owned render tree, imports, current tests, raw controls, hardcoded/light-only styling, table/chart implementations, overlays, and route-state ownership.
2. Run the smallest current targeted tests before source edits:
   ```bash
   npx vitest run "src/app/(dashboard)/analytics/search"
   ```
...
4. Capture deterministic baseline evidence for: SC plus no-selection, invalid/deep-linked tab, per-tab partial/error, unknown seller, and results-updating states.
...
5. **Apply the route-specific data contract:** RTC; each table has a route-specific caption and primary identifier (query, product/SKU, order, or mover), aligned metrics and active sort. Position/order charts name axes/period/series, provide summaries/data alternatives, and preserve selected query/product context.
6. **Apply accessibility and responsive behavior:** AX; tabs expose selected state, combobox follows its keyboard pattern, sort buttons announce direction, and chart meaning is not hover/color only. Verify semantic DOM order, keyboard/focus behavior, no hover-only meaning, both themes, narrow widths, zoom, and reduced motion.
...
Canonical target: VC with `STORY_TEST_TARGET=src/app/(dashboard)/analytics/search` and `STORY_ID=170.7`.

Run in this order and capture commands, exit codes, test counts, and complete failure output:

```bash
cd "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn"
npx vitest run "src/app/(dashboard)/analytics/search"
npm run test:e2e -- --grep "Story 170.7-FE"
npm run lint
npm run type-check
npm run check:max-lines
npm run build
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" diff --check
```
...
- **State matrix:** SC plus no-selection, invalid/deep-linked tab, per-tab partial/error, unknown seller, and results-updating states.
- **Route-specific table/chart contract:** RTC; each table has a route-specific caption and primary identifier (query, product/SKU, order, or mover), aligned metrics and active sort. Position/order charts name axes/period/series, provide summaries/data alternatives, and preserve selected query/product context.
...
- **Required Story evidence:** VE plus each tab, no-selection, partial sibling tab, combobox keyboard, and chart/table equivalence evidence.
...
```bash
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" status --short
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" diff --check
UNSTAGED_TRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" diff --name-only)"
UNTRACKED_FILES="$(git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" ls-files --others --exclude-standard)"
...
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" add -- $EXPLICIT_ALLOWED_FILES_AND_TESTS
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" diff --cached --name-only
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" commit \
  -m "feat(analytics): migrate Story 170.7 route to shadcn" \
  -m "Story 170.7-FE — Migrate Search Analytics Workspace. Migrates only the canonical owned surface, preserves behavior/contracts, and includes targeted plus local validation evidence."
git -C "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn" push -u origin "cdx/epic-170-story-7-search-analytics-shadcn"
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_PR_URL="$(gh pr create --repo "$REPOSITORY" \
  --base main \
  --head "cdx/epic-170-story-7-search-analytics-shadcn" \
  --title "[Story 170.7-FE] Migrate Search Analytics Workspace" \
...
- Merge only after local validation and independent review pass:
  ```bash
REPOSITORY="salacoste/wb-erp-system-daytona-FE"
STORY_BRANCH="cdx/epic-170-story-7-search-analytics-shadcn"
...
```bash
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" push origin --delete "cdx/epic-170-story-7-search-analytics-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree remove "/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch -d "cdx/epic-170-story-7-search-analytics-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" worktree prune
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" ls-remote --heads origin "cdx/epic-170-story-7-search-analytics-shadcn"
git -C "/Users/r2d2/Documents/Code_Projects/wb-repricer-system-new/frontend" branch --list "cdx/epic-170-story-7-search-analytics-shadcn"
...
- `git worktree list --porcelain` contains no `/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn`.
...
| Route states misrepresent unavailable/partial data | Test and visually verify the exact matrix: SC plus no-selection, invalid/deep-linked tab, per-tab partial/error, unknown seller, and results-updating states. |
| Dense or responsive evidence loses business meaning | Enforce the canonical contract: RTC; each table has a route-specific caption and primary identifier (query, product/SKU, order, or mover), aligned metrics and active sort. Position/order charts name axes/period/series, provide summaries/data alternatives, and preserve selected query/product context. |
| Keyboard/focus/contrast regression | Apply AX; tabs expose selected state, combobox follows its keyboard pattern, sort buttons announce direction, and chart meaning is not hover/color only. and require axe plus manual keyboard/focus/zoom/theme evidence. |
...
4. **Given** the Story branch is created, **when** its base is inspected, **then** it is exactly `cdx/epic-170-story-7-search-analytics-shadcn` from current prerequisite-complete `main` and uses only `/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn`.
...
8. **Given** the PR is merged, **when** cleanup completes, **then** the merge SHA is on `main`, the remote/local `cdx/epic-170-story-7-search-analytics-shadcn` branches are absent, `/private/tmp/wb-repricer-fe-170-7-search-analytics-shadcn` is removed, `git worktree prune` ran, and absence/clean-status evidence is recorded.
