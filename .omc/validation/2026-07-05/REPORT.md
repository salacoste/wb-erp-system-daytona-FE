...
| **BD-44** | 🟢 latent | FE | `/analytics/storage` trends `MetricSummary.min/max/avg` (`storage-trends-normalizer.ts:47-49`) use `toCount` on a **polymorphic** type shared between `storage_cost` (money) and `volume` (count) — same AP#8 lie when used for the money metric. Pre-existing (2026-06-06), outside BD-16's named 8-field scope, not feeding a visible `formatCurrency` today. Surfaced in BD-16 1st-pass review. | new (2026-07-07): follow-up — split `MetricSummary` into money/count variants OR migrate to `toNullableNumber` + widen. Not blocking BD-16/BD-17. |
