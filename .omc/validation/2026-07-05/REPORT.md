...
| **info** | ℹ️ | FE | SKU per-row «Маржа %» (`plb20` 4,9 %) ≠ `(revenue_net − expenses)/revenue_net` (74,6 %) — likely the known margin-aggregated ROI mapper gap (see project memory). Backend-contract check before fixing. | **resolved BE-side (2026-07-12)**: root cause was NOT an FE field-semantics issue — it was a BE correlated SQL subquery binding unqualified `nm_id`/`sale_dt` to the `products` table, excluding sales entirely → wrong per-row margins. Fixed BE + regression SQL test; W24 verified (`lpw40_2` gross 74.88% / operating 47.87%, no nulls). The FE field binding was correct all along. Unjustified speculative `include_cogs`/`Number(nm_id)` changes were reverted BE-side. |
