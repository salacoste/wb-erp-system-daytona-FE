...
**Within-line propagation drift sub-pattern (Story 118.1-FE, from Epic 116-FE retro § A-5).** Checklist step 2's grep confirms a line CONTAINS the phrase — it does NOT confirm every occurrence WITHIN that line was fixed. In long single-line entries (canonically `sprint-status.yaml` development_status entries, which pack a full multi-clause status comment onto one line), a stale phrasing and its corrected replacement can coexist on the SAME line: a fix updates one clause, a `grep -n` still matches (on the corrected clause OR the stale one), and the author reading only the grep-match fragment never sees the second, untouched occurrence. **Rule: after a grep match on a long line, READ THE FULL LINE — not just the matched fragment — and fix every occurrence within it.** Canonical case: Story 116.1-FE in-chain 4th-pass F-1 caught the `sprint-status.yaml` Story 116.1-FE `development_status` entry carrying BOTH the superseded mechanism framing ("via Trigger 2 + user practice predating user-invoked codification") in its A-3-clause AND the corrected framing in its AC-6-clause, within ONE line — a prior fix (2nd-pass F-6) had updated only the AC-6 clause. (Section-name reference, not a `:N` line number, per the Story 97.3-FE citation-rot lesson in this same Pattern 4 section — YAML status entries churn.) Story 116.1-FE close-row Lesson 1 generalizes it: "Within-line YAML status entries hide propagation-drift from grep — read full line not just match." Mechanically: `grep -n` output truncates the displayed line in some terminals; prefer `grep -no` to count occurrences, or `awk` to print the whole matched line, when auditing long-line propagation.
