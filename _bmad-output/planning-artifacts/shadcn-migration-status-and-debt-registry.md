...
> **Snapshot date: 2026-08-30** (Epics 166-172 are complete; Story 173.5 shipped through feature PR #341 and its separate documentation closeout is in progress; Epic 173 = 5/13; Program NEXT = Story 173.6 only after Story 173.5 closeout merge and exact cleanup; Stories 166.1-173.5 are done). Канонический статус-реестр программы миграции
...
| 173-FE                       | 13              | **5**  | 173.6-13                          | **IN PROGRESS**; settings 173.1-173.5 shipped; NEXT 173.6 after Story 173.5 closeout cleanup; owners 173.8 shipments and 173.12 supplies remain |
...
**Story readiness: 81 of 94 canonical Stories complete.** Epics 166-172 are closed. Story 173.5 merged through
feature PR #341 and its separate documentation closeout is in progress, advancing Epic 173 to 5/13 and raising the recorded Vitest floor to
**19,647/0** across 1,242 files. The remaining program is Epic 173 (8 route Stories) followed by the strict
...
**172.11 SHIPPED (2026-08-28, feature PR #311 merge `8b172445` + closeout PR #312 merge `73174259`)** — MINOR-GAP: monitor route, 14 файлов (+213/−52): 18 palette + 9 hex → семантика (гейдж band-хелпер hex→color CSS-var + style-stroke; recharts var()-идиома), гард 9 (каталог 14 exact-array, мутационно-проверен); **e2e legend-ремонт** (BD-22 strict-mode, bisect-доказан pre-existing); полный пол **19 423/0/1221** (+9 exact); carry-out STATUS_COLORS → 172.12. Артефакт: `172-11-fe-migrate-the-monitor-route.md`.
**172.12 SHIPPED (2026-08-28, PR #315, merge `9498cb76`)** — FULL-class MINOR: monitoring console, 23 файла (+352/−134): 74 palette + 20 hex + 2 rawbtn → семантика ×19 файлов; heatmap-карты синхронизированы (color-mix recovered); гард 10 (каталог 32 + dead-аннотация + legend-sync pin); побочный a11y-фикс тултипа; полный пол **19 433/0/1222** (+10 exact); e2e 11×2. Артефакт: `172-12-fe-migrate-the-monitoring-operations-console.md`.
...
**173.4 SHIPPED (2026-08-29, feature PR #338 merge `6a6e1bf8` + closeout PR #339 merge `d99f812e`)** — 9 файлов (+1433/−326); truthful invalid/loading/error/empty/unavailable financial states; native-valid amount semantics; pending-safe create/update/delete overlays; explicit unavailable period/amount evidence; deterministic focus lifecycle; privacy-safe responsive/theme/reflow/axe proof; focused **4/98**; полный пол **19 615/0/1235**; e2e **37 pass / 1 optional Manager setup skip**; production build **70/70**; two exact-head product reviews APPROVE with zero findings; docs review APPROVE with zero unresolved issues; exact product and initial docs cleanup proved. Артефакт: `173-4-fe-migrate-expense-settings.md`.
**173.5 SHIPPED (2026-08-30, feature PR #341 merge `41d686de`)** — 28 файлов (+1806/−944); truthful Telegram loading/unavailable/bound/unbound states; FBS independence; semantic Telegram/status tokens; labeled switches and visible radio focus; complete quiet-hours candidate validation; pending-safe binding/unbind lifecycle; deterministic focus; responsive/theme/reflow/reduced-motion/axe discovery; focused **10/68**; свежий полный пол **19 647/0/1242**; Playwright **40 discovered**, browser execution gap explicit; production build **70/70**; product review repairs closed, separate exact-final closeout review required; documentation closeout and exact cleanup gate 173.6. Артефакт: `173-5-fe-migrate-notification-settings.md`.
**NEXT = Story 173.6 Tariff Settings after Story 173.5 documentation closeout and exact cleanup** — notification feature #341 merged; оставшиеся owner-стори: 173.8 shipments и 173.12 supplies.
...
**Recorded full Vitest floor: 19,647/0 across 1,242 files** after Story 173.5. Real screen-reader and browser-UI zoom evidence — carry-out 174.3; retained raw visual artifacts remain prohibited by repository privacy policy;
...
- Epic 172 owner chains are complete. Epic 173 settings shell 173.1, Backfill Settings 173.2, Cabinet Settings 173.3, Expense Settings 173.4, and Notification Settings 173.5 are merged; the remaining Epic 173 owners are 173.8 shipments and 173.12 supplies. Story 173.6 starts only after the Story 173.5 documentation closeout and exact cleanup; other consumers start only after the applicable owner merge and closeout cleanup.
...
- Актуальный вход: `docs/HANDOFF-2026-08-29-EPIC-173-174-FULL-MIGRATION-AND-DEBT.md`; более ранние handoff сохраняются только как история и не являются execution entry points. Credential-bearing исторический handoff остаётся открытым долгом SEC-DOC-1 до отдельного review-safe security remediation.
