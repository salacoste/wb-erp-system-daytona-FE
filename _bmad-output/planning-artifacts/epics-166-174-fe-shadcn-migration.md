...
**Current Delivery State:** `in-progress`. The canonical frontend implementation artifact is the durable cross-repository delivery record. It must contain committed backend local/origin/base SHA evidence, any exact foreign-WIP reservation and fail-closed no-overlap proof, honest RED evidence, and the exact independently reviewed backend manifest before the first backend production edit. The manifest/hash must be bound to an exact evidence commit and re-read from that committed artifact before backend GREEN work or final scope comparison.
...
**Current Delivery State:** backlog. PR #226 is a preface, not completion. Worktree creation first treats the caller-supplied Story 169.14 handoff SHA only as an expected value, fetches verified frontend `origin/main`, and authenticates the exact direct single-parent artifact-only commit. It strict-parses the committed 30-line version-3 record and adjacent cleanup-authenticated foreign-WIP payload, independently reconstructs the complete strict nine-line cleanup-authority bytes from committed/static authoritative fields, requires their SHA-256 to equal the committed authority hash, and uses synthetics to reject every changed field/hash. It proves the handoff's actual parent equals the committed frontend base, validates the exact frontend final-handoff PR repository/number/URL/MERGED state/main base/head branch/`headRefOid`/merge SHA, and proves handoff → exact frontend merge → refreshed frontend `origin/main`. It independently authenticates the recorded Story 169.14 evidence commit as single-parent, artifact-only, and ancestral to both the handoff base and refreshed frontend main; re-reads the exact evidence artifact with `git show`; globally validates/extracts the RED/reviewer/manifest payloads; recomputes their hashes; reruns the suffix-aware seven-operator privacy gate and unique attestation; authorizes the frozen manifest against Story 169.14's allowed/required sets; and requires byte/hash equality with the real backend base-to-feature manifest. It validates the exact backend PR/feature/direct-parent/merge/refreshed-origin topology, then proves exact backend and frontend final-handoff branch/worktree absence plus absence of all five source records and predecessor transaction residue. Every expected-absent record/bootstrap/transaction/worktree path must also be non-symlinked, and an executable synthetic rejects dangling symlinks. Story 169.15's own evidence commit must be the direct single-parent child of the base and change only its Story artifact; one global exact-order/no-cross-nesting marker state machine rejects duplicate/mismatched/reordered blocks before extracting retained RED, reviewer, and manifest payloads and recomputing all three hashes from file bytes. Evidence-preflight authorizes the blank-line-stripped canonical nonempty manifest as a subset of the static allowlist and excludes the evidence-artifact path; delivery and publish recovery retain the same gate. Its suffix-aware scan covers the full credential-family set and exactly `=`, `:=`, `+=`, `-=`, `?=`, `&&=`, and `||=`, with the seven mandatory prefixed negative synthetics and benign omitted-field prose control. The committed and hashed retained reviewer payload contains exactly one complete `STORY_169_15_PRIVACY_REVIEW_DISPOSITION: PASS` line; missing or duplicate dispositions fail closed. Before the final commit, publish exact mode-600 `story-169-15-review-bootstrap-v1`; post-commit/pre-reviewed-head recovery requires that byte-identical reviewed authority and remote absence, and the bootstrap is cross-checked/removed before first push. REST enumeration normalizes lowercase state with `merged_at`, rejects `CLOSED_UNMERGED`/invalid state, re-reads after creation, and does not re-merge exact `MERGED`. The ten-line lifecycle PENDING record is create-or-identical; an existing PENDING record and its recorded OPEN PR are validated before any remote mutation; finalization requires exact matching PENDING bytes; finalized state routes to restartable cleanup. Cleanup publishes the strict self-contained 24-line mode-600 `story-169-15-cleanup-transaction-v1` before either source deletion, binding base/evidence/feature/tree/cumulative-manifest/reviewer truth, exact PR/merge/main topology, source paths/hashes, deletion set, and cleanup proof. Recovery reruns live Git/GitHub topology and accepts each source only as exact-present or already absent under the authenticated transaction; malformed/reordered/extra/missing fields, wrong mode/type, symlink, foreign hash, or topology drift fails closed. The transaction is removed only after both sources are proven absent.
...
- **Test and Visual Evidence:** filters/pagination/download regressions plus responsive/theme screenshots.
- **Local Validation:** targeted finance tests plus universal checks.
- **Branch/Worktree Lifecycle:** `cdx/epic-172-story-10-finances`, dedicated temporary worktree.
- **Cleanup Evidence:** completed lifecycle and removed worktree evidence.
