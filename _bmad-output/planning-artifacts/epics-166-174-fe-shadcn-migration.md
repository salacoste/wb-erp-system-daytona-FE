...
**Delivery Record:** **Route/User Value:** cross-product semantic consistency. **Owned Surface:** `src/styles/globals.css`, `components.json`, Tailwind/PostCSS config, compiled-style/contrast probes. **Shared Dependencies:** none. **Allowed Change Surface:** token/compiler/config tests only. **Forbidden Shared Files:** primitives, compositions, AppShell, routes, APIs/hooks. **State Coverage:** all interactive, semantic, availability, chart, and light/dark states. **Responsive/Table/Chart Contract:** tokens preserve type, spacing, focus, chart distinction, and density at all target widths. **Accessibility Contract:** computed contrast and focus-token proof. **Test and Visual Evidence:** compiled CSS, contrast matrix, token showcase. **Local Validation:** token probes plus universal commands. **Branch/Worktree Lifecycle:** `cdx/epic-166-story-1-token-compiler`, exclusive token writer. **Cleanup Evidence:** proof no parallel palette remains plus universal cleanup.
