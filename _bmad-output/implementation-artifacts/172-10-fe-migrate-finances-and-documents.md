...
Status: done — PR #308 merged (`eb09f735`, commit `e1406cd2`); **born-clean + absorbed parallel delta** — 10 файлов (6 M + 4 A, +558/−105): caption RTC-контракт, download-a11y (+сброс stale-фидбека при смене формата), categoryState loading/error, filtered-empty + reset, error.tsx boundary, DocumentsBody-extraction (max-lines), гард 10 тестов (каталог 8 exact-array), **e2e-спека ПОЧИНЕНА** (glob→RegExp query-фикс, bisect-доказанный pre-existing на main); targeted 6/46; полный пол **19 414/0/1220** (floor 19 394 → +20 exact); e2e 12✓ (3 setup + 2 orders + 7 финансов); 3 ревью-прохода (все findings применены/диспозиц.); cleanup 0/0/0.
...
Per plan (canonical AC + execution checklist) — все закрыты. A11y-контракты: accessible table (caption + scroll-контейнер), named downloads, pending status announcement, non-color document/status meaning.
...
- [x] Task 7: валидация финальная (соло, без CPU-конкуренции): targeted 6/46; lint 0/0; tsc 0; max-lines OK; build --webpack EXIT=0; полный пол **19 414/0/1220 EXIT=0**; e2e npm-обёрткой 12✓; `git diff --check` чист.

## Dev Notes

- Floor-арифметика: 19 394 → **19 414 (+20 exact)** = 9 гард + 3 моих поведенческих + 6 абсорбированных (2 categoryState + 1 filtered-empty + 1 error-boundary + 2 download-failure it.each) + 1 pass-3 fix-тест; файлы 1218 → 1220 (+гард, +error.test).
- E2e-разложение обёртки: 12 = 3 setup + 2 orders + 7 финансов (6 починенных + 1 новый filtered-empty). BE login-троттл: 5 логинов израсходовано ровно (3 полных обёртки + 1 bisect + 1 финальная) — на пределе окна.
...
- /tmp logs: `172.10-{baseline,fix1,fix2,fix3,guard1,lint,tsc,maxlines,build,full,full2,mixed*,final-*,e2e,e2e2,e2e3,bisect,devserver*}.log`; диффы `172.10-review-diff.txt` (мой) и `172.10-review-diff-v2.txt` (объединённый); коллизия: `/tmp/172.10-collision-snapshot/`.
...
- Live light/dark скриншоты + 200% zoom + reduced-motion + real-SR валидация — трек 174.3 (прецедент 172.4-172.8); DOM/a11y-evidence — Playwright accessibility-снапшоты (caption role, labels, named downloads, alert/status roles).
...
- M `e2e/finances.spec.ts`; M `src/app/(dashboard)/finances/{page.tsx,error.tsx→A}`, components/{DocumentDownloadButton,DocumentsFilters,DocumentsTable}.tsx, components/__tests__/DocumentsTable.test.tsx
- A `error.tsx`, `error.test.tsx`, `components/DocumentsBody.tsx`, `__tests__/finances-presentation-source-contracts.test.ts`
...
| 2026-08-28 | Merged: PR #308 (`e1406cd2`, merge `eb09f735`); объединённый дифф: +categoryState/error-boundary/download-семантика/filtered-empty, DocumentsBody-extraction; targeted 6/46, full **19 414/0/1220** (+20 exact), e2e 12✓, 3 прохода (pass-3 MEDIUM применён); cleanup 0/0/0. **Эпик 172: 10/17.** Status: review → done. **Lessons:** (1) Playwright globs end-anchored — query URLs bypass stubs silently; RegExp routes fix; bisect proved pre-existing on main. (2) Concurrent session co-developed inside my worktree: snapshot, STOP, owner arbitration, then absorb + full re-validation. (3) Background full-suite racing build/e2e starves vitest forks (pool timeouts read as missing files) — run the floor solo. |
