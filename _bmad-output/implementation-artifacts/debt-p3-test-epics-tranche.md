...
lint/build/vitest-соло не гонялись на этой ветке ПОСЛЕ ревью — дифф не затрагивает ни surfaces этих
гейтов (e2e-спека вне eslint-glob `src/**`, вне vitest-include и build-input; md-файлы вне npm-гейтов).
**tsc ПОКРЫВАЕТ e2e** (tsconfig include `**/*.ts`, `--listFilesOnly` содержит спеку — pass-1-находка MEDIUM)
и был прогнан на ветке: **0 ошибок**. Предыдущее полное состояние гейтов = main `57d772c6` (проверено
пост-merge #431). Контракты 33/33: `npx vitest run src/test/story-174-3-state-contract.test.ts
src/test/story-174-3-surface-contract.test.ts` (33 теста в 2 файлах — живой прогон этой ветки).
...
**Lessons:** (1) PRODUCT-константа e2e-спеки — UI-роут, не API-путь: 404 пробы по нему ложный, настоящий эндпоинт ищи в хуке-консьюмере. (2) Пин недели в UI-URL живёт/умирает вместе с данными недели — re-pin делай живым пробом by-variant-эндпоинта, не догадкой. (3) Цифры handoff'ов сверяй по статике репо до атрибуции — run-level итоги регена липнут к неверной причине.
