...
- **Verify-first (VERIFIED 2026-07-03, auth restored)**: `GET /v1/moysklad/variants?limit=&offset=` → `{rows, meta:{size}}` (same envelope as /products — no `{data}` wrapper; read `.rows` + `.meta.size`). 41 variants. Row: `{id, name, code, product (parent ref), barcodes[], updated}` — **NO article** (variants lack it, per contract). Auth was broken (seed re-seed) → fixed: seed reverted to `LocalTest123!` + idempotency gap patched; storageState regenerated.
