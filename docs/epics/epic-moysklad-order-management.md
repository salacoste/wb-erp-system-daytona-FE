...
- **BLOCKED: AUTH (2026-07-03).** `/variants` verify-first couldn't run: test login `test@test.com`/`Russia23!` now returns `INVALID_CREDENTIALS` (worked in M1/M2 earlier today → backend re-seeded / password changed); `e2e/.auth/user.json` storageState token also expired (401). Backend itself is up (`/v1/meta/version` 200). This blocks verify-first (curl needs JWT) AND E2E (needs auth) for **M3 and all remaining stories (M4, M5, O1–O5)** until creds are restored. Loop paused (`CronDelete e9c4f42a`) to avoid ⛔-spin. Resume by re-`/loop` once auth works.
...
- **2026-07-03 — M3 ⛔ (BLOCKED: AUTH, loop paused)**: `feat/m3-moysklad-variants` verify-first aborted — test login `test@test.com`/`Russia23!` now `INVALID_CREDENTIALS` (3/3; worked in M1/M2 earlier today → backend re-seeded/password changed); `e2e/.auth/user.json` token expired (401); M2 E2E re-run also fails (auth redirect). Backend up (`/v1/meta/version` 200). Blocks verify-first + E2E for **M3 + all remaining (M4/M5/O1–O5)**. Loop paused via `CronDelete e9c4f42a`. Resume when creds restored.
