...
- **Verify-first (VERIFIED 2026-07-03, auth restored)**: `GET /v1/moysklad/variants?limit=&offset=` → `{rows, meta:{size}}` (same envelope as /products — no `{data}` wrapper; read `.rows` + `.meta.size`). 41 variants. Row: `{id, name, code, product (parent ref), barcodes[], updated}` — **NO article** (variants lack it, per contract). Auth was broken (seed re-seed) → fixed: seed reverted to `<E2E_TEST_PASSWORD>` + idempotency gap patched; storageState regenerated.
...
- **Verify-first (VERIFIED 2026-07-05, backend source)**: `POST /v1/supplies/:id/acceptance-act` body `{ file: base64, filename ≤255, format: 'xlsx'|'zip' }` → `{ id, docType, format, fileSize, generatedAt, expiresAt: null }` (DTO `@IsBase64`/`@IsString`/`@MaxLength(255)`/`@IsEnum`). `GET /v1/supplies/:id/acceptance-act` → binary stream (404 if none stored; **dedicated route**, NOT `/documents/:type`). WB retains acts 3 months; we store indefinitely (no `expiresAt`). Backend `SupplyDocType` enum serializes UPPERCASE (`ACCEPTANCE_ACT`/`STICKER`) — FE normalizer folds to lowercase at the boundary.
...
- **Files**: `uploadAcceptanceAct` + `downloadAcceptanceAct` (supplies-acceptance-act.ts); `useUploadAcceptanceAct`/`useDownloadAcceptanceAct`; `AcceptanceActSection.tsx`; wired on supply-detail `[id]/page.tsx`.
- **Tests**: API (upload body, download blob option, detectFormat) + hook (encode+invalidate, size guard, 404-via-ApiError) + component (empty/stored/upload/validation) + normalizer case-fold + page hook stub.
- **2-pass review**: BLOCKED → fixed 2 CRITICAL (apiClient had no Blob path → added `responseType:'blob'`; docType case-mismatch → normalizer `toLowerCase`) + HIGH (404 toast via `ApiError.status` not locale regex; client-side 25 MB size guard). 3rd pass after fixes.
...
- **2026-07-05 — O5 ✅**: acceptance-act upload/download. `uploadAcceptanceAct` (POST base64 `{file,filename,format}`) + `downloadAcceptanceAct` (GET Blob, dedicated route) + `useUploadAcceptanceAct`/`useDownloadAcceptanceAct` + `AcceptanceActSection` on supply-detail (file picker → base64 → upload; stored indicator + download). **2-pass review BLOCKED → fixed:** (1) CRITICAL — `apiClient` had no binary Blob path (returned `response.text()`); added `responseType:'blob'` option + branch (also repairs latent sticker `downloadDocument`). (2) CRITICAL — backend `docType` serializes UPPERCASE (`ACCEPTANCE_ACT`); normalizer passed through verbatim → `find(d=>d.type==='acceptance_act')` never matched; fixed `normalizeSupplyDocument` to `toLowerCase()` (also repairs latent sticker/barcode label class; covered by new uppercase normalizer test). (3) HIGH — download 404 detected via `ApiError.status===404` (backend message is English; old RU-regex was dead). (4) HIGH — client-side 25 MB size guard before base64 encoding. 3 API + 5 hook + 5 component + 1 normalizer-case-fold + page stub. **МойСклад Order Management O1–O5 COMPLETE.**
