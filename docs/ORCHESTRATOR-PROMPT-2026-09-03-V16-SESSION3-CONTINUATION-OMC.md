...
> **Вход-точка (читать ПЕРВЫМ, целиком)**: [`docs/HANDOFF-2026-09-05-V16-SESSION3-EXECUTION-AND-REMAINING-BACKLOG.md`](HANDOFF-2026-09-05-V16-SESSION3-EXECUTION-AND-REMAINING-BACKLOG.md) (сессии-2/3 исполнены: P1 закрыт, волны 1-3 done; следующий item — boundary волна-4)
...
**Первое действие после прочтения**: SESSION3-handoff (2026-09-05) целиком (§2 гейты + §3.0 волна-4) → §1 bootstrap (вкл. проверку чужого WIP) → boundary волна-4 (§4 п.1) → петля §0. D-2+волна-3 уже исполнены (статусы в §1 нового handoff'а); WCAG-модель = слоистая (артефакт волны-3).
