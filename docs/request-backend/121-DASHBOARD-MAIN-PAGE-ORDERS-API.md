...
> **⚠️ Важно: Различие между эндпоинтами трендов**
>
> В системе существует два отдельных эндпоинта для трендов:
>
> | Эндпоинт                      | Назначение              | Агрегация               | Используется в                     |
> | ----------------------------- | ----------------------- | ----------------------- | ---------------------------------- |
> | `/v1/analytics/weekly/trends` | Финансовые KPI дашборда | ISO недели (`2025-W49`) | `useTrends.ts` - главный дашборд   |
> | `/v1/analytics/orders/trends` | Аналитика заказов FBS   | Дни/недели/месяцы       | `fbs-analytics.ts` - FBS аналитика |
>
> Этот документ описывает **Orders API** (`/v1/analytics/orders/*`) для работы с заказами FBS.
> Для финансовых трендов главного дашборда см. документацию Weekly Analytics API.
