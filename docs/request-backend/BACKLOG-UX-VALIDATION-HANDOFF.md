...
- **Finding: not a defect.** The aggregation is **idempotent per-input** — `weeklyMarginFact.upsert` (`margin-calculation.service.ts:399`) and `weekly_payout_total/summary` all upsert by the unique key _(week, cabinet, reportType)_. Re-running with identical source data yields identical results.
