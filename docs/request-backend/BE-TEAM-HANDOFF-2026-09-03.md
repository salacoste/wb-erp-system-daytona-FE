...
> **Статус 2026-09-03 после обработки BE-командой: все три item закрыты.** BE PR [#230](https://github.com/salacoste/wb-erp-system-daytona/pull/230) merged (`c1e9b5e7`): sliding access-JWT rotation опубликована, SEC-DOC-1 sweep выполнен отдельным коммитом, health-семантика очереди исправлена. Локальные API/worker пересобраны на Node 24.18.0: `POST /v1/auth/refresh` с заведомо невалидным Bearer теперь доходит до маршрута и возвращает `401 UNAUTHORIZED` вместо `404`; `/v1/health` возвращает `healthy` с `database/redis/queue: up`. FE может продолжать D-2 по согласованному контракту; восстановление уже истёкшего JWT остаётся отдельным расширением контракта.
