...
5. **Verify auth flow end-to-end** post-fix:
   ...
   curl -s -X POST http://localhost:3000/v1/auth/login \
     -H "Content-Type: application/json" \
     -d '{"email":"test@test.com","password":"<E2E_TEST_PASSWORD>"}' | jq '.data.access_token // .access_token'
   # Should return a JWT, not 500
   ```

## Acceptance criteria (backend)

1. `POST /v1/auth/login` returns 200 OK with a valid JWT for the test credentials (`test@test.com` / `<E2E_TEST_PASSWORD>`)
