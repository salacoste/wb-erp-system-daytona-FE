...
| **PB-3**                    | **RESOLVED (2026-09-03)** | Нет реактивного 401-refresh в api-client (только proactive) — ИСПРАВЛЕНО: interceptor (single-flight + каскад-гейт + replay-once + 10s дедлайн), nonce-safe пративная ветка, durable-op opt-outs; live-гейт верифицирован (02:02Z: refresh 200/ревокация 401); артефакт `debt-d2-pb3-reactive-refresh.md` | post-expiration recovery — след. BE-этап; FE-расширение тривиально | артефакт D-2 + 4 ревью-прохода |
...
| **boundary cat-1**          | in-progress owner-sweep  | 459 → 401 → **372** (волна-1 finsum 58; волна-2 2026-09-03: Margin-семейство 29 + D-4 fold-in /15→/5 — поймал живой AA-fail 4.19/4.42, скрытый аттестацией D-4; артефакты `debt-p2-boundary-wave{1,2}-*.md`)                                                                 | волны 3-5 остатка 372 (SourceBadge 16, RequireJam 13, lib-residue 210); ratchet ↓ тем же коммитом                  | каталог + артефакты волн                                                 |
...
| **C13**                     | **RESOLVED (2026-09-02, сессия-2)** | GapsTable caption-dup (`src/app/(dashboard)/analytics/gaps/components/GapsTable.tsx:65,67`)                                                                                                                                                                     | aria-label → «Область прокрутки…», caption = идентичность; сиблинг-sweep (~20 таблиц) — follow-up артефакта `debt-p2-c13-c15-quality-wave.md` | артефакт волны C13+C15 |
| **C15**                     | **RESOLVED (2026-09-02, сессия-2)** | `URGENCY_CLASS` локализованные ключи (`src/app/(dashboard)/analytics/liquidity/components/LiquidationScenarioCard.tsx:20-24`)                                                                                                                                   | `ScenarioUrgencyTier` + `getScenarioUrgencyTier` (single source), typed Record, exhaustive color-switch; hex/token мисматч (production-dead) — owner follow-up | артефакт волны C13+C15 |
...
| D-2 | ✅ DONE (2026-09-03, хвост сессии-2) — **PB-3** реактивный 401-refresh | interceptor + unified rotation + opt-outs + G4-flip (12 тестов); e2e EXIT=0; live 02:02Z; флор 19424→19436 | `src/lib/api-client-refresh.ts` (NEW), `src/lib/api-client.ts`, `src/hooks/useAuth.ts` |
