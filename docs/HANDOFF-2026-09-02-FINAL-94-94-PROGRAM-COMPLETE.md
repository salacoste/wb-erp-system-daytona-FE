...
| **SEC-DOC-1**               | **redacted; ротация ОДОБРЕНА owner 2026-09-02 (исполнение — owner)** | plaintext-креды в tracked docs                                                                                                                                                                                                                   | FE: оба литерала (stale + live) изъяты, non-echoing скан = 0 (PR #383); ротация live-креда ОДОБРЕНА (how-to в decision-запросе); D-4 сканер `.http` — исполнен (PR #386); BE-репо (135 stale) + history — решение D-2/D-3 (рекомендация: history не трогать) | артефакт `debt-sec-doc-1-redact-creds.md` + decision-запрос |
