...
1. **PB-1/PB-2/PB-3** — триаж продуктовых дефектов (приоритет: PB-1 silent-failure, PB-3 auth-UX); PB-4 — ✅ resolved 2026-09-02 (D-3, PR #TBD).
2. **WCAG-свипы** — `/15-family` зарегистрированный скоуп — ✅ resolved 2026-09-02 (D-4, PR #TBD; остаток ~100 /15-сайтов — owner-sweep); `/80-sweep` (замер → replace/exception) — остаётся.
...
| D-3 | ✅ DONE (2026-09-02, PR #TBD) — **PB-4** FeedbackButtons dark-fail 3.53:1 | solid-пара `bg-status-success text-status-success-foreground` + чип-геометрия; origin-комментарий переписан; boundary-исключение снято (Map 4→3, self-test синхронен)      | `src/components/custom/ai/FeedbackButtons.tsx`; `scripts/check-shadcn-ui-boundary.mjs` + manifest §7    |
| D-4 | ✅ DONE (2026-09-02, PR #TBD) — **/15-family** <4.5:1 | те же solid-пары (канон 173.12 / 174.4-D5); замер обеих тем: 4.81/11.41 (warning), 5.13/8.00 (success) — все ≥4.5 PASS                                        | `src/components/custom/price-calculator/margin-status-helpers.ts:13,16`, `AcceptanceStatusBadge.tsx:49` |
