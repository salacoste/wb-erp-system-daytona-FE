...
- Окружение: Node **24.18.0** PATH-пин `/opt/homebrew/opt/node@24/bin` (Node-26 ломает webpack) · PM2 `wb-repricer-frontend-dev` :3100 · BE :3000 жив; **BE-контракт ready в локальной ветке, НЕ задеплоено** (live `/v1/auth/refresh` ещё 404; `/v1/health` ещё может отдавать ложный queue:down до deploy — фикс семантики в той же ветке)
- Тест-креды: только в untracked `.env.e2e` (SEC-DOC-1 канон)

## 3. Оставшийся бэклог — по приоритету, с реализационными деталями

### 3.0 СЛЕДУЮЩИЙ ITEM — P2 волна-3 «AA-quick-wins» (всё замерено, паттерн готов)
...
### 3.1 D-2 (PB-3) — CONTRACT-READY, ждёт owner-«ок» на re-scope + BE deploy
...
**Re-scope (предложен owner, ждёт подтверждения)**:
...
4. e2e синтетика (route-interception); **live-верификация = `PENDING BACKEND:` post-deploy** (curl + /v1/health)
...
| BE queue:down | ✅ фикс в BE-ветке; после deploy `/v1/health` очистится |
| BE publish/deploy | ⏳ ждём сигнал BE-команды → live-верификация refresh + health + D-2 e2e live |
