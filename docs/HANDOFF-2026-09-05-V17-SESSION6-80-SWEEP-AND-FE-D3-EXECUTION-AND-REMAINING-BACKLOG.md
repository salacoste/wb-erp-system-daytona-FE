...
4. **full-warn-on-warn/10 кластер — ✅ DONE (сессия-8, PR #417, объединено с item 5)**: волна-6 — 12 текст-FAIL (4 новых, вкл. text-white 1.41 dark) → fg-on-tint + warning-foreground-токен + underline; post-remedy worst kept 4.87. 3 ревью-прохода (P1 независимый калькулятор). Витest 19537→**19559**. Остатки: chart-2 → C5-owner — реестр §14.
5. **selected-row стеки — ✅ DONE (сессия-8, PR #417, волне-6)**: ProductTableRow warn-чип + ProductMarginCell (реестровый «primary 4.18» = margin-cell семейство) → fg-on-tint на всех 4 статусах строк (worst selHover 10.74/9.94).
