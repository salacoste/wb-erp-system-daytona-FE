---
title: 'Если Wildberries изменил тип поставки или разделил заказ на несколько заказов'
slug: wb-neskolko-tipov-postavok
source: https://selsup.ru/help/wb-neskolko-tipov-postavok/
chars: 1177
---

# Если Wildberries изменил тип поставки или разделил заказ на несколько заказов

## Если Wildberries изменил тип поставки:

Вернитесь на 1 шаг в SelSup и измените тип поставки:

![](../../out/selsup.ru/_kage/selsup.ru/wp-content/webp-express/webp-images/uploads/2021/12/izobrazhenie_2021-12-01_182622.png.webp)

## Если Wildberries разделил заказ на несколько заказов с разным типом поставок

Зайдите в каждый заказ на портале WB, нажмите кнопку «Выгрузить в Excel». ![](../../out/selsup.ru/_kage/selsup.ru/wp-content/webp-express/webp-images/uploads/2021/12/izobrazhenie_2021-12-01_185041.png.webp)

После чего создайте новый заказ в SelSup со страницы Заказы и выберите скачанный файл в поле «Перетащите файл или кликните для выбора файла с товарами».

![](../../out/selsup.ru/_kage/selsup.ru/wp-content/webp-express/webp-images/uploads/2021/12/izobrazhenie_2021-12-01_185114.png.webp)

![](../../out/selsup.ru/_kage/selsup.ru/wp-content/webp-express/webp-images/uploads/2021/12/izobrazhenie_2021-12-01_185131.png.webp)

SelSup автоматически добавит в заказ все товары и их количество из файла. Проставьте тип поставки, внешний номер и нажмите сохранить. Старый заказ можно удалить. [Перейти в помощь по созданию заказа на WB](../zakaz-na-wb/index.html).
