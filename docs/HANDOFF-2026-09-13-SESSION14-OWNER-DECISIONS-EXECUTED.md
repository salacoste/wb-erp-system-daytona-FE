...
## 4. BE-side работы — ВНЕ ЭТОГО HANDOFF

Все BE-долги (включая исполнение request-backend **#231**) ведёт BE-команда в своём трекинге —
они намеренно НЕ перечисляются здесь как открытые пункты. FE-сторона по каждому из них закрыта
или не требуется. FE-агент-команде: новые BE-запросы оформлять через `docs/request-backend/NNN-*.md`.
