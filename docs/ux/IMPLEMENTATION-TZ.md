...
### `[x]` TZ-11 · forecast-accuracy MAPE gating — verify  〔P1-2〕
- **Depends:** none.
...
### `[x]` TZ-12 · liquidity "0,0 %" misframe  〔P1-3〕
- **Depends:** none.
...
### `[x]` TZ-13 · Settings layout consistency  〔P1-4〕
...
### `[x]` TZ-14 · Empty-states vs errors distinction  〔P1-5〕
- **Depends:** none.
...
### `[x]` TZ-15 · Compact number mode  〔P2-1〕
...
- **2026-06-26** — **TZ-11 closed as no-op (already correct, FE-9).** Verified `AccuracyMetricsCards.tsx` on `/analytics/forecast-accuracy`: avgMAE leads (card 2, "основной показатель точности"); MAPE gated via `isHighMape = avgMAPE > 200` → `text-amber-600` + note "Очень высока — возможна при продажах около 0; смотрите MAE" (small-actual-SKUs contamination explanation). No `mapeValid` field exists — the threshold proxy is the mechanism. All ACs met by existing code; no changes required. Pre-flight source-trace rule (Story 105.2-FE): close as no-op with evidence.
- **2026-06-26** — **TZ-12 shipped.** Liquidity "0,0 %" misframe fix. In `LiquidityDistributionCards.tsx`, when a category has stock (`count > 0`) but ALL SKUs have zero sales (`avg_turnover_days >= 999` sentinel), the headline percentage (derived from no sales data → misleading) is replaced with neutral "Нет продаж за период". Uses the reliable sentinel (not page-scoped `no_sales_count`). Capital-distribution info preserved in the secondary `value` metric. `min-h-[2.5rem]` preserves card alignment. 2 new tests. Gates: type-check 0, eslint 0/0, full suite 16869/0, locale-percent 4, doc-citations exit 0 (100), eslint-rules OK. 2-pass review (1st: CRITICAL page-scoping + HIGH semantic; fixed via sentinel + preserving capital info; 2nd APPROVE). Follow-up: shared `isAllZeroSales` helper + benchmarks-panel neutral framing. Commit `03e1831f` + merge `30fde18a` on `main`; pushed `origin/main`.
- **2026-06-26** — **TZ-13 shipped.** Settings layout consistency. New shared `settings/layout.tsx` (2-col nav rail + content flex, no double-container) + `SettingsNav.tsx` (6 settings links with active highlighting via `usePathname` + `startsWith`, `flex-wrap` mobile, `lg:flex-col` desktop). Consolidated `/settings/cabinet` alerts: removed duplicate `<TokenHealthBanner />` (already rendered globally by the dashboard layout). AC met: settings pages share one layout shell; cabinet alerts consolidated (deduped). Gates: type-check 0, eslint 0/0, full suite 16871/0, locale-percent 4, doc-citations exit 0 (100), eslint-rules OK, next-async-params OK. 1-pass review (found HIGH double-container nesting; fixed by dropping layout's `container mx-auto py-6` — pages keep their own containers; the dashboard layout's p-4/p-6 provides outer padding). Follow-up (documented): strip per-page `container` wrappers for fully-unified max-width (currently each page retains its own `max-w-{2xl,4xl}`); add `SettingsNav` unit test. Commit `61632a0c` + merge `0919ff77` on `main`; pushed `origin/main`.
- **2026-06-26** — **TZ-14 closed as no-op (already correct).** Audited all listed pages for empty-state vs error-state distinction. All 6 areas already properly separate "no data yet" (neutral empty-state) from "failed to load" (error styling + retry): shipments (`ShipmentsEmptyState` + error message), cogs/history (`CogsHistoryError` with retry + `CogsHistoryPageStates` empty), cross-reference (`ErrorState` + `EmptyState` from `CrossReferenceStates`, properly conditional), search (error messages "Не удалось загрузить" + empty "Нет данных"), gaps (child components handle loading skeletons + TanStack Query errors), alerts (child components with empty-state sections + action errors via toasts). No empty-state reads as an error; no error reads as empty. All ACs met by existing code. Pre-flight source-trace: close as no-op.
- **2026-06-26** — **TZ-15 shipped.** Compact number mode. New `formatCurrencyCompact` (Intl compact notation: "1,2 млн ₽" / "980 тыс. ₽") in `currency-formatters.ts`, re-exported through the formatters barrel + `@/lib/utils`. Applied to `DashboardHero` tiles (the primary summary view): currency tiles show compact format; `title` attribute shows full `formatCurrency` precision on hover. Full precision preserved in the detailed P&L grid (Tier 2). Gates: type-check 0, eslint 0/0, full suite passing/0 failed, locale-percent 4, doc-citations exit 0 (100), eslint-rules OK. Follow-up (documented): apply compact to other summary views (table totals, summary cards). Commit/merge pending.
