...
- Request #17: COGS Assigned After Completed Week - Manual Recalculation
  ...
  - Status: ✅ **RESOLVED** - Frontend warning and manual recalculation implemented (2025-01-27)
...
- 😞 Poor UX: User assigns COGS → sees "— (нет продаж)" instead of margin
- 😞 No feedback: User doesn't know margin calculation is in progress
- 😞 Manual refresh required: User must manually refresh page to see calculated margin

**Expected Behavior:**
After COGS assignment, frontend should:

1. Show status "Расчёт маржи начат..." (Margin calculation started...)
...
1. After COGS assignment, user sees notification "Расчёт маржи начат..." (Margin calculation started...)
...
8. Optimistic UI: Show "Расчёт..." (Calculating...) indicator in product list while polling
...
- [x] Integrate polling into single COGS assignment (AC: 1, 2, 3, 4, 5, 6, 7, 8, 9)
  ...
  - [x] Show "Расчёт маржи начат..." toast notification
...
- [x] Integrate polling into bulk COGS assignment (AC: 1, 2, 3, 4, 5, 6, 7, 8, 9)
  ...
  - [x] Show "Расчёт маржи для N товаров начат..." notification
...
- [x] Create polling status component (AC: 8)
  ...
  - [x] Show "Расчёт..." indicator with spinner
...
src/
├── hooks/
│   ├── useMarginPolling.ts                    # NEW: Polling logic
│   ├── useManualMarginRecalculation.ts        # NEW (Request #17): Manual recalculation
│   ├── useSingleCogsAssignment.ts             # UPDATE: Add polling trigger
│   └── useBulkCogsAssignment.ts               # UPDATE: Add polling trigger
├── lib/
│   └── margin-helpers.ts                      # NEW: Helper functions (includes Request #17 helpers)
└── components/
    └── custom/
        ├── MarginCalculationStatus.tsx        # NEW: Status indicator
        ├── SingleCogsForm.tsx                 # UPDATE: Show polling state + Request #17 warning
...
- Display progress for bulk operations (e.g., "Расчёт для 500 товаров...")
...
- If `null` → still calculating
- If `number` → calculation complete
...
- ✅ Phase 1 Complete: Core polling infrastructure (helpers + polling hook)
- ✅ Phase 2 Complete: Integration with COGS assignment hooks
- ✅ Phase 3 Complete: UI components (MarginCalculationStatus + form updates)
- ✅ Phase 4 Complete: Unit tests (23 tests, all passing) and ProductList polling status display
...
- ✅ Clean architecture with proper separation of concerns (helpers, hooks, store, components)
- ✅ Comprehensive test coverage (23 tests: 19 for helpers, 4 for polling hook)
- ✅ Proper use of Zustand for lightweight state management
- ✅ Well-implemented polling strategies for different scenarios (single/historical/bulk)
- ✅ Good error handling with timeout scenarios
- ✅ Proper cleanup on unmount prevents memory leaks
- ✅ Optimistic UI with "Расчёт..." badge in ProductList
- ✅ Form disabled during polling prevents duplicate requests
- ✅ Query invalidation after successful margin calculation
...
- Project Structure: ✅ Files organized correctly (lib, hooks, stores, components)
- Testing Strategy: ✅ Comprehensive test coverage (23 tests, all passing)
...
- [x] Integration with ProductList
  ...
  - [x] Badge "Расчёт..." with spinner animation
  - [x] Automatic badge removal when margin calculated

### Security Review

✅ **PASS** - Uses existing API client with proper authentication headers. No security vulnerabilities identified.

### Performance Considerations

✅ **PASS** - Efficient polling with configurable intervals (3-5s). Proper cleanup on unmount prevents memory leaks. Zustand store provides lightweight state management.
...
Gate: PASS → docs/qa/gates/4.8-margin-recalculation-polling.yml
...
✅ **Done** - All tests passing (23 tests), comprehensive coverage, all 10 ACs met, gate PASS, QA review complete.
