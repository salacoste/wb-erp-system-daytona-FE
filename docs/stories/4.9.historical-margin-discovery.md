...
✅ **PO Approved** | ✅ **Backend Ready** (Stories 23.8 & 23.9 deployed) | ✅ **Ready for Review**
...
```
┌─────────────────────────────────────────────────────────────┐
│ Термобелье (173589742)                                      │
│ COGS: 11.00 ₽                                              │
│ Маржа: — (нет продаж)                    ← USER STOPS HERE │
└─────────────────────────────────────────────────────────────┘
```
...
│ ┌─────────────────────────────────────────────────────────┐│
│ │ 📊 Последняя продажа: W44                               ││
│ │    92.32% маржа • 5 шт • 3 недели назад                 ││
│ │                                                         ││
│ │    [История продаж →]                                   ││
│ └─────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
...
| Before                      | After                           |
| --------------------------- | ------------------------------- |
| "— (нет продаж)" — dead end | Historical context + navigation |
...
**Location**: ProductList.tsx — renders inside product row when `missing_data_reason === "NO_SALES_DATA"`
...
│                                                                   │
│  ┌─ StatusLine ─────────────────────────────────────────────────┐│
│  │ ℹ️ Нет продаж за W47                                         ││
│  └───────────────────────────────────────────────────────────────┘│
│                                                                   │
│  ┌─ HistoricalBadge (when data available) ──────────────────────┐│
│  │                                                               ││
│  │  Последняя продажа:                                          ││
│  │  ┌────────────────────────────────────────────────────────┐  ││
│  │  │ W44  │  92.32%  │  5 шт  │  3 нед. назад              │  ││
│  │  │ week │  margin  │  qty   │  gap                        │  ││
│  │  └────────────────────────────────────────────────────────┘  ││
│  │        ↑               ↑                                      ││
│  │    ISO week      Green if >0                                  ││
...
│  └───────────────────────────────────────────────────────────────┘│
│                                                                   │
│  ┌─ NoHistoryMessage (when no sales in 12 weeks) ───────────────┐│
...
| "Последняя продажа:" | Inter | 12px | `text-muted-foreground` (#6B7280) | Regular  |
...
| Gap (X нед. назад)   | Inter | 12px | `text-muted-foreground` (#6B7280) | Regular  |
...
| ---------------- | ------- | ---- | ----------------------- | ----------- |
| Info (ℹ️)        | Lucide  | 14px | `text-muted-foreground` | StatusLine  |
| History (📈)     | Lucide  | 12px | `text-primary`          | HistoryLink |
| ChevronRight (→) | Lucide  | 12px | `text-primary`          | HistoryLink |
...
│                                                             │
│ Последняя продажа:                                          │
│ W44 • -5.23% • 2 шт • 5 нед. назад                         │
│         ↑                                                   │
│    Red badge (negative margin warning)                      │
│                                                             │
│ 📈 История продаж →                                         │
└─────────────────────────────────────────────────────────────┘
```
...
| **Navigation**   | Same tab (no `target="_blank"`) — preserves back navigation |
| **Cursor**       | `pointer`                                                   |
...
| **Power Users**  | Ctrl+click / ⌘+click opens in new tab (browser default)     |

**Rationale**: ProductList → Analytics is a natural exploration flow. Browser back button returns user to product list with state preserved.

#### Hover States

```
Default:    📈 История продаж →
            ↑ text-primary (#E53935)

Hover:      📈 История продаж →
            ↑ text-primary-dark (#D32F2F) + underline

Focus:      [📈 История продаж →]
...
When user clicks "История продаж →", they land on `/analytics/sku?nm_id=173589742`

#### Filter Alert Component

```
┌─────────────────────────────────────────────────────────────────────┐
│ ℹ️ Фильтр по товару: 173589742 (Термобелье)    [Показать все ✕]   │
└─────────────────────────────────────────────────────────────────────┘
                                                       ↑
...
| **Week Selector** | Still functional — user can browse different weeks     |
| **Empty State**   | "Нет данных за выбранную неделю" if no sales that week |
...
| --------------- | --------------- | ------------------------------------------------------------------------- |
| StatusLine      | `role="status"` | "Нет продаж за неделю 47"                                                 |
| HistoricalBadge | `aria-label`    | "Последняя продажа: неделя 44, маржа 92 процента, 5 штук, 3 недели назад" |
| HistoryLink     | `role="link"`   | "История продаж, ссылка"                                                  |
| FilterAlert     | `role="alert"`  | "Фильтр по товару 173589742 активен"                                      |

#### Color Contrast

| Element         | Foreground | Background | Ratio | Pass |
| --------------- | ---------- | ---------- | ----- | ---- |
| StatusLine      | #6B7280    | #FFFFFF    | 5.4:1 | ✅   |
| Positive Margin | #059669    | #FFFFFF    | 4.6:1 | ✅   |
| Negative Margin | #DC2626    | #FFFFFF    | 4.5:1 | ✅   |
| HistoryLink     | #E53935    | #FFFFFF    | 4.5:1 | ✅   |
...
| ID                   | Text                              | Context                      |
| -------------------- | --------------------------------- | ---------------------------- |
| `status.no_sales`    | Нет продаж за {week}              | StatusLine when no sales     |
| `history.last_sale`  | Последняя продажа:                | Label before historical data |
| `history.weeks_ago`  | {n} нед. назад                    | Gap indicator                |
| `history.no_history` | Нет продаж за последние 12 недель | When no historical data      |
| `history.link`       | История продаж                    | Link text                    |
| `filter.label`       | Фильтр по товару: {nmId}          | Filter alert                 |
| `filter.clear`       | Показать все                      | Clear filter button          |
| `filter.empty`       | Нет данных за выбранную неделю    | Empty state when filtered    |
...
| `last_sales_margin_pct === null` | Hide margin, show only week        | "W44 • 5 шт"                  |
| `weeks_since_last_sale === 0`    | Show "на этой неделе"              | (edge case, shouldn't happen) |
| `weeks_since_last_sale > 52`     | Show "более года назад"            | Indicates very stale product  |
| Network error loading history    | Show error state on analytics page | "Ошибка загрузки данных"      |
...
| **AC3** | If `last_sales_week` is null, show "Нет продаж за последние 12 недель"                                                                   | P0       |
| **AC4** | Margin badge color: green (>0), red (<0), gray (=0)                                                                                      | P0       |
| **AC5** | "История продаж" link navigates to `/analytics/sku?nm_id={nmId}` (same tab)                                                              | P0       |
| **AC6** | Analytics page shows filter alert when `nm_id` param present                                                                             | P0       |
| **AC7** | Filter can be cleared with "Показать все" button                                                                                         | P1       |
...
| #        | Criteria                            | Target             |
| -------- | ----------------------------------- | ------------------ |
| **NFR1** | Component renders in <50ms          | ✅ Backend batched |
...
| ------------------------------------- | ----------- | ---------------------------- |
| Story 23.9 (Backend inline fields)    | ✅ Deployed | 4 new fields in products API |
| Story 23.8 (Backend history endpoint) | ✅ Deployed | For future detail page       |
| Story 4.5 (Margin by SKU page)        | ✅ Done     | Base page to enhance         |
...
| ------------- | ----- | ---------- | ----------- |
| UX Expert     | Sally | 2025-01-27 | ✅ Complete |
| Product Owner | Sarah | 2025-11-26 | ✅ Approved |
| Tech Lead     | —     | —          | ⏳ Pending  |
...
5. **Analytics/SKU Page Filter (AC6-AC8)**: Added `nm_id` URL parameter support with filter alert and "Показать все" clear button
...
- Coding Standards: ✓ TypeScript strict mode, proper types, JSDoc comments
- Project Structure: ✓ Components in `custom/`, types in `types/`, helpers in `lib/`
- Testing Strategy: ✓ Unit tests with @testing-library/react, mocked dependencies
...
- [x] ~~**CONCERN**: ProductList.tsx exceeds 200-line limit~~ → **RESOLVED**: Refactored from 534→198 lines (extracted 6 sub-components)
- [x] ~~**OPTIONAL**: Remove console.log statements~~ → **RESOLVED**: All debug statements removed
...
✅ **PASS** - NFR1 (<50ms render) achievable:
...
| File                          | Action     | Description                                   |
| ----------------------------- | ---------- | --------------------------------------------- |
| `ProductList.tsx`             | REFACTORED | 534→198 lines, removed console.log statements |
...
**Gate: PASS** → `docs/qa/gates/4.9-historical-margin-discovery.yml`

### Recommended Status

✅ **Ready for Done** - All acceptance criteria implemented and tested. No blocking issues.

**Post-QA Cleanup Complete**: All improvements implemented. ProductList.tsx refactored from 534→198 lines, debug statements removed, 6 sub-components extracted for better maintainability.
