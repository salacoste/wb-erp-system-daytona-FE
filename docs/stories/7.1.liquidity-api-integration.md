...
10. ⏳ Backend implements Request #55
11. ⏳ Backend provides test data / staging environment
...
export function getLiquidityStatusLabel(category: LiquidityCategory): string {
  const labels: Record<LiquidityCategory, string> = {
    highly_liquid: 'Высоколиквидный',
    medium_liquid: 'Средняя ликвидность',
...
├── hooks/
│   ├── useLiquidity.ts           ← NEW
│   └── useLiquidityTrends.ts     ← NEW
├── lib/
│   ├── api.ts                    ← Add getLiquidity, getLiquidityTrends
│   └── liquidity-utils.ts        ← NEW
└── mocks/
    └── handlers/
        └── liquidity.ts          ← NEW
```
...
- [ ] ⏳ Backend implements trends endpoint
