...
// change-set scan only. They are regenerated by `npx bmad-method install` (redactions would
// not stick), hold only template teaching examples, and are not in PRIVACY_SCAN_ROOTS. The
// scan-roots walk (IGNORED_DIRECTORIES) is deliberately unaffected.
...
function isExcludedFromChangeSet(candidate) {
  ...
  return [...EXCLUDED_CHANGE_SET_PREFIXES].some(prefix => relative.startsWith(prefix))
}
