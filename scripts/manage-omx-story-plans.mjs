...
const storyConfig = {
  ...
  162.4: {
    ...
    steps: [
      'Lock the canonical raw 36-site inventory and exact 38-site semantic inventory at base SHA `9a882a1de72e8716a1969002a648e027f4a05c0f`, while preserving the exact 57-site Story 162.3 regression.',
      'Replace each operations/settings truth fallback with explicit fixture, visible-state, and request/response assertions; apply only the minimal application/API/type boundary repairs required for deterministic, contract-accurate evidence.',
      'Run backfill, supplies, supply-planning, COGS, and calculator coverage and record semantic `38 → 0` evidence.',
    ],
    verify: [
    ...
    risk: 'Mutating paths remain behind the sandbox acknowledgement guard and require deterministic fixtures; production-path edits stay limited to focus semantics, deterministic interaction state, and the existing backend sticker generation/download contract with focused unit and lifecycle coverage.',
  },
  162.5: {
    files: [
    ...
    steps: [
      'Reconcile the historical 58-call baseline with the current 55 browser waits plus two timer-delayed route fixtures, and prove the static guard is RED before implementation.',
      'Install exact-path deterministic GET fixtures, then use response predicates, visible/browser-state transitions, `page.emulateMedia({ reducedMotion: "reduce" })`, and bounded diagnostics; remove obsolete Liquidity tests for nonexistent search, pagination, and sticky-header behavior.',
      'Add a fail-closed fixed-wait scanner with Vitest regression coverage, then run repeated targeted localhost executions and record historical/current-to-zero counts, runtime, and retry behavior.',
    ],
    verify: [
    ...
    risk: 'Broad route globs can intercept page documents, live fallback can make retry tests self-heal, and network-idle can couple independent requests; fixtures must match exact GET API paths, fail closed, and wait only for the state under test.',
  },
  162.6: {
    files: [
    ...
    steps: [
      'Lock immutable canonical `67`, reconcile it with `65` current browser waits plus four timer-backed route fixtures at base SHA `aee43c154e0b3ff494a6dc6ee3cacb34043765d9`, and prove the union fixed-wait scanner is RED with exactly `69` Story 162.6 findings while preserving Story 162.5 coverage.',
      ...
      'Use only `GET /v1/analytics/orders/volume` for order volume, require `from`/`to` plus applicable `aggregation`/`include_cogs`, and correct or delete the obsolete `DASHBOARD_API_ROUTES.ordersVolume` value `/analytics/orders-volume`; loading/error cases must prove the intercepted request plus named terminal UI state.',
      'Use reduced motion for animated charts, prove period URL/localStorage plus selected-period data, delete obsolete FBS selector coverage and false-green optional/body-only fallbacks, and never use network-idle or Playwright clock as a substitute for observable UI state.',
      'Run the exact 20-spec target twice with `--workers=1 --repeat-each=2 --retries=0`, record historical/current/final `67 / 65+4 / 0` evidence, runtime, skips, and zero retry-only success, then require two fresh code-reviewer approvals before fresh architect/verifier, PR, ancestry, and cleanup gates.',
    ],
    verify: [
      'npm run check:e2e-waits',
      'npx vitest run src/test/e2e-fixed-waits.test.ts',
    ...
    risk: 'Broad routes, the obsolete `/analytics/orders-volume` alias, missing order-volume query parameters, retry fallthrough, body-only assertions, obsolete selectors, and unrelated network-idle can all create false greens; fixtures must be exact, fail closed, retry-aware, independently releasable, and coupled to intercepted requests plus named terminal UI results.',
  },
  162.7: {
    files: [
      'e2e/supply-planning.spec.ts',
