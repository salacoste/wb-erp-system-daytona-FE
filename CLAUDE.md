...
All chart/status color values in TS read **only** from CSS variables: strings = `var(--color-<role>)` full form; classes = semantic utilities (`bg-valence-1`, `text-status-information`, alpha modifiers `/10`). Raw triplets (`var(--valence-1)`) are NOT colors (resolve to black/transparent); `hsl(var(--x))` is banned in TS strings (re-inflates the boundary checker's CONTEXTUAL_HEX when the wrapped role contains a digit). Tints = `color-mix(in srgb, var(--color-token) N%, var(--color-card))`. Hex literals are banned for new code. **Enforcement is partial**: `node scripts/check-shadcn-ui-boundary.mjs` (ratchet baseline 0, **exceptions register = 0** — the 3 legacy exceptions were lifted at C5-W4) catches contextual hex and digit-bearing color functions, but digitless variable wrappers (`var(--chart-negative)`, `hsl(var(--valence-neutral))`) are NOT detected — review must enforce the broader canon. Scanner-semantics changes remain a separate owner-ledger item. Byte-twin roles (e.g. `chart-4 ≡ chart-positive` both themes, `chart-8 ≡ chart-negative` dark) are accepted W1 riders — don't remap without an owner decision.
