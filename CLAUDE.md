...
| ESLint | `npx eslint 'frontend/src/**/*.ts' 'frontend/src/**/*.tsx'` | 0 errors, 129 warnings (112 `no-explicit-any` + 17 `jsx-a11y/control-has-associated-label`) | Source: this section (manual). Notes: any ERROR is a regression. 112 warnings are pre-existing `no-explicit-any` surfaced by Story 98.1-FE enforcement fix; 17 are `jsx-a11y/control-has-associated-label` warns introduced by Story 109.6-FE (17 pre-existing violations prevent `error` severity — fix + ratchet planned for Epic 110 Story 110.1). `max-lines` rule enforced via root `eslint.config.js` (flat config) at cap 200 for source files, 800 for test files, with `skipBlankLines` + `skipComments` (Stories 98.1-FE, 99.1-FE). `next lint` is deprecated and does NOT load `frontend/.eslintrc.json` — enforcement is exclusively through `npx eslint` from monorepo root. 114 → 112 in Story 100.1-FE (2 warnings removed with deleted tax-calculations.ts). 112 → 129 in Story 109.6-FE (17 new jsx-a11y warns). |
