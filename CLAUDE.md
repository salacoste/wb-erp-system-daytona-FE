...
| Vitest | `npm test -- --run` | ≥ 19579 passing / 0 failed (floor: 19118 after 174.2 dead-test deletion; +237 from 174.3 window; +8 contract tests 174.4; +52 redact suite debt-FE-D9; +6 nonce-mint suite D-1/PB-1; +3 urgency-tier suite C15; +12 reactive-refresh suite D-2/PB-3; +3 wave-3 AA re-pins; +9 /80-sweep style pins; +16 FE-D3 sanitizer pins; +28 FE-D1 retry/ApiError-preservation pins; +29 FE-D5 web-locks/claim suite; +16 fe-d3-family hook-fallback pins; +22 wave-6 WCAG style pins; +11 route-guards exact-array pins, session-9; +3 C5-W1 token-contract/stack pins; +6 C5-W4 waterfall guard rewrite/numeric contrast/supplies re-pins) |
| UI boundary | `node scripts/check-shadcn-ui-boundary.mjs` | **TERMINAL: 0 = baseline 0 AND `exceptions = 0 registered` (C5 complete 2026-09-12: all 22 suppressed sites migrated — waterfall 11, PriceHistorySheet 6, FunnelTab 5; register emptied; canon in Design System § Chart-Color Canon). Any increase = STOP.** |
...
### Chart-Color Canon (C5, terminal — 2026-09-12)

All chart/status color values in TS read **only** from CSS variables: strings = `var(--color-<role>)` full form; classes = semantic utilities (`bg-valence-1`, `text-status-information`, alpha modifiers `/10`). Raw triplets (`var(--valence-1)`) are NOT colors (resolve to black/transparent); `hsl(var(--x))` is banned in TS strings (re-inflates the boundary checker's CONTEXTUAL_HEX). Tints = `color-mix(in srgb, var(--color-token) N%, var(--color-card))`. Hex literals are banned for new code. Enforcer: `node scripts/check-shadcn-ui-boundary.mjs` (ratchet baseline 0, **exceptions register = 0** — the 3 legacy exceptions were lifted at C5-W4). Byte-twin roles (e.g. `chart-4 ≡ chart-positive` both themes, `chart-8 ≡ chart-negative` dark) are accepted W1 riders — don't remap without an owner decision.

**WCAG 1.4.11 verdict (owner decision (a), 2026-09-12)**: closed for **dark theme (7.6–14.0:1) + component pairs** (waterfall sign/category bars, warn borders ≥3:1 solid `border-status-warning`, TrendGraph banner 5.00:1 AA-pass). **Legacy-light is accepted**: light valence family + chart-7/8/10 as TEXT on white = 1.92–3.76:1 (pixel-identical to legacy; chart-9 light = 4.83 AA-PASS, outside the fork). All valence consumers are fills/swatches, not text — this preserves legacy rendering, not a regression.
