...
Empirical evidence (4-of-4 user-invoked 4th-passes surfacing substantive findings — basis for the MANDATORY promotion):
...
**Observation 1 — 4-pass empirical bound (fixed point)**: 6 of 6 most recent novel-pattern / discipline-codification stories converged on 4-pass chain length INDEPENDENT of finding density (1.79× range observed when including Story 115.1's post-4th-pass cumulative — 14 to 25):

- Story 112.3-FE (25 cumulative findings — canonical count per `grep -c "^- F-" 112-3-fe-anomaly-resolution-admin-ui.md`) → 4 passes
- Story 112.4-FE (25 cumulative findings) → 4 passes
- Story 113.1-FE (19 cumulative findings) → 4 passes
- Story 113.2-FE (18 cumulative findings) → 4 passes
- Story 114.1-FE (16 cumulative findings — 5+5+3+3; closed at 3 then extended to 4 by user-invoked post-close `/code-review 114.1`) → 4 passes — **empirical-bound deviation test outcome: bound HOLDS; 3-pass close was reverted by the user-invoked 4th-pass mechanism Story 114.1's own Lesson 1 attests to**
- Story 115.1-FE (14 cumulative findings — 6+5+1+2; closed at 3 then extended to 4 by user-invoked post-close `/code-review 115.1` — **identical pattern to Story 114.1-FE**) → 4 passes — **6-of-6 consecutive validation of 4-pass bound; close-at-3 reverted twice in a row by the very mechanism Story 115.1 codifies (User-invoked post-close 4th-pass record subsection)**
...
**Observation 4 — Recursive self-validation is structural**: discipline-codification stories systematically self-demonstrate the rules they ship. Empirical instances: Pattern 4 family Stories 97.1-FE through 97.5-FE (5 stories; 97.4-FE = origin per Epic 97-FE A-4 codification of "structurally permanent" framing), Story 111.1-FE (Lessons-length validator + AP#10), Stories 112.3-FE / 112.4-FE / 113.1-FE / 113.2-FE, Story 114.1-FE itself (this promotion + Empirical observations — demonstrated the pattern across its own ≥3-pass chain) — **11+ instances spanning Epics 97/111/112/113/114** (per inline canonical-examples list in Trigger 4 above). The rule operates correctly at the meta-level by accident of the codification's structure. Practical implication: every discipline-codification story should expect to fully self-demonstrate its own rule before close. Trigger 4 MANDATORY (Story 114.1-FE promotion) ensures the discipline does not depend on user intervention.
...
**Empirical record (5-of-5 user-invoked post-close 4th-passes surfacing substantive findings)**:
...
**Recommended invocation pattern**: for discipline-codification stories, user/coordinator invokes `/code-review <story>` post-close as a structural-discipline check on close-row narrative + Lessons accuracy. The mechanism has **5-of-5 incidence at surfacing substantive findings in the current codification-story corpus** (still small N; sampling biased toward stories where coordinators chose to invoke, but the small-N caveat from Story 115.1-FE 1st-pass F-5 is partially resolved — 5-of-5 with one additional observation). Recommend routine post-close practice for discipline-codification stories.

**Relationship to 4-pass empirical bound** (Observation 1 above): the user-invoked post-close 4th-pass is one of the two mechanisms (alongside in-chain recursive meta-claim self-validation) that produces the 4-pass empirical bound observed across the 5-of-5 user-invoked stories (112.4 + 113.1 + 113.2 + 114.1 + 115.1). **Story 112.3-FE's 4-pass length was achieved via in-chain Trigger 1 + Trigger 3 escalation (NOT user-invoked) — making 112.3 a complementary data point for Observation 1's 6-of-6 bound but NOT for THIS mechanism's empirical record.** **The bound is a property of the multi-pass review process itself**, robust to rule refinement — when one mechanism shortens the chain (e.g., adopted-up-front blanket qualifier), another extends it (e.g., user-invoked post-close 4th-pass catching attestation drift).
