...
8. `docs/HANDOFF-2026-09-03-V15-SESSION2-EXECUTION-AND-REMAINING-BACKLOG.md` — the session-2 record and reference (statuses updated in place by session-3, which superseded it as the entry point): what session-2 shipped (8 PRs), §3 catalogs, §4 process canon + session-2 traps.
9. `docs/ORCHESTRATOR-PROMPT-2026-09-03-V16-SESSION3-CONTINUATION-OMC.md` — the session-3 orchestration prompt: OMC sub-agent delegation (delegation matrix, model tiers, forbidden surfaces including the parity script, boundary exceptions, pinned scenarioIds), the per-item control loop, and the work order. Its entry-point pointer now leads to the SESSION3 handoff below.
10. `docs/HANDOFF-2026-09-05-V16-SESSION3-EXECUTION-AND-REMAINING-BACKLOG.md` — **the current entry point (session-4)**: sessions-2/3 executed (13 PRs; P1 fully closed, P2 waves 1-3; D-2 merged PR #403 with the live refresh chain verified; wave-3 merged PR #404 with the layered-compositing discovery, floor 19,439, boundary 372), live gate/environment state (main `afb2915f`, 2026-09-05), the prioritized remaining backlog (**§3.0 next item: boundary wave-4 "component families"** with live residual counts — the 174.2 catalog drifts, always re-scan live; §3.1 wave-5 lib-residue, §3.2 `/80` sweep + FE-D1/D3/D5, §3.4 BE questions, §3.5 owner-decision ledger), the full technical-debt register (§4), and the execution canon (§5: the wave-3 layered-compositing model is mandatory). Priority on conflict: item mini-plan > SESSION3 handoff > V16 prompt > CLAUDE.md > previous handoffs — and **live gate runs are the final authority** because numbers in the docs drift.
...
- **Session-3 (2026-09-03/05, V16)** — record: `docs/HANDOFF-2026-09-05-V16-SESSION3-EXECUTION-AND-REMAINING-BACKLOG.md` (now the entry point for session-4). Executed: **D-2 (PB-3) reactive 401-refresh merged as PR #403** (`f772eee6` — interceptor with single-flight refresh from the store, cascade gate, replay-once, 10s deadline, nonce-safe `store.refreshToken()` counter-fix, opt-outs ×3, G4=12, e2e EXIT=0, **live chain verified** on a locally rebuilt BE: refresh 200 / rotation / 401 `TOKEN_REVOKED`; floor → 19,436) and **P2 wave-3 "AA quick-wins" merged as PR #404** (2026-09-05, 3 review passes, floor 19,436 → **19,439**, boundary unchanged at 372 — semantic tokens only; artifact `debt-p2-wave3-aa-quickwins.md` is the new contrast canon, with the falsified "over card" model replaced by layered compositing). Remaining order: **boundary wave-4 component families (next)**, wave-5 lib-residue, the `/80` sweep, FE-D3/D1/D5, logger-redact wave, then P3 items.
