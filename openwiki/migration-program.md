...
sources:
  ...
  - id: openwiki-source-6b6793846d84fcde352d3d3e
    resource: repo://_bmad-output/implementation-artifacts/debt-p3-prettier-md-docs.md
...
14. `docs/HANDOFF-2026-09-09-V19-SESSION9-PRETTIER-BROWSER02-PRIVACY-EXECUTED-AND-REMAINING-BACKLOG.md` — **the current entry point and session baseline (V19, 2026-09-09)**: session-9 executed 4 items / 6 PRs (#420-#425), all merged, cleanup 0/0/0 ×4 (details below in the debt session history). Live state on main `be325f2a`: Vitest **19,559/0**, lint 0/0, tsc 0, webpack build clean, boundary 118 = baseline with 3 exceptions, docs 95, locale 4, lessons 0, privacy exit 0 on bare main (after #423/#424/#425), 174.3 contracts 33/33, and treewide `prettier --embedded-language-formatting=off --check "docs/**/*.md"` exit 0. Process canon remains the V18 prompt (§0/§2/§5/§8 unchanged); the next orchestrator prompt (V20) must point at this handoff. Priority on conflict: item mini-plan > V19 handoff > V18 prompt > CLAUDE.md > earlier handoffs — and **live gate runs are the final authority** because numbers in the docs drift.
...
- **Session-9 (V19, 2026-09-08/09) — the current baseline** — 4 items / 6 PRs, all merged, cleanup 0/0/0 ×4 (record: `docs/HANDOFF-2026-09-09-V19-SESSION9-PRETTIER-BROWSER02-PRIVACY-EXECUTED-AND-REMAINING-BACKLOG.md`):
...
- **P3 process leftovers (V19 §3.0)** — the remaining window items after session-9: unifying ~25 route guards (behavior class, mini-plan + opus executor, tests-first; the largest live item); harness restart-per-run (test infrastructure); the FR-7 / AT-matrix / Manager-creds test epics; the docs-95 baseline canonical-vs-archival split; and deleting the stale pm2 registration — **re-identify first**: pm2 id 5 is currently the live `wb-repricer-frontend-dev` process and must not be deleted; reconcile which stale process V16 meant.

The standing FE-debt table (FE-D1…FE-D9), wave carry-outs C1–C18, BE-debt, and contrast escalations remain catalogued in `_bmad-output/planning-artifacts/shadcn-migration-status-and-debt-registry.md` (final program snapshot 2026-09-02, then extended in place by appended debt-session sections: §9 `/80`-sweep, §10 FE-D3 + follow-ups, §11 FE-D1, §12 FE-D5, §13 fe-d3-family, §14 WCAG wave-6, §15 dead queryClient, §16 prettier-md, §17 CABINET-BROWSER-02 + privacy-residual birth, §18 docs redaction + tool-dir quirks, §19 scanner exclusion with accepted residual), cross-referenced against the final handoff's escalation register. The debt-session Vitest floor trajectory: 19,363 (program close) → 19,439 (wave-3) → 19,448/19,464 (session-6) → 19,492 (FE-D1) → 19,521/19,537/19,559 (session-8; the current CLAUDE.md floor is **19,559/0**, unchanged by session-9, whose PRs were docs and gate-script scoped).
...
nder `.omx/plans/` for the executable detail. For frontend stories, the practical takeaway is: the paid-storage wire contract above is authoritative, and no other story may touch backend contracts.
 `.omx/plans/` for the executable detail. For frontend stories, the practical takeaway is: the paid-storage wire contract above is authoritative, and no other story may touch backend contracts.
