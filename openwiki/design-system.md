...
sources:
  ...
  - id: openwiki-source-de60c3a1a7a87a20895fd687
    resource: repo://src/app/(dashboard)/analytics/advertising/components/__tests__/advertising-presentation-source-contracts.test.tsx
  ...
  - id: openwiki-source-d8906ab17e9d120e28d49782
    resource: repo://src/app/(dashboard)/analytics/ai-admin/anomalies/components/ResolveAnomalyDialog.tsx
  - id: openwiki-source-0aba79c5c49b3c70614c8b51
    resource: repo://src/app/(dashboard)/analytics/ai-admin/models/components/AdminModelsContent.tsx
  - id: openwiki-source-5f1a3eb935b7eb5112f97612
    resource: repo://src/app/(dashboard)/analytics/ai-admin/preferences/components/AiPreferencesForm.tsx
  - id: openwiki-source-3ee95ab14cd5d2ee1ec10270
    resource: repo://src/app/(dashboard)/analytics/cross-reference/components/CrossReferencePageContent.tsx
  - id: openwiki-source-fd416faea877b6a45bd79e4b
  ...
  - id: openwiki-source-eb60b394335ca65828eef992
    resource: repo://src/components/product/metrics/__tests__/metric-composition-source-contracts.test.ts
  ...
  - id: openwiki-source-0e1fc0457c1d1c291ebb6df1
    resource: repo://src/components/product/states/__tests__/state-composition-source-contracts.test.ts
...
- `_bmad-output/planning-artifacts/shadcn-migration-status-and-debt-registry.md` — rolling status/debt snapshot of the migration waves (latest snapshot text: 2026-08-26, superseded for status by `_bmad-output/implementation-artifacts/sprint-status.yaml` last_updated 2026-08-27; 64/94 canonical stories complete; Epics 166–168, 169, 170, 171 closed; 172 in progress at 7/17, NEXT = 172.8).
...
**Epic 172 (business operations, 17 stories) is IN PROGRESS at 7/17** (PRs #278–#293, 2026-08-26/27): 172.1 business dashboard (PR #278, FULL cycle — 127 files across `src/app/(dashboard)/dashboard/**` and the `src/components/custom/dashboard/**` family incl. `BaseMetricCard`, executed in four delegated waves; `DashboardStatusStrip` consolidates the 8 conditional banners into one expandable status line using `status-*` token tones while children stay mounted via `hidden` so banner state and DOM assertions are preserved; `dashboard-presentation-source-contracts` + `dashboard-widgets-presentation-source-contracts` guards added, full floor 19 281 → 19 297); 172.2 canned automation rules gallery (PR #280 — `CannedRulesGallery` born-clean on merged shadcn primitives, py-6 debt and raw-button closed, new gallery e2e package with a fixture controller); 172.3 installed rules list (PR #282 — status tokens across badge/safety/banner, incl. `InstalledRuleRow`); 172.4 installed rule detail/editor (PR #285 — editor status tokens, `WritebackSafetyAcknowledgement` in the editor, and the 163.3 editor spec finally live 8/8); 172.5 single-product COGS (PR #287 — 23 files to 0 palette incl. two live transitives `MarginCalculationStatus` polling and `ResizableTableHead` grip; guard catalog pinned at 21, 11 re-pins; floor 19 319 → 19 327); 172.6 bulk COGS assignment (PR #289 — 8 production files to 0 palette incl. 2 blue-override drops, floor → 19 334); 172.7 COGS history (PR #293 — born-clean caption contract `captionText`→`TableCaption` per the 171.9 canon, tabular-nums, floor → 19 343). The dashboard, the automation domain (gallery + list + editor), and the COGS domain (single/bulk/history) are migrated end-to-end. **NEXT = 172.8 price calculator** (cross-team OMC handoff: `docs/HANDOFF-2026-08-27-CROSS-TEAM-OMC-ORCHESTRATOR-172-8-CONTINUATION.md`; only 172.14 remains owner-coordinated). Epics 173 (13 stories) and 174 (5 stories, final consolidation) remain in backlog. The formerly parallel 169 lane is closed (15/15).
...
Current guards: `advertising`, `anomalies`, `brand-share` (in `src/components/custom/analytics/__tests__/`), `campaign-detail` (in `src/components/custom/advertising/__tests__/`), `cross-reference`, `forecast`, `forecast-accuracy`, `funnel`, `gaps`, `liquidity`, `model-registry`, `returns`, `search`, `storage`, `supply-planning`; the models `[id]` subroutes from Epic 171 — `evaluations-list`, `sku-accuracy`, `model-performance` (under `src/app/(dashboard)/analytics/models/[id]/`); and the Epic 172 family — `dashboard`, `dashboard-widgets` (in `src/components/custom/dashboard/__tests__/`), `canned-rules`, `installed-rules`, `installed-rule-editor` (under `src/app/(dashboard)/automation/`); and the COGS domain from Epic 172 — `cogs-single` (`src/app/(dashboard)/cogs/__tests__/cogs-single-presentation-source-contracts.test.ts`), `bulk-cogs` (`src/app/(dashboard)/cogs/bulk/__tests__/bulk-cogs-presentation-source-contracts.test.ts`), `cogs-history` (`src/app/(dashboard)/cogs/history/__tests__/cogs-history-presentation-source-contracts.test.ts`). The 171.8 anchor-safe lesson is canon for all relative-path guards: filter relative segments before joining to an absolute path, or a sibling worktree whose plan-pinned name contains the subtree name will silently empty the owned-file catalog.

## Remaining migration debt registry

Per `_bmad-output/planning-artifacts/shadcn-migration-status-and-debt-registry.md`, `_bmad-output/implementation-artifacts/sprint-status.yaml` (last_updated 2026-08-27), and `docs/HANDOFF-2026-08-27-CROSS-TEAM-OMC-ORCHESTRATOR-172-8-CONTINUATION.md` (64/94 canonical stories; Epic 169 closed 15/15, Epic 172 at 7/17; full Vitest floor ≥ 19 356 passing per `CLAUDE.md`, lint 0/0, tsc 0):

| Debt | Owner / due |
|------|-------------|
| Epic 172 remainder: 172.8–172.17 (NEXT = 172.8 COGS price calculator; 172.14 owner-coordinated) | Current lane |
