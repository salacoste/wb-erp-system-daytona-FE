...
Remaining backlog: the **FE-debt queue is EMPTY** — the session-14 waves closed every owner-decision item (A2 OrganicTab, apiClient sanitization, financial tokens / logger-redact / scanner bundle, docs-baseline split), the boundary canon is terminal, registry rows §34–§41 are closed, and there are no open PRs. Per the handoff §4 scope boundary (updated after PR #457): **all BE-side debts — including execution of request-backend #231 (NestJS filter audit for raw pg/redis driver leaks in error envelopes), the D-2 remote publish of the backend refresh branch, and a BE-seed for e2e Manager-creds — are outside this FE handoff** and are tracked by the BE team in their own tracking; they are deliberately not listed as open FE items. The FE side of each is closed or not required. Route new backend-blocking work through `docs/request-backend/NNN-*.md` requests (e.g. the existing `docs/request-backend/231-error-envelope-raw-driver-leak-audit.md`). New FE work means product stories or new owner decisions only.
