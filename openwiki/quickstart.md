...
Remaining backlog: no unblocked implementation items — only the 3 owner-decision items with prepared AskUserQuestion requests in handoff §3 (A2 OrganicTab `/80` tier; apiClient-wide sanitization ~131 `.tsx` echo sites; the financial tokens / logger-redact / scanner-semantics bundle of 3 sub-decisions) and BE follow-ups (remote publish of the D-2 refresh branch, FE-D3-residual NestJS filters, BE-seed for Manager-creds). Implementing any owner-decision item without an owner answer is prohibited (V21 §8, inherited by V22). The current orchestrator prompt is V22 (`docs/ORCHESTRATOR-PROMPT-2026-09-12-V22-OWNER-DECISIONS-QUEUE-OMC-SUBAGENTS.md`): session-14 runs the 3-item owner-decision queue via AskUserQuestion first, then executes only the chosen options; no owner answer → STOP. Route backend-blocking work to `docs/request-backend/` (e.g. `docs/request-backend/230-auth-refresh-endpoint-missing.md`).
