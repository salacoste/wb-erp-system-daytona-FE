...
const getPriceGapStyles = (pct: number) => {
  if (pct > 20) {
    return {
      container:
        'p-3 rounded-lg bg-financial-positive/5 text-financial-positive border border-financial-positive/30',
      icon: TrendingUp,
    }
  }
  if (pct > 10) {
    return {
      container:
        'p-3 rounded-lg bg-status-warning text-status-warning-foreground border border-status-warning',
      icon: TrendingUp,
    }
  }
  return {
