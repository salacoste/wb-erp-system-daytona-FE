...
export type BrandSortField = AggregatedSortField
...
export function MarginByBrandTable({
  ...
  const sortedData = useMemo(
    () => [...data].sort((a, b) => compareAggregatedItems(a, b, sortField, sortOrder, 'brand')),
  ...
  if (!data || data.length === 0) {
    return (
      /* Story 170.3: empty-state tokens — mirrors page.tsx page-level empty branch. */
  ...
  return (
    <div className="rounded-md border overflow-x-auto">
      {/* Story 170.3: tabular-nums inherits into shared Header/Row cells (no shared
      ...
      <Table
        ...
        scrollContainerAriaLabel="Таблица маржинальности по брендам"
      >
        {/* Story 170.3: picker-semantic caption (169.7) — names the analysis without
            verbatim-duplicating the adjacent CardTitle. */}
        <TableCaption>Таблица маржинальности по брендам</TableCaption>
        ...
        <TableBody>
          {sortedData.map((item, index) => (
            <MarginAggregatedTableRow
              ...
              rowKey={item.brand || index}
              item={item}
              entityField="brand"
              entityFallback="(Без бренда)"
