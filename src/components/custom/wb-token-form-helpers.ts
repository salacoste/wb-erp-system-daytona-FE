...
// fe-d3-family: the sanitizer moved to its canonical home (lib, not components —
// hooks must not import from components/). Imported for local use by
// getErrorMessage's fallback branch AND re-exported so existing importers
// (incl. the SHA-pinned wb-token-form-helpers.test.ts) keep resolving.
import { sanitizeFallbackMessage } from '@/lib/sanitize-fallback-message'
export { sanitizeFallbackMessage }
