...
describe('MetricCard', () => {
  ...
  it('displays error message when error is provided', { timeout: 5000 }, async () => {
    render(<MetricCard title="Total Payable" value={1000} error="Ошибка загрузки" />)
  ...
  it('applies custom className', { timeout: 5000 }, async () => {
    const { container } = render(<MetricCard title="Test" value={1000} className="custom-class" />)
  ...
  it('prioritizes error over loading state', { timeout: 5000 }, async () => {
    render(<MetricCard title="Test" value={1000} isLoading={true} error="Error message" />)
