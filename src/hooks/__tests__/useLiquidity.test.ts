...
describe('useLiquidity', () => {
  ...
  describe('loading states', () => {
    ...
    it('should respect enabled option', async () => {
      const { result } = renderHookWithClient(() => useLiquidity({}, { enabled: false }))
...
describe('useLiquidityTrends', () => {
  ...
  it('should fetch trends with custom period', async () => {
    const { result } = renderHookWithClient(() => useLiquidityTrends({ period: 30 }))
...
describe('summary data structure', () => {
  ...
  it('should have correct benchmarks structure', async () => {
    ...
    expect(['excellent', 'good', 'warning', 'critical']).toContain(benchmarks?.overall_status)
  })
})
