...
function iroasLabel(interp: string | null): { text: string; cls: string; icon: string | null } {
  ...
  // light; dark passes at every alpha — all numbers vs bg-card):
  // /90 fails at 4.23 (light; dark 7.76), only /95+ passes at 4.66 (light;
  // dark 8.55) — visually indistinguishable from full, so no alpha can carry
  // the tier distance on text. Both positive tiers now
