 ...
 *
 * Features:
 * - View aggregated margin analysis by brand
 * - Sortable table (margin, revenue, profit, brand)
...
export default function MarginAnalysisByBrandPage() {
  ...
  // Handle brand click - drill down to SKU level
  const handleBrandClick = (brand: string) => {
    // Navigate to SKU analytics with brand filter and date range
    router.push(`/analytics/sku?weekStart=${weekStart}&weekEnd=${weekEnd}&brand=${encodeURIComponent(brand)}`)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Агрегация:</strong> Данные суммируются по всем товарам каждого бренда. Кликните
          на строку бренда для просмотра детализации по товарам (SKU).
        </AlertDescription>
      </Alert>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Всего брендов:</span>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Кликните на строку для детализации по товарам бренда. Сортировка по клику на
            заголовок столбца.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {data?.data ? (
            <MarginByBrandTable data={data.data} onBrandClick={handleBrandClick} />
          ) : (
        ...
        <CardContent className="space-y-3 text-sm text-blue-900">
          <div>
            ...
            <p className="mt-1 text-blue-800">
              Все показатели суммируются по товарам каждого бренда. Средняя маржа рассчитывается
          ...
          <div>
            ...
            <p className="mt-1 text-blue-800">
              Кликните на бренд для просмотра детализации по товарам (SKU). Это позволяет
              выявить наиболее и наименее прибыльные товары внутри бренда.
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-1 text-blue-800">
              Зелёный — прибыльные бренды. Красный — убыточные бренды. Жёлтый фон — есть товары
              без себестоимости.
            </p>
          </div>
        </CardContent>
