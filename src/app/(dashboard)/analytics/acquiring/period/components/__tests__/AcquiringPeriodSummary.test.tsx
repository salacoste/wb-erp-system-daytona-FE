/**
 * Tests for AcquiringPeriodSummary
 * Epic 90-FE Story 90.4: Acquiring Period Detail View
 *
 * Adapted from Story 90.3's AcquiringReportDetailSummary.test.tsx.
 * Null-vs-zero discipline: null fee/VAT excluded from sum, footnote shown.
 */
...
describe('AcquiringPeriodSummary', () => {
  ...
  it('excludes null items from sum and shows pluralized footnote', () => {
    ...
    // Both fee and VAT each have 1 null → two footnotes (getAllByText required)
  ...
  it('handles empty transactions: 0 fees, 0 VAT, 0 count, no footnotes', () => {
    render(<AcquiringPeriodSummary transactions={[]} />)
  ...
  it('single transaction with null VAT: fee sums correctly, VAT footnote appears', () => {
    const transactions = [makeTransaction({ rrdId: 1, acquiringFee: 500, acquiringFeeVat: null })]
    render(<AcquiringPeriodSummary transactions={transactions} />)

    // Fee total: 500 ₽
    expect(screen.getByText(/500\s*₽/)).toBeInTheDocument()
    // VAT has 1 null → singular form "транзакция"
    expect(screen.getByText(/не включает 1 транзакция с неизвестными/)).toBeInTheDocument()
    // Count = 1
