...
export function MergedGroupRows({ group, onProductClick }: MergedGroupRowsProps) {
  ...
  const rowspanClasses =
    'px-4 py-4 text-center align-middle bg-muted border-r-2 border-border text-sm font-medium text-muted-foreground md:sticky md:left-0 md:z-10'
  ...
  const detailRowClasses =
    'bg-card hover:bg-muted cursor-pointer transition-colors border-b border-border'
    ...
    <>
      ...
      {group.products.map(product => (
        ...
        >
          ...
          {/* Story 37.4 AC 19-20, 25: Sticky on tablet/mobile */}
          <td
            className={`${detailCellClasses} text-left ${!hasSingleProduct ? 'md:sticky md:left-[150px] md:z-10 md:bg-card' : 'md:sticky md:left-0 md:z-10 md:bg-card'}`}
          >
