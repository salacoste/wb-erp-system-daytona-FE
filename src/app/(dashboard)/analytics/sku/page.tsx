      ...
      <div className="space-y-6">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <div className="flex items-center justify-between">
              <span>
                {errorSkuFinancials instanceof Error
                  ? errorSkuFinancials.message
  ...
  const stats =
    skuData.length > 0
      ? {
          ...
          totalProfit: skuData
            .filter(item => !item.missingCogs)
            .reduce((sum, item) => sum + item.profit.operating, 0),
        }
      : null
        ...
        <Alert className="border-blue-200 bg-blue-50">
          ...
          <AlertDescription className="space-y-2">
            ...
            <div className="text-blue-700 text-sm">
              <strong>Прибыль</strong> = Выручка − COGS − Все расходы (логистика, хранение, комиссия
              WB, эквайринг, штрафы и др.)
            </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between p-3 bg-green-100 rounded-lg border border-green-300">
                        ...
                        <span className="text-lg font-bold text-green-700">
                          {cabinetExpenses.sales_gross.toLocaleString('ru-RU', {
                      ...
                      <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                        ...
                        <span className="text-lg font-bold text-red-600">
                          {cabinetExpenses.marketplace_commission.toLocaleString('ru-RU', {
                            maximumFractionDigits: 2,
                          })}{' '}
                          ₽
                        </span>
                      ...
                      <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                        ...
                        <span className="text-lg font-bold text-red-600">
                          {(cabinetExpenses.acquiring_fee ?? 0).toLocaleString('ru-RU', {
                            maximumFractionDigits: 2,
                          })}{' '}
                          ₽
                        </span>
                      ...
                      <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                        ...
                        <span className="text-lg font-bold text-red-600">
                          {cabinetExpenses.cogs_total.toLocaleString('ru-RU', {
                            maximumFractionDigits: 2,
                          })}{' '}
                          ₽
                        </span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`ml-1 px-1.5 py-0.5 text-xs font-medium rounded ${
                              cabinetExpenses.gross_profit_sku >= 0
                                ? 'bg-blue-200 text-blue-700'
                                : 'bg-red-200 text-red-700'
                            }`}
                          >
                            {pct(cabinetExpenses.gross_profit_sku)}%
                          </span>
                        </div>
                        <span
                          className={`text-xl font-bold ${cabinetExpenses.gross_profit_sku >= 0 ? 'text-blue-700' : 'text-red-600'}`}
                        >
                          {cabinetExpenses.gross_profit_sku.toLocaleString('ru-RU', {
                            maximumFractionDigits: 2,
                          })}{' '}
                          ₽
                        </span>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                        <div className="text-center p-2 bg-amber-50 rounded border border-amber-200">
                          ...
                          <div className="text-sm font-bold text-amber-800">
                            {(cabinetExpenses.logistics ?? 0).toLocaleString('ru-RU', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            ₽
                          </div>
                          <div className="text-xs text-amber-500">
                            {pct(cabinetExpenses.logistics ?? 0)}%
                          </div>
                        </div>
                        <div
                        ...
                        >
                          ...
                          <div className="text-sm font-bold text-amber-800">
                            {cabinetExpenses.storage.toLocaleString('ru-RU', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            ₽
                          </div>
                          <div className="text-xs text-amber-500">
                            {pct(cabinetExpenses.storage)}%
                          </div>
                        ...
                        <div className="text-center p-2 bg-amber-50 rounded border border-amber-200">
                          ...
                          <div className="text-sm font-bold text-amber-800">
                            {cabinetExpenses.other_adjustments.toLocaleString('ru-RU', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            ₽
                          </div>
                          <div className="text-xs text-amber-500">
                            {pct(cabinetExpenses.other_adjustments)}%
                          </div>
                        </div>
                        <div className="text-center p-2 bg-amber-50 rounded border border-amber-200">
                          <div className="text-xs text-amber-600">Коррект. ВВ</div>
                          <div className="text-sm font-bold text-amber-800">
                            {cabinetExpenses.wb_commission_adj.toLocaleString('ru-RU', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            ₽
                          </div>
                          <div className="text-xs text-amber-500">
                            {pct(cabinetExpenses.wb_commission_adj)}%
                          </div>
                        </div>
                        <div className="text-center p-2 bg-amber-50 rounded border border-amber-200">
                          <div className="text-xs text-amber-600">Штрафы</div>
                          <div className="text-sm font-bold text-amber-800">
                            {cabinetExpenses.penalties.toLocaleString('ru-RU', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            ₽
                          </div>
                          <div className="text-xs text-amber-500">
                            {pct(cabinetExpenses.penalties)}%
                          </div>
                        </div>
                        <div className="text-center p-2 bg-amber-50 rounded border border-amber-200">
                          <div className="text-xs text-amber-600">Платн. приёмка</div>
                          <div className="text-sm font-bold text-amber-800">
                            {cabinetExpenses.paid_acceptance.toLocaleString('ru-RU', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            ₽
                          </div>
                          <div className="text-xs text-amber-500">
                            {pct(cabinetExpenses.paid_acceptance)}%
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 bg-amber-100 rounded-lg border border-amber-300">
                        ...
                        <span className="text-lg font-bold text-amber-700">
                          {cabinetExpenses.total.toLocaleString('ru-RU', {
                            maximumFractionDigits: 2,
                          })}{' '}
                          ₽
                        </span>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`font-bold text-xl ${netProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            =
                          </span>
                          <span
                            className={`text-base font-semibold ${netProfit >= 0 ? 'text-green-800' : 'text-red-800'}`}
                          >
                            ЧИСТАЯ ПРИБЫЛЬ
                          </span>
                          <span
                            className={`ml-1 px-2 py-0.5 text-sm font-bold rounded ${
                              netProfit >= 0
                        ...
                        <span
                          className={`text-2xl font-bold ${netProfit >= 0 ? 'text-green-700' : 'text-red-600'}`}
                        >
