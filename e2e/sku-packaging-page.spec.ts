/**
 * E2E Tests: SKU Packaging Management Page
 * Route: /shipments/sku-packaging
 * Page: src/app/(dashboard)/shipments/sku-packaging/page.tsx
 *
 ...
 *
 * Run: npx playwright test e2e/sku-packaging-page.spec.ts
 */
...
test.describe('SKU Packaging Management Page', () => {
  ...
  test('renders page heading "Упаковка товаров"', async ({ page }) => {
  ...
  test('shows table, empty state, or error with retry', async ({ page }) => {
    ...
    const hasEmptyState = (await page.getByText(/добавить упаковку/i).count()) > 0
